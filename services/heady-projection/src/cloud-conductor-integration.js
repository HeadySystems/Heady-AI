...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: services/heady-projection/src/cloud-conductor-integration.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
...
const {
  ProjectionSwarm
} = require('./projection-swarm');
const {
  integrateConductor
} = require('./conductor-integration');
const {
  broadcastUpdate,
  bindToProjectionSource,
  closeAll: closeAllSSE
} = require('./projection-sse');
...
function createCloudConductorService(config = {}) {
  ...
  function _getConductor() {
    if (!_conductor) {
      try {
        ...
        _conductor.route = async task => ({
          serviceGroup: 'projection',
          weight: 0.45
        });
        _conductor.getStatus = () => ({
          shim: true
        });
      }
    }
  ...
  function _loadProjectionBees(swarm) {
    const beeModules = [{
      path: './health-bee',
      domain: 'health-projection',
      priority: 0.95
    }, {
      path: './config-bee',
      domain: 'config-projection',
      priority: 0.90
    }, {
      path: './telemetry-bee',
      ...
      path: './vector-bee',
      domain: 'vector-projection',
      priority: 0.70
    }, {
      path: './task-bee',
      domain: 'task-projection',
      priority: 0.65
    }, {
      path: './topology-bee',
      domain: 'topology-projection',
      priority: 0.60
    }];
  ...
  app.post('/pubsub/push', async (req, res) => {
    ...
    try {
      ...
      logger.error({
        err: err.message
      }, '[CloudConductor] Pub/Sub push error');
      _auditWrite({
        event: 'pubsub:error',
        error: err.message
      });
  ...
  app.get('/health', (req, res) => {
    ...
    const body = {
      ...
      swarm: {
        ...
        errorRate: swarmStatus.telemetry?.totalRuns > 0 ? +(swarmStatus.telemetry.totalErrors / swarmStatus.telemetry.totalRuns).toFixed(4) : 0
      },
      pubsub: {
  ...
  app.get('/status', (req, res) => {
    try {
      ...
      res.json({
        ...
        manager: manager.getStatus ? manager.getStatus() : {
          shim: true
        },
        conductor: conductor.getStatus ? conductor.getStatus() : {
          shim: true
        },
        pubsub: {
          processed: _pubsubMessagesProcessed,
  ...
  app.use((err, req, res, next) => {
    logger.error({
      err: err.message,
      path: req.path
    }, '[CloudConductor] Unhandled error');
    res.status(500).json({
      ok: false,
      error: err.message
    });
...
if (require.main === module) {
  ...
  app.start().then(server => {
    ...
    logger.error({
      err: err.message
    }, '[CloudConductor] Fatal startup error');
    process.exit(1);
  });
}
