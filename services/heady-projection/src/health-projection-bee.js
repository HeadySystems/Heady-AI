...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: services/heady-projection/src/health-projection-bee.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
...
function getServices() {
  ...
  return [{
    ...
    name: 'vector-store',
    url: "http://0.0.0.0:3001/health",
    critical: true
  }, {
    name: 'swarm-coordinator',
    url: "http://0.0.0.0:3002/health",
    critical: true
  }, {
    name: 'task-runner',
    url: "http://0.0.0.0:3003/health",
    critical: false
  }, {
    name: 'telemetry-sink',
    url: "http://0.0.0.0:3004/health",
    critical: false
  }];
}
  ...
  return async function probeServices() {
    ...
    await Promise.all(services.map(async svc => {
      ...
      if (probeResult.ok) {
        ...
        logger.warn(`[${tag}] service unhealthy`, {
          service: svc.name,
          ...probeResult
        });
      }
    ...
    if (global.eventBus) {
      global.eventBus.emit('projection:health', {
        worker: tag,
        data: result
      });
    }
...
function makeAggregateHealthWorker() {
  return async function aggregateHealth() {
    ...
    if (global.eventBus) {
      global.eventBus.emit('health:score', {
        score: overallScore,
...
function getWork() {
  return [makeProbeServicesWorker(), makeAggregateHealthWorker(), makeDetectAnomaliesWorker()];
}
module.exports = {
  domain,
  description,
