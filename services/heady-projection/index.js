...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: services/heady-projection/index.js                                                    ║
// ║  LAYER: root                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
...
class ProjectionSwarm {
  ...
  async _runBee(bee) {
    ...
    try {
      ...
      const results = await Promise.allSettled(workers.slice(0, this.maxConcurrent).map(fn => fn()));
      const merged = {};
...
const vectorMemoryBee = makeStubbedBee('vector-memory', Math.round(PHI * 8000), [async () => ({
  bee: 'vector-memory',
  action: 'scan',
...
const configBee = makeStubbedBee('config', Math.round(PHI * 10000), [async () => ({
  bee: 'config',
  action: 'check',
...
const healthBee = makeStubbedBee('health', Math.round(PHI * 6000), [async () => ({
  bee: 'health',
  action: 'aggregate',
...
const telemetryBee = makeStubbedBee('telemetry', Math.round(PHI * 4000), [async () => ({
  bee: 'telemetry',
  action: 'collect',
...
const topologyBee = makeStubbedBee('topology', Math.round(PHI * 15000), [async () => ({
  bee: 'topology',
  action: 'discover',
...
const taskQueueBee = makeStubbedBee('task-queue', Math.round(PHI * 5000), [async () => ({
  bee: 'task-queue',
  action: 'poll',
...
class HeadyConductorClient {
  constructor(url) {
    this.url = url || process.env.CONDUCTOR_URL || "http://0.0.0.0:3848";
...
const swarm = new ProjectionSwarm(manager, BEES, {
  maxConcurrent: 6
});
...
process.on('unhandledRejection', r => {
  log('error', 'Unhandled rejection:', r);
});
