...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: services/heady-chrono/src/index.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
'use strict';
...
mcp.tool('health_check', 'Check HeadyChrono health and temporal coherence', {}, async () => ({
  content: [{
    type: 'text',
    text: JSON.stringify({
      status: 'operational',
...
mcp.tool('heady_chrono_trace', 'Trace causal chains across time — follow the thread of events that led to a state', {
  event_id: {
    type: 'string',
    description: 'Event identifier to trace from'
  },
  depth: {
    type: 'number',
    description: 'Max causal chain depth (default: 8)'
  },
  direction: {
    type: 'string',
    description: 'backward | forward | bidirectional'
  }
}, async ({
  event_id,
  depth = 8,
...
mcp.tool('heady_chrono_predict', 'Project future states based on temporal patterns and phi-scaled decay', {
  context: {
    type: 'string',
...
mcp.tool('heady_chrono_diff', 'Compare two points in time — what changed, what drifted, what emerged', {
  t1: {
    type: 'string',
    description: 'ISO timestamp or relative (e.g., "2h ago")'
  },
  t2: {
    type: 'string',
    description: 'ISO timestamp or "now"'
  },
  scope: {
    type: 'string',
    description: 'system | service | user | global'
  }
}, async ({
  t1,
  t2 = 'now',
...
mcp.tool('heady_chrono_replay', 'Replay system state at a historical point — time-travel debugging', {
  timestamp: {
    type: 'string',
    description: 'Point in time to replay'
  },
  service: {
    type: 'string',
    description: 'Service to replay'
  },
  include_context: {
    type: 'boolean',
...
mcp.resource('heady://chrono/timeline', 'heady://chrono/timeline', 'Live system timeline with causal annotations', 'application/json', async () => ({
  contents: [{
...
mcp.resource('heady://chrono/patterns', 'heady://chrono/patterns', 'Detected temporal patterns and recurring cycles', 'application/json', async () => ({
  contents: [{
...
serve({
  ...
  logger.info(`[HeadyChrono] Temporal Intelligence Engine projected on :${PORT}`);
  logger.info(`[HeadyChrono] SSE: http://localhost:${PORT}/sse`);
  logger.info(`[HeadyChrono] Health: http://localhost:${PORT}/health`);
});