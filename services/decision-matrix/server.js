...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: services/decision-matrix/server.js                                                    ║
// ║  LAYER: root                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
  ...
  const weighted = options.map(option => {
    ...
    criteria.forEach((c, i) => {
      ...
      scores[c.name] = {
        ...
        weighted: (score * weight).toFixed(3)
      };
    ...
    return {
      ...
      totalScore: (totalScore / totalWeight).toFixed(3)
    };
  });
  return {
    ranked: weighted.sort((a, b) => b.totalScore - a.totalScore),
    criteria,
    phiWeighted: true
  };
}
const server = http.createServer((req, res) => {
  ...
  if (parsed.pathname === '/evaluate' && req.method === 'POST') {
    ...
    req.on('end', () => {
      ...
      res.end(JSON.stringify(evaluateDecision(options || [], criteria || []), null, 2));
    });
    return;
  }
  res.end(JSON.stringify({
    ...
    version: '1.0.0',
    endpoints: {
      '/evaluate': 'POST {options, criteria}'
    }
  }));
});
const PORT = process.env.PORT || 8122;
server.listen(PORT, () => logger.info(`⚖️ Decision Matrix on :${PORT}`));
module.exports = {
  evaluateDecision
};