...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: services/heady-mcp/connector-discovery.js                                                    ║
// ║  LAYER: root                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
...
class ConnectorAutoDiscovery {
  constructor(options = {}) {
    this.searchPaths = options.searchPaths || [process.env.HOME + '/Heady', process.env.HOME + '/.agents', '/usr/local/lib/node_modules'];
  ...
  async scanFilesystem() {
    ...
    return {
      scanned: this.searchPaths.length,
      found: found.length
    };
  }
  ...
  async scanProcesses() {
    const found = [];
    try {
      const output = execSync('pgrep -la "mcp" 2>/dev/null || true', {
        encoding: 'utf-8',
        timeout: 5000
      });
  ...
  _walkDir(dir, maxDepth, depth = 0) {
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isFile()) results.push(full);else if (entry.isDirectory()) results.push(...this._walkDir(full, maxDepth, depth + 1));
      }
