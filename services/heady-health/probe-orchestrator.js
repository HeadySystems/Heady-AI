...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: services/heady-health/probe-orchestrator.js                                                    ║
// ║  LAYER: root                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
...
const DOMAINS = ['headyme.com', 'headyapi.com', 'headysystems.com', 'headyconnection.org', 'headybuddy.org', 'headymcp.com', 'headyio.com', 'headybot.com', 'heady-ai.com'];
class ProbeOrchestrator {
  ...
  async probeE2E(domain) {
    ...
    try {
      ...
      checks.push({
        check: 'api_health',
        pass: health.includes('healthy') || health.includes('status')
      });
    } catch (err) {
      checks.push({
        check: 'api_health',
  ...
  async probeVisual(domain) {
    ...
    try {
      ...
      const isHealthy = analysis.hasPublicContent && !analysis.hasErrorPage && analysis.contentLength > 500;
      return {
        level: 4,
        name: 'visual',
        domain,
  ...
  async fullSweep() {
    ...
    for (const domain of this.domains) {
      ...
      const probes = [await this.probePing(domain), await this.probeFunctional(domain), await this.probeE2E(domain), await this.probeVisual(domain), await this.probeSweep(domain)];
