...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: services/heady-security/middleware/rate-limiter-advanced.js                                                    ║
// ║  LAYER: root                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
...
const TIERS = {
  free: {
    requestsPerHour: 100,
    burstMultiplier: 1.5,
    // Allow 150% of hourly rate as burst window
  ...
  internal: {
    ...
    // Effectively unlimited for internal services
...
class InMemorySlidingWindowStore {
  ...
  async consumeToken(key, maxTokens, refillRate, refillIntervalMs) {
    ...
    return {
      allowed: false,
      tokensRemaining: 0,
      nextRefillAt: bucket.lastRefill + refillIntervalMs
    };
  }
  async reset(key) {
  ...
  _cleanup() {
    ...
    for (const [key, timestamps] of this._windows) {
      ...
      if (valid.length === 0) this._windows.delete(key);else this._windows.set(key, valid);
    }
  }
}
...
class AdvancedRateLimiter {
  ...
  async check(identity = {}) {
    ...
    for (const key of keys) {
      try {
        const windowResult = await this._store.increment(`${this._keyPrefix}${key}`, windowMs);

        // Burst check (short window token bucket)
        const burstResult = await this._store.consumeToken(`${this._keyPrefix}burst:${key}`, burstLimit, Math.ceil(burstLimit / 60),
        // refill rate: burstLimit per minute
  ...
  async _handleBreach(info) {
    ...
    if (tier?.webhookOnBreach && this._webhookUrl) {
      await this._sendWebhook(info).catch(err => logger.error('[RATE-LIMITER] Webhook error:', err.message));
    }
  }
  async _sendWebhook(info) {
    ...
    const headers = {
      ...
      'X-Heady-Signature': crypto.createHmac('sha256', process.env.WEBHOOK_SECRET || 'change-me').update(body).digest('hex')
    };
