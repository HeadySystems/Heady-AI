...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: services/heady-ide/heady-ide-server.js                                                    ║
// ║  LAYER: root                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
// ╔══════════════════════════════════════════════════════════════════╗
...
const ALLOWED_ORIGINS = (process.env.CORS_ALLOWED_ORIGINS || '').split(',').concat(['https://heady-ai.com', 'https://headyme.com', 'https://headysystems.com', 'https://headyapi.com', 'https://headymcp.com', 'https://headyio.com', 'https://headybot.com', 'https://headybuddy.org', 'https://headyconnection.org', 'https://headylens.com', 'https://headyfinance.com', "http://0.0.0.0:3000" // dev only
]).filter(Boolean);
async function main() {
  // ── Boot all services ──────────────────────────────────────────
  const services = await bootHeadyServices({
    tenantId: 'system'
  });
  const app = express();
  ...
  app.use(cors({
    origin: (origin, cb) => {
      if (!origin || ALLOWED_ORIGINS.includes(origin)) cb(null, true);else cb(new Error('CORS blocked'));
    },
    credentials: true
  }));
  ...
  app.get('/api/workspace', authMiddleware, async (req, res) => {
    if (!services.db) return res.json({
      workspaces: []
    });
    const {
      rows
    } = await services.db.query('SELECT * FROM ide_workspaces WHERE user_id = $1 ORDER BY updated_at DESC LIMIT 21', [req.tenantId]);
    res.json({
      workspaces: rows
    });
  });
  ...
  app.post('/api/chat', authMiddleware, async (req, res) => {
    ...
    try {
      const result = provider ? await services.heady.call(provider, messages, options || {}) : await services.heady.smart(messages, options || {});
  ...
  app.post('/api/battle', authMiddleware, async (req, res) => {
    ...
    try {
      const result = await services.heady.battle(messages, {
        candidates,
        ...options
      });
      res.json(result);
  ...
  server.listen(PORT, () => {
    log.info({
      port: PORT,
      providers: Object.keys(services.heady.providers).length
    }, `HeadyAI-IDE Server online at :${PORT}`);
  });
