...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: services/search-service/src/index.ts                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
...
class SearchService {
  ...
  constructor() {
    ...
    this.hybridEngine = new HybridSearchEngine(this.vectorEngine, this.fullTextEngine, PSI, PSI_SQUARED, this.logger);
  ...
  private async handleHybridSearch(req: Request<any, any, SearchRequest>, res: Response): Promise<void> {
    ...
    try {
      ...
      const results = await this.hybridEngine.search(query, embedding, validLimit, validOffset, minConfidence);
  ...
  private async handleVectorSearch(req: Request<any, any, VectorSearchRequest>, res: Response): Promise<void> {
    const requestId = Math.random().toString(36).substring(7);
    const startTime = Date.now();
    try {
      const {
      ...
      const embedding = await this.embeddingsService.generate(query);
      const results = await this.vectorEngine.search(embedding, validLimit, validOffset, validThreshold);
      const duration = Date.now() - startTime;
      this.logger.info({
        component: 'search',
        searchType: 'vector',
  ...
  private async handleFullTextSearch(req: Request<any, any, SearchRequest>, res: Response): Promise<void> {
    const requestId = Math.random().toString(36).substring(7);
    const startTime = Date.now();
    try {
      const {
  ...
  private async handleAutocomplete(req: Request<any, any, any, {
    q: string;
    limit: string;
