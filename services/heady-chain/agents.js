...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: services/heady-chain/agents.js                                                    ║
// ║  LAYER: root                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
'use strict';
...
async function callLLM(messages, opts = {}) {
  ...
  try {
    const result = await httpPost(`${config.HEADY_INFER_URL}/infer`, payload, config.HEADY_INFER_TIMEOUT_MS);
    ...
    return {
      type: 'text',
      content: body.response || body.text || JSON.stringify(body)
    };
  } catch (err) {
...
class ReActAgent {
  constructor(opts = {}) {
    ...
    this.memory = opts.memory || new BufferMemory({
      maxSize: 20
    });
...
Final Answer: [your complete answer]`;
  ...
  async run(input, context = {}) {
    ...
    messages.push({
      role: 'system',
      content: interpolate(this.systemPrompt, context)
    });
    ...
    while (iterations < this.maxIterations) {
      ...
      if (parsed.type === 'action') {
        ...
        messages.push({
          role: 'user',
          content: 'Please continue with Thought/Action or Final Answer format.'
        });
      }
    }
...
class PlanAndExecuteAgent {
  ...
  async plan(objective, context = '') {
    const messages = this.plannerPrompt.format({
      objective,
      context
    });
    ...
    while (rounds < this.maxRounds) {
      ...
      if (response.type === 'tool_calls') {
        ...
        for (const toolCall of response.tool_calls) {
          ...
          let fnArgs = {};
          try {
            fnArgs = typeof toolCall.function?.arguments === 'string' ? JSON.parse(toolCall.function.arguments) : toolCall.function?.arguments || toolCall.input || {};
          } catch {
            fnArgs = {};
          }
          let toolResult;
...
Or: {"final_answer": "answer"}`;
  ...
  _buildAgentList() {
    return [...this.agents.entries()].map(([name, {
      description,
      capabilities
    }]) => `- ${name}: ${description} (capabilities: ${capabilities.join(', ')})`).join('\n');
  }
  ...
  async run(task, context = {}) {
    ...
    const messages = [{
      role: 'system',
      content: interpolate(this.systemPrompt, {
        agentList
      })
    }, {
      role: 'user',
    ...
    while (rounds < this.maxRounds) {
      ...
      if (directive.delegate && Array.isArray(directive.delegate)) {
        ...
        const results = await Promise.all(delegations.map(async ({
          agent: agentName,
          task: subTask
        }) => {
          ...
          try {
            ...
            if (typeof agent.run === 'function') {
              ...
              result = {
                error: 'Agent has no run/chat method'
              };
            }
            return {
              agent: agentName,
...
Return JSON:
{
  ...
  async critique(content, task = '', context = {}) {
    ...
    result.passed = result.passed !== undefined ? result.passed : (result.score || 0) >= this.passingScore;
    return result;
  }
