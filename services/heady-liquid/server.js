...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: services/heady-liquid/server.js                                                    ║
// ║  LAYER: root                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
...
if (process.env.SENTRY_DSN) {
  Sentry.init({
    ...
    tracesSampleRate: PSI,
    // ~61.8% sampling — φ-scaled
    profilesSampleRate: PSI * PSI // ~38.2%
  });
  logger.info('  ✅ Sentry wired — DSN:', process.env.SENTRY_DSN.split('@')[1]?.split('/')[0] || 'configured');
}
...
if (process.env.GOOGLE_APPLICATION_CREDENTIALS || process.env.FIREBASE_PROJECT_ID) {
  try {
    ...
    logger.info('  ✅ Firebase Admin wired — project:', firebaseApp.options.projectId);
  } catch (e) {
    logger.info('  ⚠️ Firebase Admin:', e.message);
  }
}
...
if (process.env.DATABASE_URL) {
  ...
  logger.info('  ✅ Neon Postgres wired — project: green-water-91851995');
}
...
if (process.env.UPSTASH_REDIS_ENDPOINT && process.env.UPSTASH_REDIS_PASSWORD) {
  ...
  logger.info('  ✅ Upstash Redis wired — endpoint:', process.env.UPSTASH_REDIS_ENDPOINT);
}
...
if (process.env.PINECONE_API_KEY) {
  ...
  logger.info('  ✅ Pinecone wired — vector DB initialized');
}
...
app.get('/api/data/query', async (req, res) => {
  if (!sql) return res.status(503).json({
    error: 'Neon Postgres not configured — set DATABASE_URL'
  });
  try {
...
app.get('/api/data/tables', async (req, res) => {
  if (!sql) return res.status(503).json({
    error: 'Neon Postgres not configured'
  });
  try {
...
app.get('/api/cache/ping', async (req, res) => {
  if (!redis) return res.status(503).json({
...
app.get('/api/cache/:key', async (req, res) => {
  ...
  if (val === null) return res.status(404).json({
    key: req.params.key,
    found: false
  });
  res.json({
...
app.get('/api/vectors/indexes', async (req, res) => {
  if (!pinecone) return res.status(503).json({
...
app.post('/api/vectors/upsert', async (req, res) => {
  ...
  if (!indexName || !vectors) return res.status(400).json({
    error: 'index and vectors required'
  });
  try {
...
app.post('/api/vectors/query', async (req, res) => {
  ...
  if (!indexName || !vector) return res.status(400).json({
    error: 'index and vector required'
  });
  try {
    const index = pinecone.index(indexName);
    const ns = namespace ? index.namespace(namespace) : index;
...
app.post('/api/auth/verify', async (req, res) => {
  if (!firebaseApp) return res.status(503).json({
...
app.get('/api/monitoring/sentry/test', (req, res) => {
  if (!process.env.SENTRY_DSN) return res.status(503).json({
  ...
  res.json({
    ...
    dsn: process.env.SENTRY_DSN.split('@')[1]?.split('/')[0] || 'configured'
  });
});
...
app.get('/api/liquid-nodes/status', (req, res) => {
  const nodes = [{
    ...
    name: 'Neon Postgres',
    type: 'database',
    status: sql ? 'wired' : 'disconnected'
  }, {
    id: 'upstash',
    name: 'Upstash Redis',
    type: 'cache',
    status: redis ? 'wired' : 'disconnected'
  }, {
    id: 'pinecone',
    name: 'Pinecone',
    type: 'vector-db',
    status: pinecone ? 'wired' : 'disconnected'
  }, {
    id: 'firebase',
    name: 'Firebase Admin',
    type: 'auth',
    status: firebaseApp ? 'wired' : 'disconnected'
  }, {
    id: 'sentry',
    name: 'Sentry',
    type: 'monitoring',
    status: process.env.SENTRY_DSN ? 'wired' : 'disconnected'
  }];
...
app.listen(PORT, () => {
  ...
  logger.info(`  🌀 φ = ${PHI} | ψ = ${PSI}`);
  logger.info(`  🚀 Listening on port ${PORT}`);
  logger.info(`  📍 Endpoints: /_heady/health, /api/data/*, /api/cache/*, /api/vectors/*, /api/auth/*, /api/monitoring/*\n`);
});