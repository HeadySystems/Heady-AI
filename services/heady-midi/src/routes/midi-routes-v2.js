...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: services/heady-midi/src/routes/midi-routes-v2.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
import { Router } from 'express';
import { WebSocketServer } from 'ws';
import { FIB, CHANNEL, CHANNEL_LABELS, CHANNEL_COLORS, EVENT_BUFFER_SIZE, MAX_WS_CLIENTS } from '../shared/midi-constants.js';
import { encodeSetTempo, encodeTransport, encodeSetTrackVolume, encodeTriggerClip, encodeSetDeviceParam, encodeCreateMidiTrack, encodeCreateAudioTrack, encodeSetTrackSend, encodeSetTrackEQ, encodeArmTrack, encodeSetClipColor, encodeSetClipName, encodeQuantizeClip, encodeDuplicateClip, encodeDeleteClip, encodeStatusRequest, encodeGetTrackNames, encodeGetDeviceChain, encodeSetMacro, encodeLoadPreset, encodeCCRecordEnable, encodeSetLoopRegion, encodeSetTimeSignature, encodeSetTrackRouting, encodeSoloTrack, encodeMuteTrack, encodeSetSceneName, encodeFireScene, encodeCaptureMidi, encodeConsolidateClip, encodeUndo, encodeAIArrangement, encodeAIGeneratePattern, encodeVersionNegotiate } from '../shared/sysex-codec.js';
...
function rateLimiter(req, res, next) {
  ...
  if (b.count > RATE_MAX) return res.status(429).json({
...
setInterval(() => {
  const n = Date.now();
  for (const [k, v] of rateBuckets) if (n >= v.resetAt) rateBuckets.delete(k);
}, RATE_WINDOW);

// ─── Ring Buffer (capacity = 987, Fibonacci) ──────────────────────
class RingBuffer {
  constructor(cap = EVENT_BUFFER_SIZE) {
    this._b = new Array(cap).fill(null);
    this._h = 0;
    this._n = 0;
    this.cap = cap;
  }
  push(e) {
    this._b[this._h] = e;
    this._h = (this._h + 1) % this.cap;
    if (this._n < this.cap) this._n++;
  }
  query(since = 0, limit = 100) {
    const out = [],
      s = this._n < this.cap ? 0 : this._h;
    for (let i = 0; i < this._n && out.length < limit; i++) {
      const e = this._b[(s + i) % this.cap];
      if (e && e.timestamp >= since) out.push(e);
    }
    return out;
  }
  get size() {
...
const fail = (res, msg) => res.status(400).json({
...
const SYSEX_ENC = new Map([['VERSION_NEGOTIATE', p => encodeVersionNegotiate(p.version)], ['SET_TEMPO', p => encodeSetTempo(p.bpm)], ['SET_TRACK_VOLUME', p => encodeSetTrackVolume(p.track, p.volume)], ['TRIGGER_CLIP', p => encodeTriggerClip(p.track, p.scene)], ['SET_DEVICE_PARAM', p => encodeSetDeviceParam(p.track, p.device, p.param, p.value)], ['TRANSPORT', p => encodeTransport(p.action)], ['CREATE_MIDI_TRACK', p => encodeCreateMidiTrack(p.name)], ['CREATE_AUDIO_TRACK', p => encodeCreateAudioTrack(p.name)], ['SET_TRACK_SEND', p => encodeSetTrackSend(p.track, p.send, p.value)], ['SET_TRACK_EQ', p => encodeSetTrackEQ(p.track, p.band, p.freqHi, p.freqLo, p.gain, p.q)], ['ARM_TRACK', p => encodeArmTrack(p.track, p.armState)], ['SET_CLIP_COLOR', p => encodeSetClipColor(p.track, p.scene, p.r, p.g, p.b)], ['SET_CLIP_NAME', p => encodeSetClipName(p.track, p.scene, p.name)], ['QUANTIZE_CLIP', p => encodeQuantizeClip(p.track, p.scene, p.quantize)], ['DUPLICATE_CLIP', p => encodeDuplicateClip(p.srcTrack, p.srcScene, p.dstTrack, p.dstScene)], ['DELETE_CLIP', p => encodeDeleteClip(p.track, p.scene)], ['STATUS_REQUEST', () => encodeStatusRequest()], ['GET_TRACK_NAMES', () => encodeGetTrackNames()], ['GET_DEVICE_CHAIN', p => encodeGetDeviceChain(p.track)], ['SET_MACRO', p => encodeSetMacro(p.track, p.device, p.macroIndex, p.value)], ['LOAD_PRESET', p => encodeLoadPreset(p.track, p.device, p.presetIndex)], ['CC_RECORD_ENABLE', p => encodeCCRecordEnable(p.track, p.ccNumber, p.enable)], ['SET_LOOP_REGION', p => encodeSetLoopRegion(p.startBar, p.startBeat, p.endBar, p.endBeat)], ['SET_TIME_SIGNATURE', p => encodeSetTimeSignature(p.numerator, p.denominator)], ['SET_TRACK_ROUTING', p => encodeSetTrackRouting(p.track, p.inputType, p.inputChannel)], ['SOLO_TRACK', p => encodeSoloTrack(p.track, p.soloState)], ['MUTE_TRACK', p => encodeMuteTrack(p.track, p.muteState)], ['SET_SCENE_NAME', p => encodeSetSceneName(p.scene, p.name)], ['FIRE_SCENE', p => encodeFireScene(p.sceneIndex)], ['CAPTURE_MIDI', () => encodeCaptureMidi()], ['CONSOLIDATE_CLIP', p => encodeConsolidateClip(p.track, p.scene)], ['UNDO', () => encodeUndo()], ['AI_ARRANGEMENT', p => encodeAIArrangement(p.data)], ['AI_GENERATE_PATTERN', p => encodeAIGeneratePattern(p.data)]]);
...
export function createMidiRouter(deps) {
  ...
  router.get('/status', (_req, res) => {
    try {
      ...
      res.json({
        channels,
        uptime: Date.now() - (eventBus?.getUptime?.() || t0),
        totalEvents: eventBus?.getTotalEvents?.() ?? buf.size
      });
    } catch (e) {
      res.status(500).json({
  ...
  router.post('/sequencer/play', (_req, res) => {
    try {
      if (typeof sequencer?.play !== 'function') return res.status(500).json({
  ...
  router.post('/sequencer/stop', (_req, res) => {
    try {
      if (typeof sequencer?.stop !== 'function') return res.status(500).json({
        error: 'Sequencer unavailable'
      });
      sequencer.stop();
      res.json({
  ...
  router.post('/sequencer/tempo', (req, res) => {
    try {
      ...
      if (typeof sequencer?.setTempo !== 'function') return res.status(500).json({
        error: 'Sequencer unavailable'
      });
  ...
  router.post('/sequencer/pattern', (req, res) => {
    try {
      ...
      if (typeof sequencer?.addPattern !== 'function') return res.status(500).json({
        error: 'Sequencer unavailable'
      });
  ...
  router.post('/mappings', (req, res) => {
    try {
      ...
      if (typeof mappingService?.create !== 'function') return res.status(500).json({
        error: 'Mapping service unavailable'
      });
      const mapping = mappingService.create({
        deviceId,
        ccNumber,
        channel,
        targetParam,
  ...
  router.delete('/mappings/:id', (req, res) => {
    try {
      if (typeof mappingService?.remove !== 'function') return res.status(500).json({
        error: 'Mapping service unavailable'
      });
  ...
  router.post('/sysex/send', (req, res) => {
    try {
      ...
      if (typeof wsProxy?.send !== 'function') return res.status(500).json({
        error: 'SysEx proxy unavailable'
      });
      wsProxy.send(frame);
      res.json({
        ok: true,
        ...
        hex: Array.from(frame).map(b => b.toString(16).padStart(2, '0')).join(' ')
      });
    } catch (e) {
      res.status(500).json({
  ...
  router.get('/network/peers', (_req, res) => {
    try {
      const peers = [...(wsProxy?.getUdpPeers?.() || []).map(p => ({
        ...p,
        transport: 'udp'
      })), ...(wsProxy?.getWsPeers?.() || []).map(p => ({
        ...p,
        transport: 'ws'
      }))];
      res.json({
  ...
  router.post('/arrangement', (req, res) => {
    try {
      ...
      if (typeof arrangementPipeline?.run !== 'function') return res.status(500).json({
      ...
      arrangementPipeline.run({
        ...
        jobId
      }).catch(err => eventBus?.emit?.('arrangement:error', {
        jobId,
        error: err.message
      }));
      res.status(202).json({
...
export function setupMidiWebSocket(server, deps) {
  ...
  /** @type {Set<import('ws').WebSocket>} */
  const umpClients = new Set();
  const wssJson = new WebSocketServer({
    noServer: true
  });
  const wssUmp = new WebSocketServer({
    noServer: true
  });

  // ── JSON stream (/ws/midi) ────────────────────────────────────
  wssJson.on('connection', ws => {
    if (jsonClients.size >= MAX_WS_CLIENTS) {
    ...
    ws.send(JSON.stringify({
      type: 'connected',
      message: 'HeadyMIDI real-time stream',
    ...
    ws.on('message', raw => {
      try {
        ...
        if (msg.ping) ws.send(JSON.stringify({
          type: 'pong',
          timestamp: Date.now()
        }));
  ...
  wssUmp.on('connection', ws => {
    if (umpClients.size >= MAX_WS_CLIENTS) {
      ws.close(1013, 'Max clients reached');
      return;
    }
    umpClients.add(ws);
  ...
  wsProxy?.on?.('ump', frame => {
    for (const ws of umpClients) if (ws.readyState === 1) ws.send(frame);
  });
