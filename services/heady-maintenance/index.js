...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: services/heady-maintenance/index.js                                                    ║
// ║  LAYER: root                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
...
async function stageBrand() {
    ...
    async function check(dir, depth = 0) {
        if (depth > 5)
            return;
        const basename = dir.split('/').pop() || '';
        if (PROTECTED_DIRS.has(basename) || basename.startsWith('.'))
            return;
        const entries = await readdir(dir, { withFileTypes: true }).catch(() => []);
        for (const entry of entries) {
            const full = join(dir, entry.name);
...
// ────────────────────────────────────────────────────────────
async function stageTrim(dryRun = true) {
    const start = Date.now();
    const staleFound = [];
    const now = Date.now();
...
// ────────────────────────────────────────────────────────────
function stageCommit(dryRun = true) {
    const start = Date.now();
    try {
...
// ────────────────────────────────────────────────────────────
function stageDeploy(dryRun = true) {
    const start = Date.now();
    try {
        if (!dryRun) {
...
// ────────────────────────────────────────────────────────────
function stagePush(dryRun = true) {
    const start = Date.now();
    try {
        if (!dryRun) {
...
// ────────────────────────────────────────────────────────────
async function runMaintenanceCycle(dryRun = true) {
...
app.get('/maintenance/status', async () => {
    if (!lastResult)
        return { state: 'idle', lastRun: null };
