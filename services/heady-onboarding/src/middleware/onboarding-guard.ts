...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: services/heady-onboarding/src/middleware/onboarding-guard.ts                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
...
export enum OnboardingStage {
  ...
  // Stage 1: username → @headyme.com
  EMAIL_CONFIG = 'email-config',
  // Stage 2: email forwarding or secure client
  PERMISSIONS = 'permissions',
  // Stage 3: cloud-only vs hybrid
  BUDDY_SETUP = 'buddy-setup',
  // Stage 4: custom UI + context switcher
  COMPLETE = 'complete' // Stage 5: welcome → dashboard (API key in settings)
  ,
}
const STAGE_ORDER: OnboardingStage[] = [OnboardingStage.CREATE_ACCOUNT, OnboardingStage.EMAIL_CONFIG, OnboardingStage.PERMISSIONS, OnboardingStage.BUDDY_SETUP, OnboardingStage.COMPLETE];
...
const BYPASS_PATHS = ['/api/', '/onboarding/', '/_next/', '/favicon', '/css/', '/js/', '/images/', '/health'];
 ...
 */
function getOnboardingRedirect(request: NextRequest, state: OnboardingState | null): {
  ...
  const requestedStage = STAGE_ORDER.find(s => path === STAGE_PATHS[s] || path.startsWith(STAGE_PATHS[s]));
  if (requestedStage) {
...
export function onboardingGuard(request: NextRequest): NextResponse | null {
  ...
  const {
    redirect,
    reason
  } = getOnboardingRedirect(request, state);
 ...
 */
export function advanceOnboardingStage(currentStage: OnboardingStage, userId: string, provider: string): OnboardingState {
