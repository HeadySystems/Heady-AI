...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: services/heady-onboarding/src/app/api/onboarding/route.ts                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const currentState = onboardingCookie ? JSON.parse(decodeURIComponent(onboardingCookie.value)) : null;

    // Advance to next stage
    const nextStage = getNextStage(stage);
    const newState = advanceOnboardingStage(stage as OnboardingStage, currentState?.userId ?? 'unknown', currentState?.provider ?? 'unknown');
...
async function processStage(stage: string, data: Record<string, unknown>): Promise<StageResult> {
  switch (stage) {
    case 'create-account':
...
async function processCreateAccount(data: Record<string, unknown>): Promise<StageResult> {
  ...
  try {
    const checkResponse = await fetch(`${process.env.HEADY_API_URL ?? 'https://api.headyapi.com'}/internal/check-username`, {
      method: 'POST',
      headers: {
...
async function processEmailConfig(data: Record<string, unknown>): Promise<StageResult> {
  ...
  if (emailMode === EmailMode.FORWARD_PROVIDER || emailMode === EmailMode.FORWARD_CUSTOM) {
    if (!forwardTo || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(forwardTo)) {
      return {
        success: false,
        error: 'Valid forwarding email required',
        statusCode: 422
      };
    }
  }
  ...
  try {
    await fetch(`${process.env.HEADY_API_URL ?? 'https://api.headyapi.com'}/internal/email/configure`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${process.env.HEADY_INTERNAL_KEY}`
...
async function processPermissions(data: Record<string, unknown>): Promise<StageResult> {
...
async function processBuddySetup(data: Record<string, unknown>): Promise<StageResult> {
  ...
  if (buddyName.length < 1 || buddyName.length > 30) {
    return {
      success: false,
      error: 'Buddy name must be 1-30 characters',
      statusCode: 422
    };
  }
  ...
  try {
    await fetch(`${process.env.HEADY_BUDDY_URL ?? 'https://buddy.headybuddy.org'}/api/configure`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${process.env.HEADY_INTERNAL_KEY}`
...
async function processComplete(data: Record<string, unknown>): Promise<StageResult> {
  // Generate API key — but DON'T display it here.
  // It goes to Settings > API page.
  try {
    const response = await fetch(`${process.env.HEADY_API_URL ?? 'https://api.headyapi.com'}/internal/api-keys/generate`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${process.env.HEADY_INTERNAL_KEY}`
