...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: services/heady-sync/server.js                                                    ║
// ║  LAYER: root                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
...
function sync(deviceId, state) {
  ...
  return {
    synced: device,
    connectedDevices: store.devices.length
  };
}
const server = http.createServer((req, res) => {
  ...
  if (parsed.pathname === '/sync' && req.method === 'POST') {
    let body = '';
    ...
    req.on('end', () => {
      ...
      res.end(JSON.stringify(sync(deviceId, state)));
    });
    return;
  }
  ...
  res.end(JSON.stringify({
    ...
    version: '1.0.0',
    endpoints: {
      '/sync': 'POST {deviceId, state}',
      '/devices': 'GET'
    }
  }));
});
const PORT = process.env.PORT || 8129;
server.listen(PORT, () => logger.info(`🔄 HeadySync on :${PORT}`));
module.exports = {
  sync
};