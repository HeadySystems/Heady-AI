...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: services/admin-ui/server.js                                                    ║
// ║  LAYER: root                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
...
app.post('/api/chat', async (req, res) => {
  ...
  for (const p of providers) {
    ...
    try {
      ...
      logger.error(`[${p.name}] failed:`, err.message);
    }
  }
  return res.json({
...
app.get('/api/status', (_req, res) => {
  ...
  if (OPENAI_KEY) providers.push({
    name: 'OpenAI',
    model: 'gpt-4o-mini',
    status: 'ready'
  });
  if (ANTHROPIC_KEY) providers.push({
    name: 'Anthropic',
    model: 'claude-sonnet',
    status: 'ready'
  });
  if (GEMINI_KEY) providers.push({
    name: 'Gemini',
    model: 'gemini-2.0-flash',
    status: 'ready'
  });
...
app.listen(PORT, () => logger.info(`⚡ Heady Admin UI running on port ${PORT}`));