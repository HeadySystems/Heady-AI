...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: services/vector-memory-service/service.js                                                    ║
// ║  LAYER: root                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/**
 ...
 */
import { Pool } from 'pg';
import { PHI, FIB, CSL_THRESHOLDS, POOL_SIZES, DEDUP_THRESHOLD, EMBEDDING } from '../../shared/phi-math.js';
...
export async function storeVector(req) {
    ...
    const dupCheck = await pool.query(`SELECT id, 1 - (embedding <=> $1::vector) as similarity
     FROM vector_memory
     WHERE namespace = $2
     ORDER BY embedding <=> $1::vector
     LIMIT 1`, [`[${embedding.join(',')}]`, namespace || 'default']);
    if (dupCheck.rows.length > 0 && dupCheck.rows[0].similarity > DEDUP_THRESHOLD) {
        ...
        await pool.query(`UPDATE vector_memory SET content = $1, embedding = $2::vector, projection_3d = $3,
       metadata = $4, updated_at = $5, coherence_score = $6 WHERE id = $7`, [content, `[${embedding.join(',')}]`, projection3D, JSON.stringify(metadata || {}),
    ...
    // Insert new vector
    await pool.query(`INSERT INTO vector_memory (id, content, embedding, projection_3d, metadata, namespace, created_at, updated_at, coherence_score)
     VALUES ($1, $2, $3::vector, $4, $5, $6, $7, $8, $9)`, [id, content, `[${embedding.join(',')}]`, projection3D, JSON.stringify(metadata || {}),
...
export async function searchVectors(query) {
  ...
  `;
    const params = [`[${vector.join(',')}]`, minScore];
    if (filter) {
        let paramIdx = 3;
    ...
    return result.rows.map((row) => ({
        ...
        content: row.content,
        projection3D: row.projection_3d,
        ...(includeMetadata ? { metadata: JSON.parse(row.metadata) } : {}),
    }));
}
// ═══ Memory Stats ═══
...
// ═══ Drift Detection ═══
export async function detectDrift(namespace = 'default') {
    const result = await pool.query(`SELECT AVG(coherence_score) as avg_coherence,
