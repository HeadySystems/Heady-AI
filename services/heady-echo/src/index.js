...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: services/heady-echo/src/index.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
...
const voiceProfiles = {
  ...
  professional: {
    ...
    speed: 1.0,
    warmth: 0.618,
    personality: 'clear, authoritative'
  },
  warm: {
    pitch: 1.02,
    speed: 0.95,
    warmth: 0.927,
    personality: 'gentle, empathetic'
  },
  energetic: {
    pitch: 1.08,
    speed: 1.15,
    warmth: 0.691,
    personality: 'upbeat, motivating'
  }
};

// ── Tools ───────────────────────────────────────────────────────

mcp.tool('health_check', 'Check HeadyEcho health and audio subsystem status', {}, async () => ({
  content: [{
    type: 'text',
    text: JSON.stringify({
      status: 'operational',
...
mcp.tool('heady_echo_transcribe', 'Real-time voice-to-text with speaker diarization and intent extraction', {
  audio_url: {
    type: 'string',
    description: 'URL to audio file or stream'
  },
  language: {
    type: 'string',
  ...
  extract_intent: {
    type: 'boolean',
    description: 'Extract structured intent from speech'
  },
  diarize: {
    type: 'boolean',
...
mcp.tool('heady_echo_synthesize', 'Generate natural speech from text with persona-aligned voice profiles', {
  text: {
    type: 'string',
  ...
  voice_profile: {
    type: 'string',
    description: 'buddy | professional | warm | energetic'
  },
  format: {
    type: 'string',
    description: 'mp3 | wav | opus'
  }
}, async ({
  text,
  voice_profile = 'buddy',
...
mcp.tool('heady_echo_fingerprint', 'Audio fingerprinting — identify, classify, and tag audio content', {
  audio_url: {
    type: 'string',
    description: 'URL to audio to fingerprint'
  },
  match_against: {
    type: 'string',
    description: 'library | web | custom'
  }
}, async ({
  audio_url,
  match_against = 'library'
}) => ({
  content: [{
    type: 'text',
    text: JSON.stringify({
...
mcp.tool('heady_echo_soundscape', 'Generate ambient soundscapes aligned to Sacred Geometry phi-harmonics', {
  mood: {
    type: 'string',
    description: 'focus | creative | calm | energetic | sacred'
  },
  duration_seconds: {
    type: 'number',
  ...
  phi_harmonics: {
    type: 'boolean',
    description: 'Use phi-ratio frequency relationships'
  }
}, async ({
  mood = 'focus',
    ...
    content: [{
      ...
      text: JSON.stringify({
        soundscape: {
          ...
          layers: [{
            type: 'base_tone',
            frequency: PHI_HARMONICS.root,
            volume: PSI
          }, {
            type: 'harmonic',
            frequency: PHI_HARMONICS.phi_1,
            volume: PSI * PSI
          }, {
            type: 'binaural_beat',
            frequency: 10 * PSI,
...
mcp.resource('heady://echo/voices', 'heady://echo/voices', 'Available voice profiles and their characteristics', 'application/json', async () => ({
  contents: [{
...
mcp.resource('heady://echo/library', 'heady://echo/library', 'Audio fingerprint library', 'application/json', async () => ({
  contents: [{
...
serve({
  ...
  logger.info(`[HeadyEcho] Voice & Audio Intelligence projected on :${PORT}`);
  logger.info(`[HeadyEcho] SSE: http://localhost:${PORT}/sse`);
  logger.info(`[HeadyEcho] Health: http://localhost:${PORT}/health`);
});