...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: services/asset-pipeline/index.js                                                    ║
// ║  LAYER: root                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
'use strict';
...
const CSL_THRESHOLDS = {
  MINIMUM: 1 - PSI2 * 1,
  // 0.500 (noise floor, using 1 - psi^0 * 0.5 identity)
...
const SERVICE_PORT = parseInt(process.env.ASSET_PIPELINE_PORT || '3368', 10);
...
class StructuredLogger {
  ...
  info(msg, meta) {
    this._emit('INFO', msg, meta);
  }
  warn(msg, meta) {
    this._emit('WARN', msg, meta);
  }
  error(msg, meta) {
    this._emit('ERROR', msg, meta);
  }
  debug(msg, meta) {
    this._emit('DEBUG', msg, meta);
  }
}
...
class Bulkhead {
  ...
  stats() {
    return {
      active: this.active,
      queued: this.queued,
      maxConcurrent: BULKHEAD_CONCURRENT,
      maxQueued: BULKHEAD_QUEUE
    };
  }
}
...
class TelemetryEmitter extends EventEmitter {
  ...
  getMetrics() {
    const avg = this.metrics.requests_total > 0 ? Math.round(this.metrics.latency_sum_ms / this.metrics.requests_total) : 0;
    return {
      ...this.metrics,
      avg_latency_ms: avg,
      service: SERVICE_NAME
...
function setSecurityHeaders(res) {
  ...
  res.setHeader('Content-Security-Policy', "default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; " + "img-src 'self' data: https:; font-src 'self'; connect-src 'self' https://*.headysystems.com; " + "frame-ancestors 'self' https://*.headysystems.com");
}
...
class PhiRateLimiter {
  ...
  cleanup() {
    ...
    for (const [key, window] of this.windows) {
      if (now - window.start > this.windowMs * FIB[3]) {
        // 3x window for cleanup
...
const server = http.createServer(async (req, res) => {
  ...
  if (!rateCheck.allowed) {
    res.writeHead(429, {
      ...
      'Retry-After': Math.ceil(rateLimiter.windowMs / 1000).toString()
    });
    res.end(JSON.stringify({
      error: 'Rate limit exceeded',
      retryAfterMs: rateLimiter.windowMs
    }));
    return;
  }
  try {
    await bulkhead.execute(async () => {
      ...
      if (url === '/metrics') {
        ...
        const lines = [`# HELP heady_requests_total Total requests`, `# TYPE heady_requests_total counter`, `heady_requests_total{service="${SERVICE_NAME}",domain="${SERVICE_DOMAIN}"} ${m.requests_total}`, `# HELP heady_errors_total Total errors`, `# TYPE heady_errors_total counter`, `heady_errors_total{service="${SERVICE_NAME}",domain="${SERVICE_DOMAIN}"} ${m.errors_total}`, `# HELP heady_avg_latency_ms Average latency`, `# TYPE heady_avg_latency_ms gauge`, `heady_avg_latency_ms{service="${SERVICE_NAME}",domain="${SERVICE_DOMAIN}"} ${m.avg_latency_ms}`, `# HELP heady_bulkhead_active Active requests in bulkhead`, `# TYPE heady_bulkhead_active gauge`, `heady_bulkhead_active{service="${SERVICE_NAME}"} ${bulkhead.stats().active}`];
        res.writeHead(200, {
      ...
      if (url === '/info') {
        ...
        res.end(JSON.stringify({
          ...
          port: SERVICE_PORT,
          domain: SERVICE_DOMAIN,
          description: 'Image optimization and CDN upload',
          version: '4.0.0',
          phi: PHI,
      ...
      if (url === '/' && method === 'GET') {
        ...
        res.end(JSON.stringify({
          ...
          status: 'operational',
          domain: SERVICE_DOMAIN,
          message: 'Image optimization and CDN upload',
    ...
    if (err.code === 'BULKHEAD_FULL') {
      ...
      res.end(JSON.stringify({
        ...
        code: 'BULKHEAD_FULL',
        retryAfterMs: Math.round(PHI * 1000)
      }));
    } else {
  ...
  if (latency > REQUEST_TIMEOUT_MS * PSI) {
    logger.warn('Slow request', {
      latency_ms: latency,
      threshold_ms: Math.round(REQUEST_TIMEOUT_MS * PSI)
    });
  }
});
...
async function gracefulShutdown(signal) {
  ...
  for (let i = shutdownHandlers.length - 1; i >= 0; i--) {
    ...
    try {
      ...
      logger.error(`Cleanup error in ${handler.name}`, {
        error: err.message
      });
    }
  }
