...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: heady-monorepo/src/providers/provider-connector.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/**
...
class BaseAdapter {
  ...
  // Subclasses override these with real implementations:
  async generate(prompt, opts = {}) {
    throw new Error(`${this.name}.generate not implemented — override in subclass`);
  }

  async embed(text, opts = {}) {
    // Default: use OpenAI-compatible embedding if API key is set
    const embeddingUrl = `${this.baseUrl}/embeddings`;
    const model = opts.model || 'text-embedding-3-small';
    const input = Array.isArray(text) ? text : [text];
    const res = await this._fetch(embeddingUrl, {
      body: { model, input },
      headers: this._authHeaders(),
    }).catch(() => null);
    if (res) {
      const data = await res.json().catch(() => ({}));
      return { embeddings: data.data?.map(d => d.embedding) || [], model, raw: data };
    }
    throw new Error(`${this.name}.embed not implemented — override in subclass`);
  }

  async streamGenerate(prompt, opts = {}) {
    // Default: fall back to non-streaming generate
