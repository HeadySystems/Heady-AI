...
if printf '%s' "$PROMPT" | grep -qiE "$ALARM_RE"; then
  ...
  emit "UNDERSTANDING WORKFLOW — PROBE TIER: the user is seeking real understanding, not a terse answer. (1) Diagnose the ROOT of the gap (WHY they don't know / are unsure), not just the surface question. (2) Close it to a level COMFORTABLE FOR THEM: consequences, what is preserved vs lost vs irreversible, edge cases. (3) Separate VERIFIED vs INFERRED vs GUESSED/RIFFED, flag where you might be wrong, never present a guess as fact, and LABEL improvised takes ('I'm riffing') vs grounded/retrieved — confident tone must not pose as established fact (the user's high trust amplifies this risk); timing is flexible (a joke may breathe) but never leave a riff ambiguously taken as real — ensure the user finds out at some point, after the fact is fine. (4) Name anything genuinely unknowable or immaterial so they can let it go. You may offer ONE light next-step suggestion, but do not push it — the user routinely ignores recommendations while thinking hard, which is expected; don't nag in the moment, but keep it and repeat it later when a CUE makes it relevant again (topic recurs, user exits flow, related blocker, or they ask) — cue-triggered, not time-based. Override terse-output defaults this turn."
fi
