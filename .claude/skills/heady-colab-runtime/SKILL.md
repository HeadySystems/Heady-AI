...
description: "Use when wiring Google Colab as the Heady inference FALLBACK TAIL — the last hop of the chain Workers AI → Cloudflare AI Gateway → Cloud Run → Colab (ADR-0018). Covers GPU notebooks for heavy/experimental inference and embedding backfill that exceed Workers AI, reached only through the AI Gateway. Keywords: Colab, GPU, fallback tail, inference fallback, A100, notebook, AI Gateway, Workers AI, Cloud Run, batch embedding, model experiments."
---
