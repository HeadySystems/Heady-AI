...
description: "Living Dashboard for Heady — builds and deploys real-time monitoring dashboards for the Heady ecosystem using Vanilla Web Components by default, with React reserved for complex canvas components. Use for interactive visualizations of service health, coherence, topology, provider performance, token budgets, swarm activity, costs, and pipeline throughput. Keywords: dashboard, monitoring, visualization, health check, coherence, Sacred Geometry, React, Web Components, provider metrics, token budget, swarm status, pipeline monitoring, real-time, service health, topology visualization, operational dashboard."
---
