...
description: "Use when implementing persistent agent state on Cloudflare using Durable Objects, hibernatable WebSockets, SQLite storage, alarm scheduling, and agent lifecycle state machines. Covers edge-origin workload partitioning, Vectorize sync with pgvector, edge embedding caching, and smart routing between Cloudflare Workers AI and Cloud Run origin. All sizing uses Fibonacci, all timing uses phi-backoff. Keywords: Durable Objects, Cloudflare Workers, edge AI, hibernatable WebSocket, agent state, state machine, Vectorize, edge inference, workload partitioning, Cloud Run, Heady edge."
---
