...
def render_nav(site: dict) -> str:
    links_html = "".join(
        f'<li><a href="{esc(link["href"])}" class="{"cta" if link.get("cta") else ""}">{esc(link["label"])}</a></li>'
...
def render_hero(site: dict, domain: str, profile: dict) -> str:
  ...
  <div class="hero-content hero-grid{' hero-grid-auth' if aside else ''}">
    <div>
      ...
      <h1 class="hero-title fade-in fade-in-delay-1">{esc(site.get('heroTitle', site.get('tagline', '')))}</h1>
...
def render_footer(site: dict, domain: str) -> str:
    ...
    for col in site.get("footerCols", []):
        links_html = ''.join(f'<li><a href="{esc(href)}">{esc(label)}</a></li>' for label, href in col["links"])
...
<main id="main-content">
  ...
  {render_features(site)}
  {render_stats(site)}
  {render_deep_dive(site, paragraphs)}
  {render_how_it_works(steps)}
  {render_tech_stack(site, tech_stack)}
  {render_ecosystem(domain)}
  {render_use_cases(use_cases)}
  {render_faq(site, faqs)}
  {render_cta(site, domain)}
</main>
