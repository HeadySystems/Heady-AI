...
- **OpenID Connect** module for Firebase Auth SSO integration (legacy ADR-0009).
...
3. **Auth integration:** Drupal user accounts are provisioned via Firebase Auth OIDC (legacy ADR-0009). No Drupal-native password auth for editorial accounts — all login via `/user/login/openid-connect/firebase`.
...
5. **Infrastructure:** Drupal 11 deploys on Cloud Run `us-east1` (legacy ADR-0016, ADR-0036) backed by Neon Postgres (legacy ADR-0016). File assets (images, PDFs) go to Cloudflare R2 via the S3-compatible adapter.

6. **ESM boundary:** Drupal is PHP-based and runs outside the Node.js ESM boundary (legacy ADR-0011). The content-gateway Cloudflare Worker (Node.js ESM) is the adapter layer. No PHP code is permitted inside `src/` of the Heady monorepo.
...
- Drupal operates entirely outside the monorepo `src/` boundary — it is a peer service, not a module.
- The legacy ADR-0011 ESM mandate does not apply to Drupal PHP code.
...
4. Emits `content.updated` event to Upstash Redis EventSpine (legacy ADR-0013).

## Related ADRs

- Legacy ADR-0009: Firebase Auth + httpOnly cookies (OIDC integration)
- Legacy ADR-0013: Upstash Redis as EventSpine (publish webhooks)
- Legacy ADR-0016: Neon replaces Cloud SQL (Drupal DB backend)
...
- ADR-0039: Content-gateway Cloudflare Worker contract (the edge adapter this ADR presupposes)
- ADR-0019 (canonical corpus): Frontend & UI framework selection

## Reconciliation (2026-08-09 transfer)

- **No conflict with canonical ADR-0019.** Canonical ADR-0019 (frontend framework selection, Reconciliation v2) endorses vanilla + Twig + web components for app frontends. These decisions are complementary, not conflicting: this ADR scopes Drupal 11 to CONTENT — `headyconnection.org` and documentation hubs — consumed headlessly via JSON:API; app frontends remain governed by ADR-0019. Drupal renders zero end-user HTML, so it never competes with the frontend framework decision.
- **Cache TTLs remain valid.** The KV cache TTLs specified here (89s hot / 233s warm / 610s cold) are φ-scaled Fibonacci values and carry forward unchanged.

## Provenance

- **Source:** `/home/headyme/Heady-AI/docs/ADR/0020-drupal-11-headless-cms.md` (legacy docs/ADR/0020)
- **Transferred:** 2026-08-09 into the canonical corpus as ADR-0034.
