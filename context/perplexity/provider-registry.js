...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: context/perplexity/provider-registry.js                                                    ║
// ║  LAYER: root                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/*
...
module.exports = {
    ...
    twitch: {
        type: 'oauth',
        name: 'Twitch',
        icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="#fff"><path d="M11.571 4.714h1.715v5.143H11.57zm4.715 0H18v5.143h-1.714zM6 0L1.714 4.286v15.428h5.143V24l4.286-4.286h3.428L22.286 12V0zm14.571 11.143l-3.428 3.428h-3.429l-3 3v-3H6.857V1.714h13.714z"/></svg>',
        color: '#fff',
        bg: '#9146FF',
        envKey: 'HEADY_TWITCH_CLIENT_ID',
        envSecret: 'HEADY_TWITCH_CLIENT_SECRET',
        authorizeUrl: 'https://id.twitch.tv/oauth2/authorize',
        tokenUrl: 'https://id.twitch.tv/oauth2/token',
        profileUrl: 'https://api.twitch.tv/helix/users',
        scope: 'user:read:email',
        tokenPrefix: 'hdy_tt_',
        profileHeaders: (token) => ({ 'Authorization': `Bearer ${token}`, 'Client-Id': process.env.HEADY_TWITCH_CLIENT_ID }),
        extractUser: (_tokens, profile) => {
            const user = Array.isArray(profile.data) ? profile.data[0] : profile;
            return { email: user.email || `${user.login}@twitch.noreply`, name: user.display_name || user.login, photo: user.profile_image_url || null };
        },
    },
    ...
    figma: {
        type: 'oauth',
        name: 'Figma',
        icon: '<svg width="18" height="18" viewBox="0 0 24 24"><path fill="#F24E1E" d="M8 24c2.208 0 4-1.792 4-4v-4H8c-2.208 0-4 1.792-4 4s1.792 4 4 4z"/><path fill="#A259FF" d="M4 12c0-2.208 1.792-4 4-4h4v8H8c-2.208 0-4-1.792-4-4z"/><path fill="#F24E1E" d="M4 4c0-2.208 1.792-4 4-4h4v8H8C5.792 8 4 6.208 4 4z"/><path fill="#0ACF83" d="M12 0h4c2.208 0 4 1.792 4 4s-1.792 4-4 4h-4z"/><path fill="#1ABCFE" d="M20 12c0 2.208-1.792 4-4 4s-4-1.792-4-4 1.792-4 4-4 4 1.792 4 4z"/></svg>',
        color: '#fff',
        bg: '#1E1E1E',
        envKey: 'HEADY_FIGMA_CLIENT_ID',
        envSecret: 'HEADY_FIGMA_CLIENT_SECRET',
        authorizeUrl: 'https://www.figma.com/oauth',
        tokenUrl: 'https://www.figma.com/api/oauth/token',
        profileUrl: 'https://api.figma.com/v1/me',
        scope: 'file_read',
        tokenPrefix: 'hdy_fg_',
        extractUser: (_tokens, profile) => ({
            email: profile.email,
            name: profile.handle || profile.email?.split('@')[0],
            photo: profile.img_url || null,
        }),
    },
    ...
    notion: {
        type: 'oauth',
        name: 'NotebookLM',
        icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="#fff"><path d="M4.459 4.208c.746.606 1.026.56 2.428.466l13.215-.793c.28 0 .047-.28-.046-.326L18.1 2.168c-.42-.326-.98-.7-2.055-.607L3.01 2.636c-.466.047-.56.28-.374.466zm.793 3.08v13.904c0 .747.373 1.027 1.214.98l14.523-.84c.841-.046.934-.56.934-1.166V6.354c0-.606-.233-.933-.747-.886l-15.177.887c-.56.047-.747.327-.747.933zm14.337.745c.093.42 0 .84-.42.888l-.7.14v10.264c-.608.327-1.168.514-1.635.514-.747 0-.934-.234-1.495-.933l-4.577-7.186v6.952l1.448.327s0 .84-1.168.84l-3.222.187c-.093-.187 0-.653.327-.727l.84-.233V9.854L7.822 9.76c-.094-.42.14-1.026.793-1.073l3.456-.234 4.764 7.28V9.527l-1.214-.14c-.094-.514.28-.887.747-.933zM2.877 1.56l13.728-1.02c1.682-.14 2.101.093 2.8.607l3.876 2.708c.467.374.607.467.607 1.073v17.138c0 1.073-.374 1.7-1.728 1.793l-15.457.934c-1.027.047-1.54-.094-2.101-.794L1.588 19.91C.98 19.118.747 18.513.747 17.812V3.254c0-.887.373-1.634 2.13-1.694z"/></svg>',
        color: '#fff',
        bg: '#000',
        envKey: 'HEADY_NOTION_CLIENT_ID',
        envSecret: 'HEADY_NOTION_CLIENT_SECRET',
        authorizeUrl: 'https://api.notion.com/v1/oauth/authorize',
        tokenUrl: 'https://api.notion.com/v1/oauth/token',
        profileUrl: null, // extract from token response
        scope: '',
        tokenPrefix: 'hdy_nt_',
        tokenAuth: 'basic',
        extractUser: (tokens, _profile) => ({
            email: tokens.owner?.user?.person?.email || null,
            name: tokens.owner?.user?.name || 'NotebookLM User',
            photo: tokens.owner?.user?.avatar_url || null,
        }),
    },
    ...
    stripe: {
        type: 'apikey',
        name: 'Stripe',
        icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="#fff"><path d="M13.976 9.15c-2.172-.806-3.356-1.426-3.356-2.409 0-.831.683-1.305 1.901-1.305 2.227 0 4.515.858 6.09 1.631l.89-5.494C18.252.975 15.697 0 12.165 0 9.667 0 7.589.654 6.104 1.872 4.56 3.147 3.757 4.992 3.757 7.218c0 4.039 2.467 5.76 6.476 7.219 2.585.92 3.445 1.574 3.445 2.583 0 .98-.84 1.545-2.354 1.545-1.875 0-4.965-.921-6.99-2.109l-.9 5.555C5.175 22.99 8.385 24 11.714 24c2.641 0 4.843-.624 6.328-1.813 1.664-1.305 2.525-3.236 2.525-5.732 0-4.128-2.524-5.851-6.591-7.305z"/></svg>',
        color: '#fff',
        bg: '#635BFF',
        validateUrl: 'https://api.stripe.com/v1/account',
        validateMethod: 'GET',
        validateHeaders: (key) => ({ 'Authorization': `Bearer ${key}` }),
        tokenPrefix: 'hdy_st_',
        keyPlaceholder: 'sk_...',
    ...
    // ─── Additional OAuth Providers (cont.) ───────────────────────
    gitlab: {
        type: 'oauth',
        name: 'GitLab',
        icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="#E24329"><path d="M23.955 13.587l-1.342-4.135-2.664-8.189a.455.455 0 00-.867 0L16.418 9.45H7.582L4.918 1.263a.455.455 0 00-.867 0L1.387 9.452.045 13.587a.924.924 0 00.331 1.023L12 23.054l11.624-8.443a.92.92 0 00.331-1.024z"/></svg>',
        color: '#fff',
        bg: '#FC6D26',
        envKey: 'HEADY_GITLAB_CLIENT_ID',
        envSecret: 'HEADY_GITLAB_CLIENT_SECRET',
    ...
    bitbucket: {
        type: 'oauth',
        name: 'Bitbucket',
        icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="#2684FF"><path d="M.778 1.213a.768.768 0 00-.768.892l3.263 19.81c.084.5.515.868 1.022.873H19.95a.772.772 0 00.77-.646l3.27-20.03a.768.768 0 00-.768-.891zM14.52 15.53H9.522L8.17 8.466h7.561z"/></svg>',
        color: '#fff',
        bg: '#0052CC',
        envKey: 'HEADY_BITBUCKET_CLIENT_ID',
        envSecret: 'HEADY_BITBUCKET_CLIENT_SECRET',
        ...
        tokenContentType: 'application/x-www-form-urlencoded',
        extractUser: (_tokens, profile) => ({
            email: profile.email || `${profile.username}@bitbucket.noreply`,
            name: profile.display_name || profile.username,
            photo: profile.links?.avatar?.href || null,
        }),
    },
    ...
    reddit: {
        ...
        tokenContentType: 'application/x-www-form-urlencoded',
        tokenAuth: 'basic',
        profileHeaders: (token) => ({ 'Authorization': `Bearer ${token}`, 'User-Agent': 'HeadyMe/1.0' }),
        extractUser: (_tokens, profile) => ({
            email: null,
            name: profile.name || profile.subreddit?.display_name_prefixed,
            photo: profile.icon_img ? profile.icon_img.split('?')[0] : null,
        }),
    },
};
