...
<!-- ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
<!-- ║  FILE: dropzone/02-AI-Prompts/perplexity/model-racing-prompts/04-battle-arena-model-racing.md                                                    ║
<!-- ║  LAYER: root                                                  ║
<!-- ╚══════════════════════════════════════════════════════════════════╝
<!-- HEADY_BRAND:END
-->
# Heady™ Battle Arena + Model Racing + Perplexity Integration
...
## Objective
Wire the battle-arena-protocol (full tournament system) into the task execution pipeline so every significant task can be model-raced. Integrate the Perplexity Research API as a contestant in the arena. Implement deterministic comparison between arena contestants using CSL confidence-weighted scoring.

## Specific Deliverables — Build ALL Files

### 1. Battle-Sim Task Orchestrator
Build a 9-stage pipeline:
`Task → Sim → CSL Gate → Battle/MC → Bee → Swarm → Result → Drift → Audit`
- HeadySims pre-flight: predict resource needs, estimate success
- CSL gate: confidence check with phi thresholds
- HeadyBattle: run task through multiple contestants, pick winner by consensus
- HeadyMC: Monte Carlo sampling to find determinism boundary
- HeadyBees: spawn domain-specific worker
- HeadySwarms: route to optimal swarm in 17-swarm topology
- Drift check + continuous action analysis

### 2. Arena Contestant Management
- Register Perplexity Sonar as a contestant
- Register Claude, GPT-4, Gemini as contestants
- Each contestant: provider, model, API endpoint, temperature=0, seed=42
- Consensus scoring: accuracy (0.30), reasoning (0.25), creativity (0.20), conciseness (0.15), safety (0.10)

### 3. Comparison Framework
- `compareOutputs(headyResult, externalOutput)` — Jaccard + hash + length ratio + phi-weighted score
- `comparePipelineRuns(runA, runB)` — self-consistency check
- Determinism verdict: ≥ 0.618 deterministic, ≥ 0.382 marginal, < 0.382 non-deterministic

### 4. Perplexity Integration
- Wire `PerplexityResearchService` as arena contestant
- Mode selection: quick/deep/academic/news based on task domain
- Budget tracking per contest round
- Vector memory persistence of winning outputs

### 5. Task Dispatcher Enhancement
- Add `heady-battle` and `heady-sims` to SUB_AGENTS registry
- Keywords: battle, race, compare, tournament, simulate, predict, forecast

### 6. Test Suite
- Sim pre-flight scoring
- Battle racing with multiple contestants
- MC sampling determinism measurement
- Full 9-stage pipeline
- Comparison framework
- Dispatcher routing
- Perplexity contestant integration
