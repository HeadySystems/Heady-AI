...
const SENTRY_PROJECTS = [
  ...
  'heady-eval',
  'heady-edge-worker',
  'heady-colab-runtime-1',
  'heady-colab-runtime-2',
  'heady-colab-runtime-3',
  'heady-colab-runtime-4',
];
...
function initSentry(projectName, options = {}) {
  const dsn = process.env[`SENTRY_DSN_${projectName.replace(/-/g, '_').toUpperCase()}`]
    || process.env.SENTRY_DSN;

  if (!dsn) {
    console.warn(`[Sentry] No DSN for ${projectName} — using mock`);
  ...
  try {
    ...
    console.log(`[Sentry] ✓ v8 initialized for ${projectName} (OTel tracing enabled)`);
    return Sentry;
  } catch (e) {
    console.warn(`[Sentry] @sentry/node not installed — using mock (${e.message})`);
    return createMockSentry();
  }
}
...
function startHeartbeatMonitor(Sentry, monitorSlug = 'heady-heartbeat', heartbeatFn) {
  if (Sentry._isMock) {
    console.warn('[Sentry] Mock — heartbeat monitor not started');
    return null;
  }
  ...
  const run = async () => {
    const checkInId = Sentry.captureCheckIn?.({
      ...
      checkinMargin: 5,   // 5 second tolerance
      maxRuntime: 10,     // 10 second max
      failureIssueThreshold: 3, // Alert after 3 consecutive misses
      recoveryThreshold: 1,
    });
...
function trackCslDrift(sentry, currentGate, expectedGate = 0.618) {
  ...
  if (drift > ALERT_THRESHOLDS.csl_drift_max && sentry.captureMessage) {
    sentry.captureMessage(
      `CSL gate drift: ${currentGate.toFixed(4)} (expected ${expectedGate}, drift ${drift.toFixed(4)})`,
      'warning'
    );
  }

  return { currentGate, expectedGate, drift, alert: drift > ALERT_THRESHOLDS.csl_drift_max };
}
...
function createMockSentry() {
  return {
    _isMock: true,
    captureException: (err) => console.error('[Sentry:mock]', err.message || err),
    captureMessage: (msg, level) => console.log(`[Sentry:mock:${level}]`, msg),
    captureCheckIn: () => 'mock-checkin-id',
    addBreadcrumb: () => {},
    metrics: { gauge: () => {}, increment: () => {}, distribution: () => {} },
