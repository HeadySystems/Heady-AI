...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: packages/heady-llm/src/router.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
...
export function classifyComplexity(input, cslScore = 0.618) {
  ...
  let score = 0;
  if (len > 2000) score += 0.3;else if (len > 500) score += 0.15;
...
export function selectChain(tier) {
  switch (tier) {
    case 'low':
      return ['deepseek', 'geminiLite', 'workersAI'];
    case 'mid':
      return ['groq', 'geminiLite', 'azureGPT', 'deepseek'];
    case 'high':
      return ['azureGPT', 'groq', 'geminiLite', 'deepseek'];
    default:
      return ['geminiLite', 'deepseek', 'groq'];
  }
}
async function callProvider(providerKey, req) {
  ...
  const url = providerKey === 'azureGPT' ? `${p.url}/openai/deployments/${p.model}/chat/completions?api-version=2024-12-01-preview` : p.url;
  const headers = {
    'Content-Type': 'application/json'
  };
  if (providerKey === 'azureGPT') headers['api-key'] = p.key();else headers['Authorization'] = `Bearer ${p.key()}`;
