...
async function callGemini(req) {
  ...
  const res = await fetch(
    `${AI_GATEWAY.gemini}/v1beta/models/gemini-2.0-flash-lite:generateContent`,
    {
      method: 'POST',
      headers: {
...
async function callAzureOpenAI(req) {
  ...
  const res = await fetch(
    `${AI_GATEWAY.openai}/openai/deployments/gpt-4o-mini/chat/completions?api-version=${version}`,
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
...
async function callWorkersAI(req) {
  ...
  const res = await fetch(
    `${AI_GATEWAY.workers}/@cf/meta/llama-3.1-8b-instruct`,
    {
      method: 'POST',
      headers: {
...
/**
 * @param {{ messages: Array<{role:string,content:string}>, temperature?: number, maxTokens?: number, userId?: string }} req
 * @returns {Promise<{ content:string, model:string, provider:string, cached:boolean, latencyMs:number }>}
 */
