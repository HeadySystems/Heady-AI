...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/orchestration/swarm-ignition.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
const {
...
const {
  createSwarm
} = require('../bees/bee-factory');
const logger = require('../utils/logger').child('swarm-ignition');
const PHI = (1 + Math.sqrt(5)) / 2;

// Cycle intervals derived from Sacred Geometry (φ-weighted)
const CYCLES = {
  PRUNER_INTERVAL_MS: Math.round(Math.pow(PHI, 12) * 1000),
  // ~321s (~5.3 min, nightly in prod via scheduler)
  TESTER_INTERVAL_MS: Math.round(Math.pow(PHI, 10) * 1000),
  // ~122s (~2 min, hourly in prod)
...
function igniteSwarm(options = {}) {
  ...
  if (_swarmState.ignited) {
    ...
    return {
      ok: false,
      reason: 'Already ignited',
      state: getSwarmStatus()
    };
  }
