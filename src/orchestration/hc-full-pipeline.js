...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/orchestration/hc-full-pipeline.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
...
//  0. Resolve channel, sync context, route branch
"RECON",
//  1. Deep scan codebase, services, deps, env
"INTAKE",
//  2. Parse & validate incoming request
"MEMORY",
//  3. Semantic memory search + context retrieval
"TRIAGE",
//  4. Classify priority, route to node pool
"DECOMPOSE",
//  5. Break task into subtask DAG
"TRIAL_AND_ERROR",
//  6. Sandbox candidate solutions (optional)
"ORCHESTRATE",
//  7. Execute primary task with Soul
"MONTE_CARLO",
//  8. Confidence estimation (optional)
"ARENA",
//  9. Multi-node competition (optional)
"JUDGE",
// 10. Score & rank outputs
"APPROVE",
// 11. Human approval gate (HIGH/CRITICAL)
"EXECUTE",
// 12. Metacognitive execution
"VERIFY",
// 13. Post-execution verification
"SELF_AWARENESS",
// 14. Confidence calibration & bias detection
"SELF_CRITIQUE",
// 15. Review own run for gaps
"MISTAKE_ANALYSIS",
// 16. Root cause + prevention guards
"OPT_OPS",
// 17. Profile performance, detect waste
"CONTINUOUS_SEARCH",
// 18. Search for innovations (optional)
"EVOLUTION",
// 19. Controlled parameter mutations (optional)
...
class HCFullPipeline extends EventEmitter {
  ...
  async _stageOrchestrate(run) {
    const subtasks = run._subtaskDAG || [{
      id: 's1',
      desc: run.request.task || 'task'
    }];
    ...
    if (this.selfAwareness && typeof this.selfAwareness.assessSystemState === 'function') {
      try {
        ...
        if (metacognition.confidence < 0.2) {
          throw new Error(`Metacognitive halt: system confidence ${(metacognition.confidence * 100).toFixed(0)}% ` + `(error rate 1m: ${metacognition.errorRate1m}%). ` + `Recommendations: ${metacognition.recommendations.join('; ') || 'investigate'}`);
        }
  ...
  async _stageSelfAwareness(run) {
    const stageResults = run.stages.filter(s => s.status === 'completed').map(s => ({
      stage: s.name,
    ...
    if (this.selfAwareness && typeof this.selfAwareness.assessSystemState === 'function') {
      try {
        const assessment = await this.selfAwareness.assessSystemState(run.request.task || 'pipeline self-check');
  ...
  _stageSelfCritique(run) {
    ...
    if (!executeResult?.groundedInMemory) {
      gaps.push({
        type: 'grounding',
        severity: 'medium',
        desc: 'Output not grounded in vector memory'
      });
    }
    if (!verifyResult?.passed) {
      gaps.push({
        type: 'verification',
        severity: 'high',
        desc: 'Verification stage did not pass'
      });
    }
    return {
  ...
  _stageOptOps(run) {
    const stageMetrics = run.stages.filter(s => s.metrics.durationMs).map(s => ({
      stage: s.name,
    ...
    return {
      ...
      optimizationSuggestions: bottleneck && bottleneck.durationMs > 5000 ? [`Consider parallelizing ${bottleneck.stage}`] : [],
      ts: new Date().toISOString()
    };
  }
  ...
  _computeCSLScore(run) {
    const attempted = run.stages.filter(s => s.status === STATUS.COMPLETED || s.status === STATUS.FAILED);
    ...
    for (let i = attempted.length - 1; i >= 0; i--) {
      if (attempted[i].status === STATUS.FAILED) recentFailStreak++;else break;
    }
  ...
  async _rollback(run, failedIndex) {
    ...
    this.emit("rollback:started", {
      runId: run.id,
      failedStage: run.stages[failedIndex]?.name
    });
    for (let i = failedIndex - 1; i >= 0; i--) {
      const stage = run.stages[i];
      if (stage.status === STATUS.COMPLETED) {
        stage.status = STATUS.ROLLED_BACK;
        run.rollbackLog.push({
          stage: stage.name,
          rolledBackAt: new Date().toISOString()
        });
        this.emit("stage:rolledback", {
          runId: run.id,
          stage: stage.name
        });
      }
  ...
  async _selfHeal(run, stage, error, stageIndex) {
    ...
    if (this.vectorMemory) {
      try {
        ...
        const results = await this.vectorMemory.queryMemory(query, 3, {
          type: "pipeline_resolution"
        });
