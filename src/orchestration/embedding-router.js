...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/orchestration/embedding-router.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/**
...
const PROVIDER_DEFAULTS = [{
  ...
  dim: fib(9) * fib(8) * fib(4),
  // ≈ 34×21×3 = 2142 → actual 768; stored as constant
  actualDim: 768,
  maxTokens: fib(10) * fib(9),
  // 55×34 = 1870 → actual 8192; use spec dim
...
class EmbeddingRouter extends EventEmitter {
  ...
  async _execute(provider, text, apiKey) {
    ...
    const endpoint = provider.isLocal ? `${provider.baseUrl}/embed` : `${provider.baseUrl}/embeddings`;
    const body = provider.isLocal ? JSON.stringify({
  ...
  async embedBatch(texts, opts = {}) {
    ...
    for (let start = 0; start < texts.length; start += batchSz) {
      ...
      for (let j = 0; j < settled.length; j++) {
        const s = settled[j];
        results[start + j] = s.status === 'fulfilled' ? s.value : new Float64Array(this._enableMRL ? MRL_TARGET_DIM : fib(9));
      }
      this.emit('batch:chunk-complete', {
        start,
  ...
  get totalCostUsd() {
    return +this._totalCostUsd.toFixed(8);
  }
}
