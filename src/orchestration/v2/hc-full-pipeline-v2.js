...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/orchestration/v2/hc-full-pipeline-v2.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/*
...
'TRIAGE',
// 2. Classify priority, route to correct node pool
'MONTE_CARLO',
// 3. Async risk assessment simulation
'ARENA',
// 4. Multi-node competition (if enabled)
'JUDGE',
// 5. Score & rank outputs
'APPROVE',
// 6. Human approval gate (for HIGH/CRITICAL risk)
'EXECUTE',
// 7. Run the winning strategy
'VERIFY',
...
const DEFAULT_STAGE_TIMEOUTS = {
  ...
  JUDGE: 15_000,
  APPROVE: 600_000,
  // Human approval can take up to 10 minutes
  EXECUTE: 120_000,
  VERIFY: 15_000,
  RECEIPT: 5_000
};
...
async function runMonteCarloAsync(engine, scenario, iterations) {
  ...
  for (let chunk = 0; chunk < iterations; chunk += MC_CHUNK_SIZE) {
    ...
    for (let i = chunk; i < chunkEnd; i++) {
      ...
      if (totalImpact < 0.3) successCount++;else if (totalImpact < 0.7) partialCount++;else failureCount++;
    }
  ...
  const topMitigations = Object.entries(mitigationHits).sort((a, b) => b[1] - a[1]).slice(0, 5).map(([m]) => m);
  return {
...
class HCFullPipeline extends EventEmitter {
  ...
  async _executeStageWithHeal(run, stage, stageIndex) {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        const result = await this._withTimeout(this._executeStage(run, stage), timeout, `Stage ${stage.name} timed out after ${timeout}ms`);
        stage.result = result;
  ...
  async _stageMonteCarlo(run) {
    const scenario = {
      name: run.requestId,
      baseSuccessRate: run._monteCarloBaseRate || 0.85,
      // [NEW] seeded by feedback
    ...
    if (typeof this._onRunComplete === 'function') {
      try {
        this._onRunComplete({
          runId: run.id,
          success,
          durationMs: run.finishedAt && run.startedAt ? new Date(run.finishedAt) - new Date(run.startedAt) : 0,
  ...
  listRuns(limit = 20) {
    return [...this._runs.values()].sort((a, b) => (b.startedAt || '').localeCompare(a.startedAt || '')).slice(0, limit);
  }
  status() {
