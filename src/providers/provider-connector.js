...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/providers/provider-connector.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/**
...
class BaseAdapter {
  ...
  async streamGenerate() {
    throw new Error(`${this.name}.streamGenerate not implemented`);
  }
}
...
class GoogleAdapter extends BaseAdapter {
  ...
  async health() {
    try {
      const res = await this._fetch(this._urlWithKey('/models'), {
        method: 'GET',
        timeoutMs: HEALTH_CHECK_TIMEOUT_MS
      });
      this._healthy = res.ok;
...
class CloudflareAdapter extends BaseAdapter {
  ...
  async generate(prompt, opts = {}) {
    ...
    const res = await this._fetch(`${this.baseUrl}/${model}`, {
      body
    });
    const data = await res.json();
    return {
...
class ProviderConnector {
  ...
  async healthAll() {
    const results = await Promise.allSettled(Object.entries(this.adapters).map(async ([name, adapter]) => {
    ...
    for (const r of results) {
      if (r.status === 'fulfilled') {
        ...
        summary[r.reason?.provider || 'unknown'] = {
          healthy: false,
