...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/liquid/liquid-sandbox.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/**
...
const TRUST_THRESHOLDS = Object.freeze({
  NATIVE: CSL_THRESHOLDS.CRITICAL,
  // 0.927 — only highest trust
  WASM: CSL_THRESHOLDS.HIGH,
  // 0.882
  DOCKER: CSL_THRESHOLDS.MEDIUM,
  // 0.809
...
class LiquidSandbox extends EventEmitter {
  constructor(config = {}) {
    ...
    logger.info({
      availableTiers: this._available
    }, 'LiquidSandbox initialized');
  }
  ...
  async execute(code, options = {}) {
    ...
    const instance = new SandboxInstance(crypto.randomUUID(), effectiveTier, capabilities);
    ...
    try {
      ...
      this._metrics.avgStartupMs[effectiveTier] = this._metrics._startupSums[effectiveTier] / this._metrics.executions[effectiveTier];
      this.emit('sandbox:complete', {
        id: instance.id,
  ...
  async _execFirecracker(instance, code, language, options) {
    ...
    logger.info({
      id: instance.id
    }, 'Firecracker tier — falling back to Docker with --runtime=runsc');
