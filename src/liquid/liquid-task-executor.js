...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/liquid/liquid-task-executor.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/**
...
class LiquidTaskExecutor extends EventEmitter {
  ...
  async executeDAG(dag) {
    ...
    while (completed.size + failed.size < dag.size) {
      ...
      const promises = toExecute.map(taskId => {
        ...
        return this._executeWithRetry(task).then(result => ({
          taskId,
          success: true,
          result
        })).catch(error => ({
      ...
      if (promises.length > 0) {
        ...
        const settled = await Promise.race(promises.map(p => p.then(r => {
  ...
  async _executeWithTimeout(task, timeoutMs) {
    return new Promise((resolve, reject) => {
      ...
      node.executeTask(task).then(result => {
        clearTimeout(timer);
        resolve(result);
      }).catch(err => {
        clearTimeout(timer);
  ...
  decompose(complexTask) {
    ...
    for (const [taskId, task] of dag.tasks) {
      task.priority = phiFusionScore([task.urgency || 0.5, task.complexity || 0.5, task.coherence || 0.5]);
    }
  ...
  getStats() {
    return {
      ...
      avgLatencyMs: this.totalExecuted > 0 ? Math.round(this.totalLatencyMs / this.totalExecuted) : 0,
