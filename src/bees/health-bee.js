...
const priority = 0.95;
const HEADY_DOMAINS = ['headyme.com', 'headysystems.com', 'headyconnection.org', 'headymcp.com', 'headyio.com', 'headybuddy.org', 'headybot.com', 'headyos.com', 'headyapi.com', 'heady-ai.com', 'headylens.com', 'headyex.com', 'headyfinance.com', 'headyconnection.com', 'perfecttrader.com', 'headyai.me'];
const INTERNAL_ENDPOINTS = [{
  name: 'cloud-run',
  url: '/api/health'
}, {
  name: 'brain',
  url: '/api/brain/status'
}, {
  name: 'registry',
  url: '/api/registry'
}];
function getWork(ctx = {}) {
  ...
  work.push(...HEADY_DOMAINS.map(d => async () => {
    try {
      ...
      const hasForbidden = ["0.0.0.0", 'TODO', 'FIXME'].some(f => html.includes(f));
      return {
        bee: domain,
        action: `check-${d}`,
        status: resp.status,
        latency,
        healthy: resp.ok && hasHeadyBrand && !hasForbidden,
        branding: hasHeadyBrand,
        canvas: hasCanvas,
  ...
  work.push(...INTERNAL_ENDPOINTS.map(ep => async () => {
    try {
      ...
      const resp = await fetch(`${baseUrl}${ep.url}`, {
        signal: AbortSignal.timeout(5000)
      });
      return {
        bee: domain,
        action: `liveness-${ep.name}`,
        status: resp.status,
        healthy: resp.ok
      };
    } catch (err) {
