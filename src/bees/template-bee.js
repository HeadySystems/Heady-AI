...
// ─── Nav Generator ──────────────────────────────────────────────────
const HEADY_SITES = [["HeadyMe", "https://headyme.com"], ["HeadyBuddy", "https://headybuddy.org"], ["HeadySystems", "https://headysystems.com"], ["HeadyConnection", "https://headyconnection.org"], ["HeadyMCP", "https://headymcp.com"], ["HeadyIO", "https://headyio.com"], ["HeadyBot", "https://headybot.com"], ["HeadyOS", "https://headyos.com"], ["HeadyAPI", "https://headyapi.com"]];
function generateNav(activeDomain) {
  return HEADY_SITES.map(([name, url]) => `<a href="${url}" ${url.includes(activeDomain) ? 'class="active"' : ""}>${name}</a>`).join("");
}

// ─── Auth Gate Generator ────────────────────────────────────────────
function generateAuthGate(siteName, accent, domain) {
  const providerButtons = AUTH_PROVIDERS.map(p => `<button class="auth-provider" onclick="heady_auth('${p.id}')">${p.icon}<span>${p.name}</span></button>`).join("");
  return `
...
function heady_auth(provider){
  ...
  g.style.opacity='0';
  setTimeout(function(){g.style.display='none';}, typeof phiMs === 'function' ? phiMs(500) : 500);
  try{sessionStorage.setItem('heady_auth_session',JSON.stringify({ts:Date.now(),site:'${domain}',provider:provider}));}catch (e) { logger.error('Recovered from error:', e); }
...
(function(){
  ...
  if(s){try{var d=JSON.parse(s);if(Date.now()-d.ts<86400000){var g=document.getElementById('heady-auth-gate');if(g){g.style.display='none';}}}catch (e) { logger.error('Recovered from error:', e); }}
})();
<\\/script>`;
}
function renderSite(hostname) {
  ...
  const cardHTML = (features || []).map(f => `<div class="card"><div class="ci">${f.icon}</div><h3>${f.title}</h3><p>${f.desc}</p></div>`).join("");
