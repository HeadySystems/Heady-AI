...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/bees/session-templates.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
const {
    ...
    fn: async (ctx = {}) => {
      ...
      try {
        ...
        return {
          dir: outputDir,
          rendered: false,
          reason: `No site config for ${hostname}`
        };
      } catch (err) {
  ...
  workers: [{
    ...
    fn: async (ctx = {}) => {
      ...
      try {
        ...
        } = require('../core/audio-overview');
        return {
          script: generateOverviewScript(),
          source: 'audio-overview-module'
        };
      } catch {
        ...
        return {
          ...
          source: 'fallback',
          note: 'audio-overview module not available'
        };
      }
    }
  }, {
    ...
    fn: async (ctx = {}) => {
      ...
      try {
        ...
        return {
          synthesized: false,
          error: err.message,
          note: 'TTS module not available — use NotebookLM or external TTS'
        };
      }
    }
  }],
...
const colabGpuRuntime = createBee('colab-gpu-runtime', {
  ...
  workers: [{
    ...
    fn: async (ctx = {}) => {
      ...
      if (!url) return {
        status: 'offline',
        reason: 'No embedding URL configured'
      };
      try {
    ...
    fn: async (ctx = {}) => {
      ...
      if (!keyFile) {
        ...
        try {
          ...
          try {
            execSync(`gcloud iam service-accounts describe ${saEmail} --project=${project} 2>/dev/null`, {
              encoding: 'utf8'
            });
            logger.info(`gcloud-auth-automator: SA ${saEmail} already exists`);
          } catch {
            execSync(`gcloud iam service-accounts create ${saName} --display-name="Heady™ Auto Deployer" --project=${project} --quiet`, {
              encoding: 'utf8'
            });
          }
          execSync(`gcloud projects add-iam-policy-binding ${project} --member="serviceAccount:${saEmail}" --role="roles/run.admin" --quiet 2>/dev/null`, {
            encoding: 'utf8'
          });
          ...
          execSync(`gcloud iam service-accounts keys create ${keyPath} --iam-account=${saEmail} --quiet`, {
            encoding: 'utf8'
          });
          return {
            keyFile: keyPath,
  ...
  workers: [{
    ...
    fn: async (ctx = {}) => {
      const endpoints = ctx.endpoints || [{
        name: 'cloud-run',
        url: 'https://heady-manager-609590223909.us-central1.run.app/health'
      }, {
        name: 'local',
        url: "http://0.0.0.0:8420/health"
      }, {
        name: 'colab',
        url: process.env.HEADY_EMBEDDING_URL ? `${process.env.HEADY_EMBEDDING_URL}/health` : null
      }].filter(e => e.url);
      ...
      const winners = results.filter(r => r.status === 'fulfilled').map(r => r.value).sort((a, b) => a.latency - b.latency);
      return {
  ...
  workers: [{
    ...
    fn: async (ctx = {}) => {
      ...
      (async () => {
        const deployer = require('./bee-factory').dynamicRegistry.get('cloud-run-deployer');
        return deployer?.getWork()[0].fn({
          service: 'headyme-site',
          source: 'src/landing',
          memory: '128Mi'
        });
      })()]);
      results.manager = manager.status === 'fulfilled' ? manager.value : {
        error: manager.reason?.message
      };
      results.site = site.status === 'fulfilled' ? site.value : {
        error: site.reason?.message
      };
  ...
  workers: [{
    ...
    fn: async (ctx = {}) => {
      ...
      if (analyzer) {
        ...
        results.valuation = await workers[1].fn({
          ...ctx,
          ...results.comps
        });
      }
      ...
      if (audio) {
        ...
        try {
          results.audio = await workers[1].fn({
  ...
  workers: [{
    ...
    fn: async (ctx = {}) => {
      const endpoints = ctx.endpoints || [{
        ...
        name: 'edge',
        url: 'https://heady.headyme.com/health'
      }, {
        name: 'local',
        url: "http://0.0.0.0:8420/health"
      }];
    ...
    fn: async (ctx = {}) => {
      ...
      try {
        ...
        for (const chunk of chunks) {
          ...
          if (result.ok) embedded++;else errors++;
        }
      } catch (err) {
        return {
          embedded,
