...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/bees/hf-auth-3d-bees.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
const {
...
async function inject(content, meta = {}) {
  if (!vectorMemory || !vectorMemory.smartIngest) {
    return {
      stored: false,
      reason: 'vector memory not available'
    };
  }
  try {
    return await vectorMemory.smartIngest({
      content,
      metadata: meta
    });
  } catch (err) {
...
const PROVIDERS = {
  oauth: ['google', 'github', 'microsoft', 'apple', 'facebook', 'amazon', 'discord', 'slack', 'linkedin', 'twitter', 'spotify', 'huggingface'],
  apikey: ['openai', 'claude', 'gemini', 'perplexity', 'mistral', 'cohere', 'groq', 'replicate', 'together', 'fireworks', 'deepseek', 'xai', 'anthropic']
};
  ...
  workers: [{
    ...
    fn: async ctx => {
      ...
      const knownFixes = [{
        ...
        fix: 'language="json"',
        reason: 'Gradio 5.x removed text lang from gr.Code'
      }, {
        pattern: "type='tuples'",
        fix: "type='messages'",
        reason: 'Gradio 5.x deprecated tuples format'
      }, {
        pattern: 'sdk_version: 4',
        fix: 'sdk_version: 5.23.0',
        reason: 'Old SDK version'
      }];
      await inject(`HF space ${spaceId} diagnosed`, {
        type: 'fix',
        source: 'hf-space-fixer',
        spaceId
      });
      return {
        spaceId,
    ...
    fn: async ctx => {
      ...
      if (!fixType) return {
        error: 'fixType is required (e.g., "gradio-lang", "sdk-version")',
        status: 'failed'
      };
      const fixMap = {
        'gradio-lang': {
        ...
        'sdk-version': {
          find: /sdk_version:\s*\d+/,
          replace: 'sdk_version: 5.23.0'
        }
      };
      const fix = fixMap[fixType];
      if (!fix) return {
        error: `Unknown fixType: ${fixType}. Available: ${Object.keys(fixMap).join(', ')}`,
        status: 'failed'
      };
...
const dataInjector3D = createBee('data-injector-3d', {
  ...
  workers: [{
    ...
    fn: async ctx => {
      ...
      if (!content) return {
        error: 'No content to inject (provide ctx.content or ctx.input)',
        injected: false
      };
      const meta = ctx.metadata || {
        type: 'memory',
        source: 'data-injector-3d'
      };
  ...
  workers: [{
    ...
    fn: async ctx => {
      ...
      if (lower.match(/hf|hugging\s?face|space|gradio/)) {
        ...
        const work = hfSpaceFixer.getWork({
          input: text,
          spaceId: text
        });
        result = work.length ? await work[0]() : {
          status: 'no-workers'
        };
        ...
        const work = universalAuth.getWork({
          provider,
          email: ctx.email
        });
        result = work.length > 1 ? await work[1]() : {
          status: 'no-workers'
        };
      } else if (lower.match(/inject|store|memory|vector|embed|3d|ingest/)) {
        route = 'data-injector-3d';
        const work = dataInjector3D.getWork({
          content: text
        });
        result = work.length ? await work[0]() : {
          status: 'no-workers'
        ...
        try {
          ...
          result = extractor.extractTasks ? extractor.extractTasks(text) : {
            tasks: []
          };
        } catch {
          result = {
            error: 'task extractor not loaded'
          };
        }
      } else {
