...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/bees/mistake-analyzer-bee.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
'use strict';
...
class MistakeAnalyzerBee {
  ...
  async execute(task) {
    ...
    // Cache pattern
    this._patternCache.set(fingerprint, {
      count: (this._patternCache.get(fingerprint)?.count ?? 0) + 1,
      lastSeen: Date.now()
    });
  ...
  _fiveWhys(mistake, context, category) {
    const templates = [`Why did "${mistake.slice(0, 80)}" occur?`, `Why was the precondition for this not validated in stage ${context.stage ?? 'UNKNOWN'}?`, `Why did the input/output contract allow this failure mode for category "${category}"?`, `Why was there no circuit breaker or retry policy to catch this class of error?`, `Root cause: Insufficient phi-harmonic coherence gate at stage boundary, allowing sub-${PSI.toFixed(3)} confidence through.`];
  ...
  _updateCoherence() {
    const regressionRate = this._analysisCount > 0 ? this._regressionHits / this._analysisCount : 0;
