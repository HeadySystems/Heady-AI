...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/bees/headybee-template-registry.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
...
const REQUIRED_SCENARIOS = ['outage', 'projection-drift', 'launch', 'stale-files', 'vulnerability-scan', 'deploy-production', 'self-healing'];
...
class HeadyBeeTemplateRegistry {
  ...
  recommend(signal = {}) {
    const ranked = this.list().map(template => ({
      template,
      match: scoreTemplate(template, signal)
  ...
  buildMaintenanceAudit(input = {}) {
    ...
    const staleCandidates = files.filter(file => file.endsWith('.log') || file.endsWith('.pid') || file.includes('service-worker') || file.includes('cloudflared') || file.includes('ngrok'));
    return {
      checkedAt: new Date().toISOString(),
      staleCandidates,
      shouldReviewCount: staleCandidates.length,
      notes: ['Audit is non-destructive. Review candidates before deletion.', 'Prefer config-level disablement for tunnels and service workers if cloud-native deploy is active.']
    };
  }
