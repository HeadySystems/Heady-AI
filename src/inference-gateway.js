...
class InferenceGateway extends EventEmitter {
    ...
    loadDynamicRoster() {
        ...
        try {
            ...
            if (!roster.models) return;
            
            // Group models by provider key
            const modelsByProvider = {};
            for (const model of Object.values(roster.models)) {
                if (!model.available) continue;
                const pKey = model.provider === 'google' ? 'gemini' : model.provider === 'anthropic' ? 'claude' : model.provider;
                if (!modelsByProvider[pKey]) modelsByProvider[pKey] = [];
                modelsByProvider[pKey].push(model);
            }

            // For each provider, sort models by ELO and map to tiers
            for (const [pKey, models] of Object.entries(modelsByProvider)) {
                if (!PROVIDERS[pKey]) continue;
                
                // Sort by ELO descending
                models.sort((a, b) => (b.elo || 1000) - (a.elo || 1000));

                const fastModels = models.filter(m => m.tier === 'fast' || m.tier === 'credits' || m.tier === 'speed');
                const qualityModels = models.filter(m => m.tier === 'quality');

                const fast = fastModels[0]?.id || models[0]?.id;
                const quality = qualityModels[0]?.id || models[0]?.id;
                const def = models[0]?.id;

                PROVIDERS[pKey].models = {
                    fast: fast || PROVIDERS[pKey].models.fast,
                    quality: quality || PROVIDERS[pKey].models.quality,
                    default: def || PROVIDERS[pKey].models.default,
                };
                
                logger.info(`[InferenceGateway] Loaded dynamic models for ${pKey}: ${JSON.stringify(PROVIDERS[pKey].models)}`);
            }
        } catch (e) {
            logger.error('[InferenceGateway] Failed to load dynamic model roster', { error: e.message });
        }
    }
    ...
    watchRoster() {
        ...
        try {
            if (fs.existsSync(ROSTER_FILE)) {
                fs.watch(ROSTER_FILE, (event) => {
                    if (event === 'change') {
                        logger.info('[InferenceGateway] Model roster changed on disk, reloading...');
                        this.loadDynamicRoster();
                    }
                });
            }
