...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/bridge/midi-to-mcp-bridge.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/**
...
class MidiParser {
  ...
  static parse(bytes) {
    ...
    while (i < bytes.length) {
      ...
      if (byte === MIDI_STATUS.TIMING_CLOCK || byte === MIDI_STATUS.ACTIVE_SENSING) {
        i++;
        continue;
      }
  ...
  static encodeNoteOn(channel, note, velocity) {
    return Buffer.from([MIDI_STATUS.NOTE_ON | channel & 0x0F, note & 0x7F, velocity & 0x7F]);
  }
  ...
  static encodeCC(channel, controller, value) {
    return Buffer.from([MIDI_STATUS.CONTROL_CHANGE | channel & 0x0F, controller & 0x7F, value & 0x7F]);
  }
}
...
class GestureRecognizer {
  ...
  recognize(event) {
    ...
    switch (event.type) {
      ...
      case 'program_change':
        return {
          gesture: 'preset_select',
          channel: event.channel,
          preset: event.program
        };
      default:
        return null;
    }
  ...
  _detectPadHit(event, ts) {
    ...
    const recent = this._history.filter(h => h.event.type === 'note_on' && h.event.note === event.note && ts - h.ts < this._windowMs && h.ts !== ts);
    return {
...
class MidiToMcpTranslator {
  ...
  _ccToMcp(event) {
    ...
    return {
      tool: mapping.tool,
      params: {
        [mapping.param]: +paramValue.toFixed(4)
      },
      meta: {
        source: 'midi_cc',
        controller: event.controller,
  ...
  _noteToMcp(event) {
    ...
    return {
      tool: mapping.tool,
      params: Object.assign({}, mapping.params, {
...
class McpDispatcher {
  constructor(opts = {}) {
    this._endpoint = opts.endpoint || "http://0.0.0.0:3000/mcp";
...
class RtpMidiSocket {
  ...
  static decodeRtpPacket(buf) {
    ...
    const midiLen = hasLongHeader ? (midiSection[0] & 0x0F) << 8 | midiSection[1] : midiSection[0] & 0x0F;
  ...
  listen(onEvent) {
    ...
    this._socket.on('message', (msg, rinfo) => {
      ...
      if (!packet) return;
      const session = this._sessions.get(packet.ssrc) || {
        ssrc: packet.ssrc,
        seq: -1
      };
...
class MidiMcpBridge {
  ...
  processMidiBytes(bytes) {
    try {
      const events = this._useUmp ? MidiParser.parseUMP(new Uint32Array(bytes.buffer)) : MidiParser.parse(bytes);
