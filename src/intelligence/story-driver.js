...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/intelligence/story-driver.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/**
...
const NARRATIVE_TEMPLATES = {
  [EVENT_TYPES.TASK_COMPLETED]: d => `Task "${d.taskName ?? 'unnamed'}" completed successfully in ${_fmtDuration(d.durationMs)} via ${d.provider ?? 'unknown provider'}.`,
  [EVENT_TYPES.TASK_FAILED]: d => `Task "${d.taskName ?? 'unnamed'}" failed after ${d.attempts ?? 1} attempt(s): ${d.errorMessage ?? 'unknown error'}.`,
  [EVENT_TYPES.HEALING_TRIGGERED]: d => `Self-healing initiated for service "${d.service ?? 'unknown'}" — reason: ${d.reason ?? 'anomaly detected'}.`,
  [EVENT_TYPES.HEALING_RESOLVED]: d => `Service "${d.service ?? 'unknown'}" restored after ${_fmtDuration(d.durationMs)}. ${d.action ?? 'Automatic recovery'} succeeded.`,
  [EVENT_TYPES.DRIFT_DETECTED]: d => `Metric drift detected in "${d.metric ?? 'unknown metric'}" — observed ${d.observedValue}, expected ${d.expectedValue} (threshold: ${d.threshold ?? 'N/A'}).`,
  [EVENT_TYPES.DRIFT_RESOLVED]: d => `Drift in "${d.metric ?? 'unknown metric'}" resolved. Values normalized back within acceptable range.`,
  [EVENT_TYPES.PIPELINE_RUN]: d => `Pipeline "${d.pipelineName ?? 'unnamed'}" ran ${d.stages ?? 0} stages in ${_fmtDuration(d.durationMs)}. ` + `Result: ${d.outcome ?? 'completed'}.`,
  [EVENT_TYPES.PIPELINE_FAILED]: d => `Pipeline "${d.pipelineName ?? 'unnamed'}" failed at stage "${d.failedStage ?? 'unknown'}": ${d.errorMessage ?? 'unknown error'}.`,
  [EVENT_TYPES.DEPLOYMENT]: d => `Service "${d.service ?? 'unknown'}" deployed version ${d.version ?? 'unknown'} to ${d.target ?? 'production'}.`,
  [EVENT_TYPES.DEPLOYMENT_FAILED]: d => `Deployment of "${d.service ?? 'unknown'}" v${d.version ?? '?'} to ${d.target ?? 'unknown'} failed: ${d.errorMessage ?? 'unknown error'}.`,
  [EVENT_TYPES.ROLLBACK]: d => `Rollback of "${d.service ?? 'unknown'}" from v${d.fromVersion ?? '?'} to v${d.toVersion ?? '?'} completed in ${_fmtDuration(d.durationMs)}.`,
  [EVENT_TYPES.BEE_SPAWNED]: d => `Bee "${d.beeId ?? 'unknown'}" (domain: ${d.domain ?? 'general'}) spawned for task "${d.taskName ?? 'unnamed'}".`,
  [EVENT_TYPES.BEE_TERMINATED]: d => `Bee "${d.beeId ?? 'unknown'}" terminated after ${_fmtDuration(d.lifetimeMs)} — reason: ${d.reason ?? 'task completed'}.`,
  [EVENT_TYPES.PROVIDER_SWITCHED]: d => `LLM provider switched from ${d.fromProvider ?? 'unknown'} to ${d.toProvider ?? 'unknown'} for task "${d.taskType ?? 'unknown'}" — reason: ${d.reason ?? 'failover'}.`,
  [EVENT_TYPES.BUDGET_WARN]: d => `Budget warning: ${d.provider ?? 'unknown'} is at ${Math.round(d.usage / d.cap * 100)}% of its daily cap ($${(d.usage ?? 0).toFixed(2)}/$${(d.cap ?? 0).toFixed(2)}).`,
  [EVENT_TYPES.CIRCUIT_OPENED]: d => `Circuit breaker OPENED for provider "${d.provider ?? 'unknown'}" after ${d.failures ?? 0} consecutive failures.`,
  [EVENT_TYPES.CIRCUIT_CLOSED]: d => `Circuit breaker CLOSED for provider "${d.provider ?? 'unknown'}" — service recovered after ${_fmtDuration(d.durationMs)}.`,
  [EVENT_TYPES.PATTERN_LEARNED]: d => `New ${d.patternType ?? 'behavioral'} pattern recorded for "${d.taskType ?? 'unknown'}" (${d.occurrences ?? 1} occurrences, confidence: ${((d.confidence ?? 0) * 100).toFixed(0)}%).`,
  [EVENT_TYPES.CONFIG_CHANGED]: d => `Configuration change detected in "${d.file ?? 'unknown file'}": ${d.changedKeys?.join(', ') ?? 'multiple keys'} updated.`,
  [EVENT_TYPES.SYSTEM_STARTED]: d => `HeadySystems™ Sovereign AI Platform v${d.version ?? '4.0.0'} started. Modules loaded: ${(d.modules ?? []).join(', ') || 'none'}.`,
  [EVENT_TYPES.SYSTEM_STOPPED]: d => `Platform gracefully shut down after ${_fmtDuration(d.uptimeMs)}. Total requests served: ${d.totalRequests ?? 0}.`,
  [EVENT_TYPES.HEALTH_CHECK]: d => `System health check: ${d.healthy ?? 0} healthy, ${d.degraded ?? 0} degraded, ${d.unhealthy ?? 0} unhealthy components.`,
  [EVENT_TYPES.MCP_TOOL_CALLED]: d => `MCP tool "${d.toolName ?? 'unknown'}" called by session "${d.sessionId ?? 'anonymous'}" — ${d.outcome ?? 'completed'} in ${_fmtDuration(d.durationMs)}.`,
  [EVENT_TYPES.VECTOR_STORED]: d => `${d.count ?? 1} vector embedding(s) stored in ${d.namespace ?? 'default'} namespace.`,
  [EVENT_TYPES.CUSTOM]: d => d.message ?? `Custom event recorded: ${JSON.stringify(d)}`
};
...
class StoryStore {
  ...
  _appendToDisk(entry) {
    try {
      fs.mkdirSync(path.dirname(this.persistPath), {
        recursive: true
      });
...
class StoryDriver extends EventEmitter {
  ...
  subscribe(emitter, bindings) {
    for (const binding of bindings) {
      emitter.on(binding.event, data => {
        ...
        this.record(binding.eventType, transformed, {
  ...
  sessionNarrative(sessionId) {
    ...
    const intro = `Session "${sessionId}" ran from ${start} to ${end} ` + `(${_fmtDuration(duration)}), comprising ${entries.length} events:\n`;
  ...
  async search(query, k = 10) {
    ...
    const hits = all.filter(e => e.narrative.toLowerCase().includes(lower) || e.eventType.toLowerCase().includes(lower) || (e.service ?? '').toLowerCase().includes(lower));
    return hits.slice(-k);
  }
  ...
  _defaultSeverity(eventType) {
    const errorTypes = [EVENT_TYPES.TASK_FAILED, EVENT_TYPES.PIPELINE_FAILED, EVENT_TYPES.DEPLOYMENT_FAILED];
    const warnTypes = [EVENT_TYPES.HEALING_TRIGGERED, EVENT_TYPES.DRIFT_DETECTED, EVENT_TYPES.BUDGET_WARN, EVENT_TYPES.CIRCUIT_OPENED];
    const criticalTypes = [EVENT_TYPES.ROLLBACK];
...
export { StoryDriver, StoryStore, EVENT_TYPES, NARRATIVE_TEMPLATES };