...
// --- Domain definitions -----------------------------------------------------
const DOMAINS = ['nodejs', 'react', 'sql', 'cloudrun', 'cloudflare', 'sacred-geometry-css', 'mcp-protocol', 'agent-orchestration', 'security', 'trading', 'devops', 'prompt-engineering'];
...
const CHALLENGE_POOL = [{
  ...
  examples: ['scheduleAll(3, cb) // cb called with 0,1,2'],
  constraints: ['No setTimeout', 'Must return Promise']
}, {
  id: 'nd-02',
  domain: 'nodejs',
  difficulty: 4,
  title: 'Stream Backpressure Handler',
  ...
  examples: ['new BackpressureTransform({ maxSize: 1024 })'],
  constraints: ['Extend Transform', 'Handle drain event']
}, {
  id: 'rc-01',
  domain: 'react',
  difficulty: 2,
  ...
  examples: ['const debouncedSearch = useDebounce(query, 300)'],
  constraints: ['Use useEffect + useState', 'Cleanup on unmount']
}, {
  id: 'rc-02',
  domain: 'react',
  difficulty: 3,
  ...
  examples: ['<VirtualList items={data} rowHeight={40} height={400} />'],
  constraints: ['Calculate visible range', 'Use absolute positioning']
}, {
  id: 'sq-01',
  domain: 'sql',
  difficulty: 3,
  ...
  examples: ['SELECT *, DENSE_RANK() OVER (PARTITION BY dept ...)'],
  constraints: ['Use DENSE_RANK', 'Order by salary DESC']
}, {
  id: 'cr-01',
  domain: 'cloudrun',
  difficulty: 3,
  ...
  examples: ['app.get("/_startup", handler)'],
  constraints: ['Parallel warmup', 'Timeout after 10s']
}, {
  id: 'cf-01',
  domain: 'cloudflare',
  difficulty: 3,
  title: 'Edge KV Rate Limiter',
  description: 'Implement a Cloudflare Worker that rate-limits requests per IP using KV storage with a sliding window.',
  examples: ['addEventListener("fetch", handleRequest)'],
  constraints: ['Use KV namespace', '60 req/min limit']
}, {
  id: 'cf-02',
  domain: 'cloudflare',
  difficulty: 4,
  ...
  examples: ['purgeByTag("blog:*")'],
  constraints: ['Use Cache API', 'Support glob matching']
}, {
  id: 'sg-01',
  domain: 'sacred-geometry-css',
  difficulty: 2,
  ...
  examples: ['.grid { grid-template-columns: 1fr 1.618fr 2.618fr; }'],
  constraints: ['Pure CSS', 'Use custom properties for phi']
}, {
  id: 'sg-02',
  domain: 'sacred-geometry-css',
  difficulty: 4,
  title: 'Fibonacci Spiral Animation',
  ...
  examples: ['@keyframes spiral { ... }'],
  constraints: ['Use CSS transforms', 'At least 8 segments']
}, {
  id: 'mp-01',
  domain: 'mcp-protocol',
  difficulty: 3,
  ...
  examples: ['registry.register({ name, schema, handler })'],
  constraints: ['JSON Schema validation', 'Semver versioning']
}, {
  id: 'mp-02',
  domain: 'mcp-protocol',
  difficulty: 5,
  title: 'MCP Streaming Transport',
  description: 'Build an MCP transport layer that supports server-sent events for streaming tool results back to the client.',
  examples: ['transport.stream(toolId, params, onChunk)'],
  constraints: ['SSE format', 'Heartbeat keepalive']
}, {
  id: 'ao-01',
  domain: 'agent-orchestration',
  difficulty: 4,
  title: 'Fan-Out Supervisor',
  ...
  examples: ['supervisor.fanOut(task, agents, consensusFn)'],
  constraints: ['Handle agent failures', 'Configurable timeout']
}, {
  id: 'ao-02',
  domain: 'agent-orchestration',
  difficulty: 5,
  ...
  examples: ['memory.write(agentId, key, value, clock)'],
  constraints: ['Vector clock merge', 'Last-writer-wins fallback']
}, {
  id: 'se-01',
  domain: 'security',
  difficulty: 3,
  title: 'Timing-Safe Comparator',
  ...
  examples: ['timingSafeEqual(input, secret)'],
  constraints: ['Fixed-time execution', 'Handle different lengths']
}, {
  id: 'se-02',
  domain: 'security',
  difficulty: 4,
  title: 'JWT Rotation Manager',
  ...
  examples: ['rotator.sign(payload) / rotator.verify(token)'],
  constraints: ['Support RS256', 'Max 2 active keys']
}, {
  id: 'tr-01',
  domain: 'trading',
  difficulty: 3,
  ...
  examples: ['detectCrossover(shortMA, longMA) // { type, index }'],
  constraints: ['Return crossover points', 'Handle edge cases']
}, {
  id: 'dv-01',
  domain: 'devops',
  difficulty: 2,
  ...
  examples: ['GET /health => { status, services: [...] }'],
  constraints: ['Parallel probes', '5s timeout per service']
}, {
  id: 'dv-02',
  domain: 'devops',
  difficulty: 4,
  title: 'Canary Deploy Controller',
  ...
  examples: ['canary.advance() // 1% -> 5% -> 20% -> 100%'],
  constraints: ['Rollback on error spike', 'Configurable steps']
}, {
  id: 'pe-01',
  ...
  examples: ['engine.render(template, { vars, examples })'],
  constraints: ['{{var}} syntax', '#if/#endif conditionals']
}];
...
function evaluateSubmission(challengeId, code) {
  ...
  if (lines <= 5) breakdown.efficiency = 8;else if (lines <= 15) breakdown.efficiency = 20;else if (lines <= 40) breakdown.efficiency = 16;else if (lines <= 80) breakdown.efficiency = 12;else breakdown.efficiency = 6;
  ...
  const feedback = score >= 80 ? 'Excellent solution!' : score >= 60 ? 'Good approach, consider edge cases and patterns.' : score >= 40 ? 'Decent start. Focus on structure and constraint coverage.' : 'Needs improvement. Review the challenge constraints.';
...
function extractPatterns(code) {
  ...
  const checks = [{
    name: 'factory',
    re: /function\s+create\w+|new\s+\w+\(|=>\s*new\s+/
  }, {
    name: 'observer',
    re: /\.on\(|\.emit\(|addEventListener|subscribe|EventEmitter/
  }, {
    name: 'middleware',
    re: /next\(\)|use\(|middleware|req\s*,\s*res\s*,\s*next/
  }, {
    name: 'singleton',
    re: /getInstance|instance\s*===\s*null|_instance/
  }, {
    name: 'strategy',
    re: /strategy|strategies\[|getStrategy/
  }, {
    name: 'decorator',
    re: /wrapper|decorate|@\w+|Object\.assign\(.*prototype/
  }, {
    name: 'promise-chain',
    re: /\.then\(|\.catch\(|async\s+function|await\s+/
  }, {
    name: 'iterator',
    re: /Symbol\.iterator|\[Symbol\.iterator\]|next\(\)\s*\{|yield\s+/
  }, {
    name: 'proxy',
    re: /new\s+Proxy\(|handler\s*=\s*\{.*get\s*:/
  }, {
    name: 'builder',
    re: /\.set\w+\(.*\)\s*\{[^}]*return\s+this|\.build\(\)/
  }, {
    name: 'circuit-breaker',
    re: /circuitBreaker|circuit_breaker|state\s*===\s*'open'/
  }, {
    name: 'pub-sub',
    re: /publish|subscribe|channel|topic/
  }];
  for (const {
    name,
    re
    ...
    if (re.test(code)) {
      detected.push({
        name,
        confidence: Math.round(PHI * 60) / 100
      });
    }
  }
  return detected;
...
function getDailyStats() {
  ...
  const avgScoreToday = completedToday > 0 ? Math.round(todaySubs.reduce((a, s) => a + s.score, 0) / completedToday) : 0;
  return {
    completedToday,
    avgScoreToday,
...
router.post('/api/v1/dojo/challenge', (req, res) => {
  try {
    ...
    const challenge = generateChallenge(difficulty ? Number(difficulty) : undefined, domain || undefined);
    res.json({
      ok: true,
      challenge
    });
