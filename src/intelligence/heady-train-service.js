...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/intelligence/heady-train-service.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
...
// ─── Skill Metrics ─────────────────────────────────────────
const SKILL_METRICS = [{
  area: 'pgvector optimization',
  score: 0.42,
  weight: 1.0,
  lastAssessed: '2026-03-01'
}, {
  area: 'multi-agent consensus',
  score: 0.38,
  weight: 0.95,
  lastAssessed: '2026-03-05'
}, {
  area: 'SOC 2 compliance',
  score: 0.33,
  weight: 0.90,
  lastAssessed: '2026-02-20'
}, {
  area: 'trading backtesting',
  score: 0.44,
  weight: 0.85,
  ...
  area: 'sacred geometry proofs',
  score: 0.59,
  weight: 0.80,
  lastAssessed: '2026-03-03'
}, {
  area: 'WebSocket streaming',
  score: 0.61,
  weight: 0.86,
  lastAssessed: '2026-03-07'
}, {
  area: 'vector embedding pipelines',
  score: 0.50,
  weight: 0.82,
  lastAssessed: '2026-02-28'
}, {
  area: 'edge function routing',
  score: 0.48,
  weight: 0.78,
  ...
  score: 0.63,
  weight: 0.70,
  lastAssessed: '2026-03-11'
}, {
  area: 'rate limiter design',
  score: 0.57,
  weight: 0.72,
  lastAssessed: '2026-03-06'
}, {
  area: 'circuit breaker patterns',
  score: 0.65,
  weight: 0.68,
  lastAssessed: '2026-03-09'
}, {
  area: 'event sourcing',
  score: 0.41,
  weight: 0.77,
...
const CURRICULUM_TEMPLATES = {
  data: {
    steps: ['Schema design review', 'Index strategy lab', 'Query optimization drill', 'Benchmark suite run'],
    resources: ['pgvector docs', 'PostgreSQL EXPLAIN guide', 'ANN benchmark repo']
  },
  infrastructure: {
    steps: ['Worker anatomy walkthrough', 'KV + D1 integration lab', 'Edge routing exercise', 'Performance profiling'],
    resources: ['CF Workers docs', 'Wrangler CLI guide', 'Edge computing patterns']
  },
  ai: {
    steps: ['Consensus algorithm study', 'Voting mechanism implementation', 'Byzantine fault tolerance lab', 'Multi-agent simulation'],
    resources: ['Raft paper', 'PBFT overview', 'Multi-agent systems textbook']
  },
  realtime: {
    steps: ['Protocol fundamentals', 'Backpressure handling lab', 'Binary frame optimization', 'Load test with k6'],
    resources: ['RFC 6455', 'ws library docs', 'WebSocket scaling patterns']
  },
  payments: {
    steps: ['API integration walkthrough', 'Webhook verification lab', 'Subscription lifecycle exercise', 'Idempotency key drill'],
    resources: ['Stripe API docs', 'Stripe testing guide', 'PCI DSS overview']
  },
  security: {
    steps: ['Control framework study', 'Evidence collection exercise', 'Audit log implementation', 'Compliance gap assessment'],
    resources: ['SOC 2 Type II guide', 'AICPA trust criteria', 'Vanta/Drata docs']
  },
  math: {
    steps: ['Golden ratio derivation', 'Fibonacci proof exercise', 'Phi-based algorithm design', 'Geometric gate implementation'],
    resources: ['Sacred geometry primer', 'CSL gate mathematics', 'Heady phi-math module']
  },
  fintech: {
    steps: ['Strategy definition framework', 'Historical data pipeline setup', 'Backtest engine walkthrough', 'Risk metric calculation'],
    resources: ['Quantitative trading intro', 'Backtrader docs', 'Sharpe ratio guide']
  }
};
...
function analyzeSkillGaps() {
  ...
  return {
    ...
    totalAreas: SKILL_METRICS.length
  };
}
...
function generateCurriculum(gap) {
  ...
  return {
    ...
    exercises: [{
      type: 'comprehension',
      prompt: `Explain the core concept behind ${topicName}`
    }, {
      type: 'implementation',
      prompt: `Build a minimal working example of ${topicName}`
    }, {
      type: 'optimization',
      prompt: `Identify and fix 3 performance issues in a ${topicName} implementation`
    }],
...
function executeTraining(curriculum) {
  ...
  const session = {
    ...
    insights: [`Covered ${stepsCompleted} steps in ${curriculum.topic}`, score >= 0.8 ? 'Strong understanding demonstrated' : 'Needs reinforcement on advanced concepts', 'Next review scheduled via spaced repetition']
  };
  ...
  bus.emit('train:session_completed', {
    sessionId,
    topic: curriculum.topic,
    score,
...
function spacedRepetition(topic) {
  ...
  if (!topicEntry) return {
    ...
    availableTopics: PRIORITY_TOPICS.map(t => t.id)
  };
  const tracker = repetitionTracker.get(topicEntry.id);
  if (!tracker) return {
    error: `No tracker entry for: ${topicEntry.id}`
  };
  const isDue = new Date() >= new Date(tracker.nextTest);
  return {
    topicId: topicEntry.id,
    topicName: topicEntry.name,
    lastTested: tracker.lastTested,
    nextTest: tracker.nextTest,
    currentInterval: tracker.interval,
    fibonacciSequence: FIB_INTERVALS,
    nextIntervalDays: FIB_INTERVALS[Math.min(tracker.reviewCount, FIB_INTERVALS.length - 1)],
    score: tracker.score,
    reviewCount: tracker.reviewCount,
    isDue,
    status: isDue ? 'REVIEW_DUE' : 'SCHEDULED'
  };
}
...
function getTrainingPriorities() {
  const priorities = PRIORITY_TOPICS.map(t => ({
    ...
    category: t.category,
    repetition: repetitionTracker.get(t.id) || null
  }));
  ...
  try {
    ...
    for (const [topicId, tracker] of repetitionTracker.entries()) {
      ...
      schedule.push({
        ...
        ...tracker,
        isDue: new Date() >= new Date(tracker.nextTest)
      });
    }
