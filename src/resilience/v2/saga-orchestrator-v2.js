...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/resilience/v2/saga-orchestrator-v2.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
'use strict';
...
class Saga extends EventEmitter {
  ...
  async run(context = {}) {
    ...
    await this._persist();
    logger.info(`[Saga:${this.name}] Started (${this._steps.length} steps)`, {
      sagaId: this.sagaId
    });
    ...
    for (const step of this._steps) {
      ...
      try {
        logger.info(`[Saga:${this.name}] → Executing: ${step.name}`, {
          idempotencyKey
        });
        const result = await withTimeout(step.execute(context), step.executeTimeoutMs, `${this.name}/${step.name} execute`);
        ...
        await this._persist();
        logger.info(`[Saga:${this.name}] ✓ ${step.name} completed (${durationMs}ms)`, {
          sagaId: this.sagaId
        });
        ...
        throw Object.assign(new Error(`Saga [${this.name}] failed at [${step.name}]: ${err.message}`), {
          sagaId: this.sagaId,
    ...
    logger.info(`[Saga:${this.name}] ✓ COMPLETED (${this._completedSteps.length} steps, ${totalMs}ms)`, {
      sagaId: this.sagaId
    });
  ...
  async _compensate(context) {
    ...
    for (let i = this._completedSteps.length - 1; i >= 0; i--) {
      ...
      logger.info(`[Saga:${this.name}] ← Compensating: ${step.name}`, {
        compKey
      });
      ...
      for (let attempt = 0; attempt <= step.compensateMaxRetries; attempt++) {
        if (attempt > 0) {
          ...
          logger.info(`[Saga:${this.name}] Retrying compensation for ${step.name} (attempt ${attempt + 1})`, {
            compKey
          });
        }
        try {
          await withTimeout(step.compensate(context, result), step.compensateTimeoutMs, `${this.name}/${step.name} compensate`);
          succeeded = true;
          ...
          lastCompErr = compErr;
          logger.warn(`[Saga:${this.name}] ⚠ Compensation failed for ${step.name} (attempt ${attempt + 1}): ${compErr.message}`, {
            compKey
          });
        }
      }
  ...
  getStatus() {
    return {
      ...
      startedAt: this._startedAt ? new Date(this._startedAt).toISOString() : null,
      finishedAt: this._finishedAt ? new Date(this._finishedAt).toISOString() : null,
      durationMs: this._startedAt ? (this._finishedAt ?? Date.now()) - this._startedAt : null
    };
  }
}
