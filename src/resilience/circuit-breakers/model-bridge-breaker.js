...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/resilience/circuit-breakers/model-bridge-breaker.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/**
...
class ResponseCache {
  ...
  get(provider, model, messages) {
    ...
    return {
      ...entry.response,
      _fromCache: true,
      _cachedAt: new Date(entry.storedAt).toISOString()
    };
  }
...
class ModelBridgeBreaker extends EventEmitter {
    ...
    for (const prov of chain) {
      ...
      for (let attempt = 0; attempt <= this._maxRetries; attempt++) {
        try {
          ...
          const result = await modelBreaker.execute(() => provBreaker.execute(() => {
            const bridge = this._getBridge();
            return withTimeout(bridge.generate({
              provider: prov,
              model,
              messages,
              options
            }), timeoutMs, `${prov}/${model} generate`);
          }));
    ...
    for (const prov of chain) {
      ...
      for (let attempt = 0; attempt <= this._maxRetries; attempt++) {
        try {
          ...
          const result = await modelBreaker.execute(() => provBreaker.execute(() => {
            const bridge = this._getBridge();
            return withTimeout(bridge.embed({
              provider: prov,
              model,
              input
            }), timeoutMs, `${prov}/${model} embed`);
          }));
  ...
  async *streamGenerate(params) {
    ...
    for (const prov of chain) {
      ...
      try {
        ...
        const bridge = this._getBridge();
        const stream = await modelBreaker.execute(() => provBreaker.execute(async () => {
          // Just verify we can start the stream within timeout
          const controller = new AbortController();
          const timer = setTimeout(() => controller.abort(), timeoutMs);
          const gen = bridge.streamGenerate({
            provider: prov,
            model,
            messages,
            options,
  ...
  snapshot() {
    return {
      ...
      providerBreakers: Object.fromEntries(PROVIDER_CHAIN.map(p => {
        try {
          return [p, registry.get(p).snapshot()];
        } catch {
          return [p, {
