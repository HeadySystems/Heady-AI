...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/services/cross-device-fs.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/*
...
class CrossDeviceFS {
  ...
  async registerDevice(name, config) {
    ...
    if (global.eventBus) {
      global.eventBus.emit('device:registered', {
        name,
  ...
  async localFind(dirPath, pattern) {
    try {
      const output = execSync(`find ${JSON.stringify(dirPath)} -name ${JSON.stringify(pattern)} -maxdepth 5 2>/dev/null`, {
        encoding: 'utf8',
        timeout: 10000
      });
  ...
  async localGrep(dirPath, query) {
    try {
      const output = execSync(`grep -rl ${JSON.stringify(query)} ${JSON.stringify(dirPath)} --include='*' -m 20 2>/dev/null`, {
        encoding: 'utf8',
        timeout: 10000
      });
      return output.trim().split('\n').filter(Boolean);
  ...
  async _getSSHArgs(deviceName) {
    ...
    if (vault && vault.isUnlocked()) {
      ...
      if (key) {
        ...
        fs.writeFileSync(keyPath, key.value, {
          mode: 0o600
        });
      }
    }
    const args = ['-o', 'StrictHostKeyChecking=no', '-o', 'ConnectTimeout=10', '-p', String(device.port)];
  ...
  async remoteExec(deviceName, command) {
    const {
      args,
      keyPath
    } = await this._getSSHArgs(deviceName);
    try {
  ...
  async remoteWrite(deviceName, filePath, content) {
    ...
    try {
      execSync(`echo ${JSON.stringify(content)} | ssh ${args.join(' ')} "cat > ${filePath}"`, {
        encoding: 'utf8',
        timeout: PHI_TIMING.CYCLE
      });
      return {
  ...
  async syncToDevice(deviceName, localPath, remotePath) {
    ...
    try {
      const output = execSync(`rsync -avz -e "${sshCmd}" ${JSON.stringify(localPath)} ${device.user}@${device.hostname}:${JSON.stringify(remotePath)}`, {
        encoding: 'utf8',
        timeout: 120000
      });
      return {
  ...
  async syncFromDevice(deviceName, remotePath, localPath) {
    ...
    const sshCmd = `ssh ${args.slice(0, -1).join(' ')}`;
    try {
      const output = execSync(`rsync -avz -e "${sshCmd}" ${device.user}@${device.hostname}:${JSON.stringify(remotePath)} ${JSON.stringify(localPath)}`, {
        encoding: 'utf8',
        timeout: 120000
      });
      return {
...
function registerCrossDeviceFSRoutes(app) {
  ...
  app.post('/api/fs/sync', async (req, res) => {
    try {
      ...
      const result = direction === 'push' ? await crossDeviceFS.syncToDevice(device, localPath, remotePath) : await crossDeviceFS.syncFromDevice(device, remotePath, localPath);
      res.json({
        ok: true,
        ...result
      });
