...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/services/service-manager.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/*
...
const {
  getHeadySimsService
} = require('./HeadySims-service');
const {
  getHeadyBattleService
} = require('./HeadyBattle-service');
const {
  getArenaModeService
} = require('./arena-mode-service');
const {
  getBranchAutomationService
} = require('./branch-automation-service');
const {
  getErrorSentinel
} = require('./error-sentinel-service');
  ...
  monitorServices() {
    ...
    for (const [name, config] of this.services) {
      ...
      if (status.state === 'running' && config.instance) {
        ...
        if (metrics && Date.now() - metrics.timestamp > PHI_TIMING.CYCLE) {
          // 30 seconds
      ...
      try {
        ...
        if (serviceStatus) {
          ...
          if (healthScore < 0.7) {
            ...
            this.emit('service_health_degraded', {
              name,
              healthScore,
              serviceStatus
            });
