...
/**
 * Run `fn(client)` inside a single transaction. Commits on success, rolls back
 * on throw. This is the atomicity primitive the transactional outbox needs: a
 * business write and its event-log append commit together or not at all.
...
async function transaction(fn) {
    ...
    try {
        ...
        _lastError = err.message;
        logger.error("[neon-db] Transaction rolled back", { error: err.message });
        return { ok: false, error: err.message };
    } finally {
        client.release();
    }
...
/** Expose the raw pool for advanced callers (LISTEN/NOTIFY, streaming replication). */
function getPool() {
    return _pool;
}
