...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/services/projection-dispatcher.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/**
...
const LAYERS = {
  ...
  "cloud-sys": {
    name: "Cloud HeadySystems",
    endpoint: "https://headyme.com"
  },
  "cloud-conn": {
    name: "Cloud HeadyConnection",
    endpoint: "https://headyme.com"
  },
  "hf-liquid": {
    name: "HF Space Liquid Node",
    endpoint: "https://headyme-heady-hf-liquid-node.hf.space"
  },
  "hybrid": {
    name: "Hybrid",
    endpoint: "https://headyme.com"
...
class ProjectionDispatcher extends EventEmitter {
  ...
  boot(opts) {
    ...
    try {
      ...
      if (eventBus) eventBus.emit('bee_swarm:discovered', {
        count: beeCount,
        highPriority
      });
    } catch (err) {
    ...
    try {
      ...
      } = require("../services/projection-service");
    ...
    try {
      const {
        VectorBridge
      } = require("../core/vector-bridge");
      this.vectorBridge = new VectorBridge({
        serviceId: 'projection-dispatcher'
      });
    ...
    try {
      ...
      this.swarmIgnition.igniteSwarm({
        enablePruner: true,
        enableTester: true,
        enableEmbedder: true
      });
  ...
  registerRoutes(app) {
    ...
    if (this.beeTemplateRegistry) {
      const {
        registerRoutes: registerTemplateRoutes
      } = this.beeTemplateRegistry;
    ...
    if (this.buddySystem) {
      const {
        registerRoutes: registerBuddyRoutes
      } = this.buddySystem;
    ...
    app.get("/api/layer", (req, res) => {
      res.json({
        ...
        endpoint: LAYERS[this.activeLayer]?.endpoint || "",
        ts: new Date().toISOString()
      });
    });
    app.post("/api/layer/switch", (req, res) => {
      const newLayer = req.body.layer;
      if (!LAYERS[newLayer]) return res.status(400).json({
        error: "Invalid layer"
      });
    ...
    app.get("/api/edge/status", async (req, res) => {
      try {
        const [healthRes, detRes] = await Promise.allSettled([fetch(`${EDGE_PROXY_URL}/v1/health`, {
          signal: AbortSignal.timeout(3000)
        }), fetch(`${EDGE_PROXY_URL}/v1/determinism`, {
          signal: AbortSignal.timeout(3000)
        })]);
        const health = healthRes.status === 'fulfilled' ? await healthRes.value.json() : {
          error: 'unreachable'
        };
        const determinism = detRes.status === 'fulfilled' ? await detRes.value.json() : {
          error: 'unreachable'
        };
        res.json({
          ok: true,
