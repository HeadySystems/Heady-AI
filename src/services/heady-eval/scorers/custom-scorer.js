...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/services/heady-eval/scorers/custom-scorer.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
'use strict';
...
class CustomScorer extends BaseScorer {
  ...
  async score(example, ctx) {
    ...
    const prompt = this._compiledPrompt.replace('{question}', question).replace('{answer}', answer).replace('{reference_block}', reference ? `## Reference Answer\n${reference}` : '');
  ...
  _buildPrompt(def) {
    const rubricText = Object.entries(def.rubric || {}).sort(([a], [b]) => Number(b) - Number(a)).map(([score, text]) => `**Score ${score}:** ${text}`).join('\n\n');
    const dimensions = def.dimensions || [];
    const dimPromptsBlock = dimensions.length && def.dimensionPrompts ? `## Sub-dimension Instructions\n${dimensions.map(d => `- **${d}**: ${def.dimensionPrompts[d] || `Score the response's ${d}`}`).join('\n')}` : '';
    const dimScores = dimensions.map(d => `"${d}": <1-5>,`).join('\n  ');
    return CUSTOM_PROMPT_TEMPLATE.replace('{dimension_name}', def.name).replace('{rubric_text}', rubricText || 'Score 1–5 from worst to best.').replace('{dimension_prompts_block}', dimPromptsBlock).replace('{dimension_scores}', dimScores);
  }
  _parseJSON(text) {
    try {
