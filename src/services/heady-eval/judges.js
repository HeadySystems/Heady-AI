...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/services/heady-eval/judges.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
'use strict';
      ...
      res.on('end', () => {
        if (res.statusCode >= 200 && res.statusCode < 300) {
          try {
            ...
            reject(new Error(`Invalid JSON from Heady™Infer: ${data.slice(0, 200)}`));
          }
        } else {
  ...
  async complete(opts) {
    ...
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        const response = await makeRequest(`${this.inferUrl}/v1/complete`, body, this.timeout);
...
class MultiJudge {
   ...
   */
  async complete(opts) {
    const results = await Promise.allSettled(this.judges.map(j => j.complete(opts)));
    ...
    for (const {
      ...
      if (goldScore !== undefined && result.score !== null) {
        results.push({
          ...
          delta: Math.abs(result.score - goldScore)
        });
      }
    }
...
async function selfConsistencyCheck(judgeClient, scorer, example, threshold = config.selfConsistencyThreshold) {
  ...
  const [r1, r2] = await Promise.all([scorer.evaluate(example, ctx), scorer.evaluate(example, ctx)]);
