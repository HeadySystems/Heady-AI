...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/services/scheduler-service/src/scheduler.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
'use strict';
...
function nextCronMatch(cronExpr, after = new Date()) {
  ...
  for (let i = 0; i < limit; i++) {
    ...
    if (matches(minExpr, min, 59) && matches(hourExpr, hour, 23) && matches(domExpr, dom, 31) && matches(monExpr, mon, 12) && matches(dowExpr, dow, 6)) {
      return candidate;
    }
  ...
  _scheduleNext(jobId, delayMs) {
    ...
    if (state.consecutiveFailures >= CIRCUIT_BREAKER_THRESHOLD) {
      if (state.status !== 'circuit-open' && state.status !== 'circuit-half-open') {
        ...
        this._log.warn('Circuit breaker OPEN', {
          jobId,
        ...
        const timer = setTimeout(() => {
          ...
          this._log.info('Circuit breaker HALF-OPEN, probing', {
            jobId
          });
  ...
  async _executeJob(jobId, retryAttempt = 0) {
    ...
    try {
      ...
      if (retryAttempt < job.maxRetries) {
        ...
        this._log.warn('Job exhausted retries', {
          jobId,
