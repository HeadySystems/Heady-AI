...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/services/unified-enterprise-autonomy.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/*
...
} = require('../bees/input-task-extractor');
...
const STALE_FILE_PATTERNS = ['.bak', '.tmp', '.temp', '.old', '.orig', '.pid', '.log', '.jsonl', '.backup', '.bak.', '.dump', '.sql', '.sqlite', '.db'];
// Scan EVERYWHERE stale files can accumulate — not just 4 dirs
const STALE_SCAN_DIRECTORIES = ['data', 'logs', 'tmp', '.cache', '_archive',
// 1.1GB / 44k files — must be scanned
'public',
// stale build artifacts
'scripts', 'build', 'dist', 'coverage' // build artifacts that slip through
];
const STALE_MIN_AGE_HOURS = 24;
const ORPHAN_FILE_PATTERNS = ['server.pid', '.autoflow.pid', '.heady-ai.pid', '.headybuddy.pid', 'heady-manager.pid', 'python-worker.pid', 'admin-ui.pid', 'npm-debug.log', 'test.txt'];
...
function rankWorkersForQueue(queue, queueWeight, workers, queuePressure = {}) {
  return workers.filter(worker => (worker.queues || []).includes(queue)).map(worker => {
...
function isLikelyStaleProjectionFile(relPath, ageHours) {
  ...
  const staleByAgeAndLocation = ageHours >= STALE_MIN_AGE_HOURS && (lower.startsWith('data/') || lower.startsWith('tmp/') || lower.startsWith('logs/') || lower.startsWith('.cache/') || lower.startsWith('_archive/') || lower.startsWith('build/') || lower.startsWith('dist/') || lower.startsWith('coverage/'));
...
class UnifiedEnterpriseAutonomyService {
  ...
  buildEnterpriseImprovementBacklog(architectureText = '') {
    ...
    const baseline = [{
      text: 'Purge credential-bearing files from git history using git filter-repo/BFG and rotate exposed secrets immediately',
      enterpriseTrack: 'security-hardening',
      impact: 'high',
      source: 'baseline',
      category: 'security',
      priority: 1.0
    }, {
      text: 'Consolidate duplicated HCFullPipeline PowerShell scripts into one parameterized deterministic build orchestrator',
      enterpriseTrack: 'delivery-automation',
      impact: 'high',
      source: 'baseline',
      category: 'dev',
      priority: 0.9
    }, {
      text: 'Standardize health endpoints for all services and wire edge circuit-breaker checks to those paths',
      enterpriseTrack: 'observability',
      impact: 'high',
      source: 'baseline',
      category: 'ops',
      priority: 0.9
    }, {
      text: 'Enforce structured pino logging across routes and orchestration to support low-latency incident triage',
      enterpriseTrack: 'observability',
      impact: 'medium',
      source: 'baseline',
      category: 'ops',
      ...
      text: 'Continuously embed user actions, analyst decisions, system actions, and environmental snapshots into vector memory with freshness SLO monitoring',
      enterpriseTrack: 'platform-foundation',
      impact: 'high',
      source: 'baseline',
      category: 'ops',
      priority: 1.0
    }, {
      text: 'Expose context coverage endpoints and fail-safe keep-alive embedding so autonomy always has current project context',
      enterpriseTrack: 'reliability-performance',
      impact: 'high',
      source: 'baseline',
      category: 'ops',
      priority: 0.95
    }, {
      text: 'Continuously optimize liquid template projections from 3D vector space and deprecate stale projection targets',
      enterpriseTrack: 'experience-delivery',
      impact: 'high',
      source: 'baseline',
      category: 'dev',
      priority: 0.95
    }, {
      text: 'Generate and enforce safe cleanup plans for stale local generated artifacts and unused projection surfaces',
      enterpriseTrack: 'reliability-performance',
      impact: 'medium',
      source: 'baseline',
      category: 'ops',
      ...
      text: 'Implement cross-device widget task synchronization with persistent personal storage and 3D vector workspace parity checks',
      enterpriseTrack: 'platform-foundation',
      impact: 'high',
      source: 'baseline',
      category: 'ops',
      ...
      text: 'Instrument auth and onboarding stages with deterministic completion telemetry and provider-level observability',
      enterpriseTrack: 'experience-delivery',
      impact: 'high',
      source: 'baseline',
      category: 'dev',
      ...
      text: 'Provision dedicated full-throttle admin trigger lane with GodBee resource profile and recursive AST auto-correction schema',
      enterpriseTrack: 'delivery-automation',
      impact: 'high',
      source: 'baseline',
      category: 'dev',
      ...
      text: 'Run autonomous heartbeat swarms for hourly TesterBee and nightly PrunerBee with safe cleanup enforcement',
      enterpriseTrack: 'reliability-performance',
      impact: 'high',
      source: 'baseline',
      category: 'ops',
    ...
    const byTrack = tasks.reduce((acc, task) => {
      acc[task.enterpriseTrack || 'platform-foundation'] = (acc[task.enterpriseTrack || 'platform-foundation'] || 0) + 1;
      return acc;
    }, {});
    return {
      generatedAt: new Date().toISOString(),
  ...
  getDeviceSyncFabricStatus() {
    ...
    return {
      ...
      authOnboarding,
      deterministicReceipt: createDeterministicReceipt({
        deviceSyncHealthy: deviceSync.healthy,
        authStageCount: authOnboarding.stageCount,
        pendingTasks: deviceSync.pendingTasks
      })
    };
  }
  getLivingSystemStatus() {
    ...
    return {
      ...
      deterministicReceipt: createDeterministicReceipt({
        vitalityScore: alive.vitalityScore,
        healthy: alive.healthy,
        selfHealActions: selfHealPlan.actionCount
      })
    };
  }
  buildLiquidArchitectureDirective() {
    ...
    return {
      ...
      directives: ['Treat interfaces and services as a single liquid runtime graph.', 'Inject templates from vector-state into desired projections at request time.', 'Track projection reads/writes to eliminate stale and unused projection surfaces.', 'Use cleanup plans in safe mode before any destructive operations.'],
      projectionOptimization: optimization,
      cleanupPlan: cleanup,
      deterministicReceipt: createDeterministicReceipt({
        optimization,
        cleanupCount: cleanup.candidates.length
      })
    };
  }
  ...
  buildAbletonBridgeStatus() {
    ...
    return {
      ...
      deterministicReceipt: createDeterministicReceipt({
        manufacturerId: ableton.manufacturer_id ?? 125,
        commandIds: (ableton.commands || []).map(c => c.id)
      })
    };
  }
  ...
  buildProjectionCleanupPlan() {
    ...
    try {
      ...
      for (const entry of rootEntries) {
        ...
        let stat;
        try {
          stat = fs.statSync(path.join(ROOT, relPath));
        } catch {
          continue;
        }
        const ageHours = (now - stat.mtimeMs) / (1000 * 60 * 60);
  ...
  applyProjectionCleanup({
    includeTracked = false,
    limit = 50
  } = {}) {
    const plan = this.buildProjectionCleanupPlan();
    const safeLimit = Math.max(1, Math.min(1000, Number(limit) || 50));
    const selected = plan.candidates.filter(candidate => includeTracked || !candidate.tracked).slice(0, safeLimit);
    const removed = [];
    const errors = [];
    let freedBytes = 0;
  ...
  getSourceOfTruthStatus() {
    ...
    const repoPolicyAligned = policy.provider === 'github' ? Boolean(sourceOfTruth && sourceOfTruth.includes('github.com')) : Boolean(sourceOfTruth);
    return {
      branch,
      commit,
      sourceOfTruth,
  ...
  validateOnboardingAndAuthFlow() {
    ...
    const requiredStages = ['auth-provider-select', 'permissions-grant', 'username-provision', 'one-click-install'];
  ...
  buildAlternateParadigmDirectives() {
    ...
    return {
      ...
      directives: ['Treat vector memory as canonical runtime state.', 'Generate device UI as template injections from 3D workspace projections.', 'Route user, analyst, system, and environment signals continuously into autonomous embedder.', 'Use projection cleanup plan continuously to remove stale local artifacts and runtime noise.', 'Coordinate headybees/headyswarms via deterministic receipts and health-gated orchestration.'],
  ...
  runSelfHealingCycle({
    ...
    const shouldCleanup = applyCleanup === 'auto' ? cleanupPlan.candidateCount > 0 : applyCleanup === true;
    let cleanup = {
      ok: true,
      selected: 0,
      removed: [],
    ...
    if (shouldCleanup) {
      cleanup = this.applyProjectionCleanup({
        includeTracked: false,
        limit: cleanupLimit
      });
...
function registerUnifiedEnterpriseAutonomyRoutes(app, service = new UnifiedEnterpriseAutonomyService()) {
  ...
  app.get('/api/unified-autonomy/nodes', (_req, res) => {
    res.json({
      ok: true,
      nodes: service.getNodeResponsibilities()
    });
  });
  app.get('/api/unified-autonomy/embedding-plan', (_req, res) => {
    res.json({
      ok: true,
      embeddingPlan: service.buildEmbeddingPlan()
    });
  });
  ...
  app.get('/api/unified-autonomy/platform-blueprint', (_req, res) => {
    res.json({
      ok: true,
      blueprint: service.buildDeveloperPlatformBlueprint()
    });
  });
  app.get('/api/unified-autonomy/onboarding-contract', (_req, res) => {
    res.json({
      ok: true,
      onboarding: service.buildOnboardingContract()
    });
  });
  ...
  app.post('/api/unified-autonomy/enterprise-backlog', (req, res) => {
    ...
    res.json({
      ok: true,
      backlog: service.buildEnterpriseImprovementBacklog(architectureText)
    });
  });
  app.get('/api/unified-autonomy/context-fabric', (_req, res) => {
    res.json({
      ok: true,
      contextFabric: service.getAlwaysOnContextStatus()
    });
  });
  app.get('/api/unified-autonomy/device-sync-fabric', (_req, res) => {
    res.json({
      ok: true,
      syncFabric: service.getDeviceSyncFabricStatus()
    });
  });
  app.get('/api/unified-autonomy/liquid-architecture', (_req, res) => {
    res.json({
      ok: true,
      liquidArchitecture: service.buildLiquidArchitectureDirective()
    });
  });
  app.get('/api/unified-autonomy/living-system', (_req, res) => {
    res.json({
      ok: true,
      livingSystem: service.getLivingSystemStatus()
    });
  });
  app.get('/api/unified-autonomy/god-mode-pipeline', (req, res) => {
    res.json({
      ok: true,
      godModePipeline: service.buildGodModePipelineStatus(req.query || {})
    });
  });
  app.get('/api/unified-autonomy/autonomous-heartbeat', (_req, res) => {
    res.json({
      ok: true,
      autonomousHeartbeat: service.buildAutonomousHeartbeatStatus()
    });
  });
  app.get('/api/unified-autonomy/ableton-bridge', (_req, res) => {
    res.json({
      ok: true,
      abletonBridge: service.buildAbletonBridgeStatus()
    });
  });
  logger.info('CONDUCTOR', '    → Endpoints: /api/unified-autonomy/health, /nodes, /embedding-plan, /dispatch, /system-projection, /source-of-truth, /projection-hygiene, /projection-cleanup/plan, /projection-cleanup/apply, /platform-blueprint, /onboarding-contract, /onboarding/validate, /directives/alternate-paradigm, /self-healing/run');
  return service;
}
module.exports = {
