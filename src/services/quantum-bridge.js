...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/services/quantum-bridge.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/*
...
const {
  HeadyWebSocket: WebSocket
} = require('../core/heady-server'); // HeadyServer built-in WS
...
class QuantumBridge {
  constructor(server, opts = {}) {
    ...
    server.on('upgrade', (request, socket, head) => {
      ...
      if (request.url.startsWith('/quantum-bridge/')) {
        ...
        const urlParams = new URL(request.url, process.env.HEADY_WS_BASE || "ws://0.0.0.0").searchParams;
        ...
        const isLocal = request.headers.host && (request.headers.host.startsWith("0.0.0.0") || request.headers.host.startsWith(process.env.REMOTE_HOST || '0.0.0.0'));
        if (!isLocal && !token) {
  ...
  _handleConnection(ws, sessionId) {
    ...
    ws.on('message', async data => {
      await telemetry.withSpan('quantum_bridge.message_receive', {
        sessionId
      }, async span => {
        try {
  ...
  _generateMultiModalVisuals(chaosFactor) {
    ...
    return {
      ...
      oscillationHz: 963 * (1 + chaosFactor * 0.1),
      // 963Hz baseline, varies with chaos
