...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/services/HeadyBattle-service.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/**
...
try {
  ({
    getAutoContext: _getAutoContext
  } = require('./heady-auto-context'));
} catch (_) {
  _getAutoContext = () => null;
}
class HeadyBattleService extends EventEmitter {
  ...
  async battle(prompt, opts = {}) {
    ...
    if (this._gateway) {
      ...
      const messages = [{
        role: 'system',
        content: 'Provide your best, most complete response. Be concise but thorough.'
      }, {
        role: 'user',
      ...
      const contestantResults = await Promise.allSettled(available.map(async providerKey => {
      ...
      const contestants = contestantResults.filter(r => r.status === 'fulfilled').map(r => r.value);
    ...
    return {
      ok: false,
      error: 'No InferenceGateway connected'
    };
  }
  ...
  async determinismTest(prompt, opts = {}) {
    ...
    const providers = opts.provider ? [opts.provider] : this._gateway.getAvailable();
    const report = {
    ...
    for (const provider of providers) {
      ...
      const determinismScore = successful.length > 0 ? 1 - (uniqueHashes.length - 1) / Math.max(1, successful.length - 1) : 0;
      ...
      const canonicalHash = Object.entries(hashCounts).sort((a, b) => b[1] - a[1])[0]?.[0];
      ...
      report.providers[provider] = {
        ...
        avgLatencyMs: successful.length > 0 ? Math.round(successful.reduce((s, o) => s + (o.latencyMs || 0), 0) / successful.length) : null,
    ...
    report.overallDeterminism = allScores.length > 0 ? parseFloat((allScores.reduce((s, v) => s + v, 0) / allScores.length).toFixed(4)) : 0;
