...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/services/notification-service/src/index.ts                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/**
...
import { PHI, PSI, FIB, type NotificationPayload, type NotificationHealthStatus, type NotificationChannel } from './types.js';
import { NotificationRouter, DigestAggregator, TemplateEngine, EmailHandler, WebhookHandler, InAppHandler } from './service.js';
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (path === '/api/notifications/send' && method === 'POST') {
      ...
      if (record.status === 'delivered') sentCount++;else failCount++;
    ...
    if (path === '/api/notifications/batch' && method === 'POST') {
      ...
      results.forEach(r => {
        if (r.status === 'delivered') sentCount++;else failCount++;
      });
      jsonRes(res, 200, {
    ...
    if (path === '/api/notifications/digest/flush' && method === 'POST') {
      ...
      jsonRes(res, batch ? 200 : 204, batch ? batch as unknown as Record<string, unknown> : {
        flushed: false
      });
      return;
    }
