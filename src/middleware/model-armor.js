...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/middleware/model-armor.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/*
...
const PII_PATTERNS = [{
  ...
  name: 'api_key',
  regex: /\b(sk-|pk_|api[_-]?key[=:]\s*)[a-zA-Z0-9_-]{20,}\b/gi,
...
const INJECTION_PATTERNS = [{
  ...
  name: 'role_escape',
  regex: /(?:you\s+are\s+now|act\s+as|pretend\s+(?:to\s+be|you're)|your\s+new\s+role)/gi
}, {
  name: 'code_injection',
  regex: /(?:eval|exec|system|__import__|os\.system|child_process|require\s*\(\s*['"](?:child_process|fs|net))/gi
}, {
  name: 'encoding_bypass',
  regex: /(?:base64|atob|btoa|hex|unicode_escape)\s*\(/gi
}, {
  name: 'delimiter_attack',
  regex: /(?:```system|<\|im_start\|>|<\|im_end\|>|\[INST\]|\[\/INST\]|<\/?s>)/gi
}, {
  name: 'data_exfil',
  regex: /(?:send|post|upload|exfiltrate|transmit)\s+(?:all|every|the)\s+(?:data|info|content|secrets)/gi
}];
...
class ModelArmor {
  ...
  getStats() {
    return {
      ...this._stats,
      blockedLogSize: this._blockedLog.length
    };
  }
  getBlockedLog() {
...
function createModelArmorMiddleware(opts = {}) {
  ...
  const middleware = (req, res, next) => {
    ...
    if (result.blocked) {
      logger.error?.(`[MODEL ARMOR] BLOCKED request to ${req.path}: ${result.injectionThreats.map(t => t.type).join(', ')}`) || logger.error(`[MODEL ARMOR] BLOCKED: ${req.path}`);
...
function registerModelArmorRoutes(app, armor) {
  app.get('/api/v2/security/armor/stats', (req, res) => res.json({
    ok: true,
    ...armor.getStats()
  }));
  app.get('/api/v2/security/armor/blocked', (req, res) => res.json({
    ok: true,
    entries: armor.getBlockedLog()
  }));
...
module.exports = {
  ModelArmor,
  createModelArmorMiddleware,
  registerModelArmorRoutes,
  PII_PATTERNS,
  INJECTION_PATTERNS
};