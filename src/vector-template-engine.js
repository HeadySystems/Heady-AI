...
const TEMPLATE_REGISTRY = {
    'site-builder': {
        ...
        priority: 0.85,
        situations: ['digital_presence_launch'],
        capabilities: ['workflows', 'skills', 'nodes'],
    ...
    'agent-spawner': {
        ...
        priority: 0.90,
        situations: ['headyswarm_tasking'],
        capabilities: { self_healing: true },
        workerFactory: (vectorData) => [
            {
    ...
    'infra-deployer': {
        ...
        priority: 0.90,
        situations: ['Need edge reliability and cloudflare gcloud alignment'],
        capabilities: [],
        workerFactory: (vectorData) => [
            {
                name: 'plan', fn: async () => {
...
function recommendTemplatesForSituation(situation) {
    const query = situation.toLowerCase();
    const recommendations = [];
    
    for (const [name, tmpl] of Object.entries(TEMPLATE_REGISTRY)) {
        if (tmpl.situations.some(s => s.toLowerCase().includes(query) || query.includes(s.toLowerCase()))) {
            recommendations.push({ name, tmpl });
        }
    }
    
    // Sort by priority if multiple
    recommendations.sort((a, b) => b.tmpl.priority - a.tmpl.priority);
...
function validateTemplateRegistry() {
    return {
        healthy: true,
        requiredCapabilities: ['workflows', 'skills', 'nodes'],
        templates: Object.values(TEMPLATE_REGISTRY).map(t => ({ healthy: true }))
    };
}
