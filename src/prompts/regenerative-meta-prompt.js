...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/prompts/regenerative-meta-prompt.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
'use strict';
...
class RegenerativePrompt {
  ...
  /** @param {function} fn */
  set onBootstrapStart(fn) {
    this._hooks.onBootstrapStart = fn;
  }
  /** @param {function} fn */
  set onBootstrapComplete(fn) {
    this._hooks.onBootstrapComplete = fn;
  }
  /** @param {function} fn */
  set onBootstrapFailed(fn) {
    this._hooks.onBootstrapFailed = fn;
  }
  ...
  async bootstrap(options = {}) {
    ...
    this._fireHook('onBootstrapStart', {
      name: this.name,
      version: this.version
    });
    ...
    for (const anchor of this.semanticAnchors) {
      const vector = anchor.vector instanceof Float32Array ? anchor.vector : _embedText(anchor.description, this.embeddingDimension);
      ...
      const qualityVec = new Float32Array(this.embeddingDimension).fill(Math.min(1, quality / Math.sqrt(this.embeddingDimension)));
      validationVectors.push(qualityVec);
    }
    logger.debug('Step 1: Anchors registered', {
      count: anchorRegistry.size
    });

    // ── Step 2: Load execution graph ─────────────────────────────────────
    const liveGraph = this._loadExecutionGraph(this.executionGraph);
    const graphVector = _embedText(JSON.stringify(Object.keys(liveGraph)), this.embeddingDimension);
      ...
      const toolChecks = this.toolRegistrations.map(async tool => {
        try {
          const {
            reachable,
            latencyMs
          } = await _probeEndpoint(tool.endpoint, timeout);
    ...
    if (!options.skipPrereqChecks) {
      const prereqChecks = this.systemPrerequisites.map(async prereq => {
        try {
          const {
            reachable
          } = await _probeEndpoint(prereq.healthEndpoint, timeout);
          ...
          logger.warn('Prereq check error', {
            service: prereq.service,
            err: err.message
          });
    ...
    if (validationVectors.length > 0) {
      ...
      const failureRatio = failedPrerequisites.length / Math.max(1, this.toolRegistrations.length + this.systemPrerequisites.length);
      this._readinessScale.adjust({
        cpuPressure: failureRatio
      });
    }
  ...
  _loadExecutionGraph(rawGraph) {
    ...
    for (const [key, node] of Object.entries(rawGraph)) {
      live[key] = {
        ...
        _live: true,
        _loadedAt: Date.now(),
        _vector: _embedText(`${key} ${JSON.stringify(node)}`.slice(0, 256), this.embeddingDimension)
      };
    }
    return live;
  }
  ...
  static deserialize(json) {
    ...
    logger.debug('RegenerativePrompt deserialized', {
      name: prompt.name
    });
    return prompt;
  }
  ...
  merge(otherPrompt) {
    const merged = new RegenerativePrompt({
      ...
      targetNode: this.targetNode === otherPrompt.targetNode ? this.targetNode : '*',
      embeddingDimension: this.embeddingDimension
    });
    ...
    for (const a of [...this.semanticAnchors, ...otherPrompt.semanticAnchors]) {
      const vecA = a.vector instanceof Float32Array ? a.vector : _embedText(a.description, this.embeddingDimension);
  ...
  diff(otherPrompt) {
    ...
    for (const id of [...setA.keys()].filter(id => setB.has(id))) {
      const vecA = setA.get(id).vector instanceof Float32Array ? setA.get(id).vector : _embedText(setA.get(id).description, this.embeddingDimension);
      const vecB = setB.get(id).vector instanceof Float32Array ? setB.get(id).vector : _embedText(setB.get(id).description, this.embeddingDimension);
      const sim = CSL.cosine_similarity(vecA, vecB);
      if (sim < READINESS_THRESHOLD) {
        anchors.changed.push({
          id,
          similarity: sim
        });
      }
