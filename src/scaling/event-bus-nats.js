...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/scaling/event-bus-nats.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/**
...
const EVENT_SUBJECTS = {
  ...
  'heady.security.alert': {
    retention: 'limits',
    maxAge: FIB[14] * 3600,
    maxMsgs: FIB[16]
  },
  'heady.security.violation': {
    retention: 'limits',
    maxAge: FIB[14] * 3600,
    maxMsgs: FIB[16]
  },
  'heady.health.heartbeat': {
    retention: 'limits',
    maxAge: FIB[9] * 3600,
    maxMsgs: FIB[14]
  },
  ...
  'heady.deploy.started': {
    retention: 'limits',
    maxAge: FIB[11] * 3600,
    maxMsgs: FIB[14]
  },
  'heady.deploy.completed': {
    retention: 'limits',
    maxAge: FIB[11] * 3600,
    maxMsgs: FIB[14]
  },
...
class MessageStore {
  ...
  getFromSequence(startSeq, limit = FIB[8]) {
    return this.messages.filter(m => m.sequence >= startSeq).slice(0, limit);
  }
  stats() {
    return {
...
class EventBusNATS {
  constructor(config = {}) {
    this.url = config.url ?? "nats://0.0.0.0:4222";
  ...
  subscribe(consumerName, subject, handler) {
    ...
    if (!consumer) return {
      error: `Consumer not found: ${consumerName}`
    };
    consumer.subscribe(subject, handler);
    this._audit('subscribe', {
