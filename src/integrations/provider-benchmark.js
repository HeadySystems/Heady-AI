...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/integrations/provider-benchmark.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/*
...
function audit(entry) {
  try {
    fs.appendFileSync(BENCH_AUDIT, JSON.stringify({
      ...entry,
...
async function benchmarkHF() {
  ...
  try {
    ...
    await client.chatCompletion({
      ...
      messages: [{
        role: "user",
        content: "Say 'benchmark ok' in 5 words or less"
      }],
      max_tokens: 20
    });
...
async function benchmarkHeadyPythia() {
  ...
  try {
    ...
    await ai.models.generateContent({
      ...
      contents: "Say 'benchmark ok' in 5 words or less"
    });
    const chatLatency = Date.now() - chatStart;
    return {
      provider: "headypythia",
...
async function runFullBenchmark(vectorMem) {
  ...
  const results = await Promise.allSettled([benchmarkLocal(), benchmarkEdge(), benchmarkHF(), benchmarkHeadyPythia(), benchmarkHeadyJules()]);
  const benchmarks = results.map(r => r.status === "fulfilled" ? r.value : {
    ok: false,
  ...
  benchResults = {
    ...benchResults,
    latest: report,
    history: [...(benchResults.history || []).slice(-50), report]
  };
  saveBenchmarks();
  audit({
...
function registerRoutes(app, vectorMem) {
  app.get("/api/benchmark/latest", (req, res) => {
    res.json({
      ok: true,
      ...(benchResults.latest || {
        message: "No benchmarks yet. POST /api/benchmark/run to start."
      })
    });
  });
  ...
  app.get("/api/benchmark/history", (req, res) => {
    res.json({
      ok: true,
      history: benchResults.history || [],
      total: (benchResults.history || []).length
    });
  });
