...
const OAUTH2_PROVIDERS = Object.freeze({
  google: {
    tokenUrl: 'https://oauth2.googleapis.com/token',
    userInfoUrl: 'https://www.googleapis.com/oauth2/v3/userinfo',
    idField: 'sub', emailField: 'email', nameField: 'name',
  },
  github: {
    tokenUrl: 'https://github.com/login/oauth/access_token',
    userInfoUrl: 'https://api.github.com/user',
    idField: 'id', emailField: 'email', nameField: 'name',
  },
  microsoft: {
    tokenUrl: 'https://login.microsoftonline.com/common/oauth2/v2.0/token',
    userInfoUrl: 'https://graph.microsoft.com/v1.0/me',
    idField: 'id', emailField: 'mail', nameField: 'displayName',
  },
});
...
class AuthManager {
  ...
  async handleOAuth2Callback(opts = {}) {
    ...
    if (!providerConfig) throw new Error(`Unsupported OAuth2 provider: ${provider}`);
    ...
    const tokenRes = await fetch(providerConfig.tokenUrl, {
      ...
      body: new URLSearchParams({
        ...
        code,
        client_id: process.env[`OAUTH2_${provider.toUpperCase()}_CLIENT_ID`] || '',
        client_secret: process.env[`OAUTH2_${provider.toUpperCase()}_CLIENT_SECRET`] || '',
        redirect_uri: storedState.redirectUri || `${this._opts.baseUrl || 'https://auth.headysystems.com'}/callback`,
      }).toString(),
    });

    if (!tokenRes.ok) {
      const errBody = await tokenRes.text();
      logger.warn('[AuthManager] OAuth2 token exchange failed', { provider, status: tokenRes.status });
    ...
    // Map provider user info to Heady user
    const oauthUser = {
      id: `oauth2:${provider}:${userInfo[providerConfig.idField] || _hashSecret(code).slice(0, 16)}`,
      email: userInfo.email || userInfo[providerConfig.emailField] || `unknown@${provider}`,
      name: userInfo.name || userInfo[providerConfig.nameField] || 'OAuth2 User',
      role: ROLES.USER,
      meta: { provider, providerUserId: userInfo[providerConfig.idField], avatar: userInfo.avatar_url || userInfo.picture },
    };

    logger.info('[AuthManager] OAuth2 callback complete', { provider, userId: oauthUser.id });
