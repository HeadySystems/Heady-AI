...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/context-weaver/relevance-scorer.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
'use strict';
...
function scoreNodes(intentEmbedding, candidateNodes, distanceMap, options = {}) {
  ...
  for (const node of candidateNodes) {
    ...
    const semanticScore = intentEmbedding && node.embedding ? cosineSimilarity(intentEmbedding, node.embedding) : 0;
    ...
    if (node.updatedAt) {
      const updatedTime = typeof node.updatedAt === 'string' ? new Date(node.updatedAt).getTime() : node.updatedAt;
      const elapsed = Math.max(0, now - updatedTime);
      recencyScore = Math.exp(-lambda * elapsed);
    }

    // ── Composite Si ───────────────────────────────────────
    const compositeScore = alpha * semanticScore + beta * proximityScore + gamma * recencyScore;
    scored.push({
      ...node,
...
function scoreByStructure(candidateNodes, distanceMap, options = {}) {
  return scoreNodes(null, candidateNodes, distanceMap, {
    ...
    beta: 0.65,
    // Heavily weight graph proximity
