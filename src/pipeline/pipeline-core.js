...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/pipeline/pipeline-core.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
'use strict';
...
// ─ Stage 0: CHANNEL_ENTRY ──────────────────────────────────────────────────
{
  ...
  // 0.500 — low bar, just needs to arrive
  ...
  async execute(context) {
    ...
    return stageResult(0, 'Channel Entry', 'ok', {
      requestId,
      channel,
      entryAt: context.entryAt
    }, start);
  },
...
// ─ Stage 2: INTAKE ────────────────────────────────────────────────────────
{
  ...
  async execute(context) {
    ...
    if (!normalized.type) {
      return stageResult(2, 'Intake', 'warn', {
        ...normalized,
        warning: 'task.type missing'
      }, start);
    }
  ...
  async execute(context) {
    ...
    for (const [d, keywords] of Object.entries(domainMap)) {
      if (keywords.some(k => typeStr.includes(k))) {
        domain = d;
        pool = d === 'code' || d === 'security' ? 'HOT' : 'WARM';
        break;
      }
    }
  ...
  async execute(context) {
    ...
    if (task.payload && Array.isArray(task.payload.steps)) {
      for (const step of task.payload.steps.slice(0, maxSubtasks)) {
        subtasks.push({
          id: `${context.requestId}_sub_${subtasks.length}`,
          step,
          status: 'pending'
        });
      }
      ...
      subtasks.push({
        id: `${context.requestId}_sub_0`,
        step: task.type || 'execute',
        status: 'pending'
      });
    }
    const decomposition = {
  ...
  async execute(context) {
    ...
    for (let i = 0; i < simCount; i++) {
      ...
      outcomes.push({
        sim: i,
        success,
        confidence: success ? 1 - PSI * PSI : PSI * PSI
      });
    }
...
// ─ Stage 9: ARENA ─────────────────────────────────────────────────────────
{
  ...
  async execute(context) {
    ...
    const config_a = {
      id: 'A',
      node: orch.primaryNode,
      strategy: 'standard',
      score: null
    };
    const config_b = {
      id: 'B',
      node: (orch.fallbackNodes || ['HeadyBuddy'])[0],
      strategy: 'phi_augmented',
      score: null
    };
...
// ─ Stage 10: JUDGE ────────────────────────────────────────────────────────
{
  ...
  parallel: false,
  required: false,
  gate: CSL_THRESHOLDS.HIGH,
  // 0.882 — high bar for judgement
...
// ─ Stage 12: EXECUTE ──────────────────────────────────────────────────────
{
  ...
  name: 'Execute',
  order: 12,
  timeout: phiTimeout(7),
  // PHI_TIMING.CYCLEms — execution can be longer
  parallel: false,
  required: true,
  gate: CSL_THRESHOLDS.MEDIUM,
  // 0.809
...
// ─ Stage 13: VERIFY ───────────────────────────────────────────────────────
{
  ...
  required: true,
  gate: CSL_THRESHOLDS.HIGH,
  // 0.882 — high bar for verification
...
// ─ Stage 14: SELF_AWARENESS ───────────────────────────────────────────────
{
  ...
  async execute(context) {
    ...
    const selfState = {
      ...
      heapUsedMB: (mem.heapUsed / 1024 / 1024).toFixed(2),
      coherenceScore: (1 - PSI * PSI * PSI).toFixed(4),
      // 0.764
      identityStable: true,
...
// ─ Stage 17: OPTIMIZATION_OPS ─────────────────────────────────────────────
{
  ...
  async execute(context) {
    ...
    const defaultOpts = [{
      id: 'opt_cache_reuse',
      recommendation: 'cache_orchestration_plan',
      phiWeight: PSI,
      applied: true
    }, {
      id: 'opt_phi_batch',
      recommendation: 'phi_batch_embedding_calls',
      phiWeight: PSI * PSI,
      applied: true
    }];
    const optResult = {
...
// ─ Stage 18: CONTINUOUS_SEARCH ────────────────────────────────────────────
{
  ...
  async execute(context) {
    ...
    const searchSignals = [{
      source: 'HeadyResearch',
      freshness: (1 - PSI * PSI * PSI).toFixed(4),
      active: true
    }, {
      source: 'SOPHIA',
      freshness: PSI.toFixed(4),
      active: true
    }, {
      source: 'vector_index',
      freshness: (1 - PSI * PSI).toFixed(4),
      active: false
    }];
...
class HCFullPipeline {
      ...
      while (attempt <= MAX_RETRIES && !succeeded) {
        try {
          ...
          context.stageTrace.push({
            stage: stageDef.id,
            order: idx,
            at: stageResult_.timestamp,
            status: stageResult_.status
          });
          succeeded = true;
        } catch (err) {
          lastError = err;
          if (attempt < MAX_RETRIES) {
            ...
            if (stageDef.required) {
              ...
              return {
                ...
                context,
                durationMs,
                sla: this._sla(durationMs),
                error: `Required stage ${stageDef.id} failed: ${err.message}`
              };
            }
  ...
  _recordRun(success, durationMs, variant) {
    ...
    if (success) this._metrics.successfulRuns++;else this._metrics.failedRuns++;
    const sla = this._sla(durationMs);
