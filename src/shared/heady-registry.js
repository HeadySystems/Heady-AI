...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/shared/heady-registry.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/*
...
function audit(entry) {
  try {
    fs.appendFileSync(REG_AUDIT, JSON.stringify({
      ...entry,
...
function getAllWithConfidence() {
  ...
  for (const [key, entry] of Object.entries(registry.entries)) {
    ...
    result[key] = {
      ...entry,
      confidence: confidenceFromAge(age),
      ageMs: age
    };
  }
  return result;
...
async function incrementalScan() {
  ...
  for (const entry of stale) {
    // Dynamic: process ALL stale entries — no batch limit
...
async function fullPopulate() {
  ...
  const endpoints = ["pulse", "compute/dashboard", "orchestrator/agents", "orchestrator/nodes", "optimize/status", "optimize/skills", "optimize/connectors", "remote/stats", "corrections/model", "vector/stats", "mcp/tools", "battle/leaderboard", "creative/pipelines", "auth/sessions", "benchmark/latest"];
  for (const ep of endpoints) await scanEndpoint(ep, `${BASE}/api/${ep}`);
  const dataDir = path.join(__dirname, "..", "data");
  const modules = [["agent-orchestrator", path.join(__dirname, "agent-orchestrator.js")], ["remote-compute", path.join(__dirname, "remote-compute.js")], ["compute-dashboard", path.join(__dirname, "compute-dashboard.js")], ["sdk-services", path.join(__dirname, "sdk-services.js")], ["provider-benchmark", path.join(__dirname, "provider-benchmark.js")], ["self-optimizer", path.join(__dirname, "self-optimizer.js")], ["corrections", path.join(__dirname, "corrections.js")], ["vector-memory", path.join(__dirname, "vector-memory.js")], ["heady-registry", path.join(__dirname, "heady-registry.js")]];
  for (const [name, p] of modules) scanModule(name, p);
  const configs = [["heady-api-key", "HEADY_API_KEY"], ["hf-token", "HF_TOKEN"], ["hf-token-2", "HF_TOKEN_2"], ["google-api-key", "GOOGLE_API_KEY"], ["headypythia-key-heady", "HEADY_PYTHIA_KEY_HEADY"], ["headynexus-key", "HEADY_NEXUS_KEY"], ["headycompute-key", "HEADY_COMPUTE_KEY"], ["groq-key", "GROQ_API_KEY"], ["perplexity-key", "PERPLEXITY_API_KEY"], ["cloudflare-token", "CLOUDFLARE_API_TOKEN"]];
  for (const [name, key] of configs) scanConfig(name, key);
  const dataFiles = [["overnight-audit", path.join(dataDir, "overnight-audit.jsonl")], ["corrections-audit", path.join(dataDir, "corrections-audit.jsonl")], ["remote-dispatch-audit", path.join(dataDir, "remote-dispatch-audit.jsonl")], ["optimization-audit", path.join(dataDir, "optimization-audit.jsonl")], ["registry-audit", path.join(dataDir, "registry-audit.jsonl")], ["vector-memory", path.join(dataDir, "vector-memory.json")], ["behavior-patterns", path.join(dataDir, "behavior-patterns.json")], ["provider-benchmarks", path.join(dataDir, "provider-benchmarks.json")], ["optimization-state", path.join(dataDir, "optimization-state.json")]];
  for (const [name, p] of dataFiles) scanDataFile(name, p);
  save();
  audit({
    type: "full_populate",
    entries: Object.keys(registry.entries).length
  });
  return {
    entries: Object.keys(registry.entries).length
  };
...
function getSummary() {
  ...
  for (const e of Object.values(entries)) {
    ...
    if (e.confidence >= 0.75) byConfidence.high++;else if (e.confidence >= 0.5) byConfidence.medium++;else if (e.confidence >= 0.25) byConfidence.low++;else byConfidence.stale++;
  }
  return {
    totalEntries: count,
