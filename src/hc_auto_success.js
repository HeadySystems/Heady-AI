...
const TASK_CATALOG = [
    ...
    {
        id: "hive-021", name: "Dynamic LLM model discovery and benchmark optimization", cat: "hive-integration", pool: "warm", w: 4,
        desc: "Check provider API endpoints for newly updated or added models, run benchmark racing tests, and adjust system-wide default routing tiers."
    },
];
    ...
    async _performWork(task) {
        ...
        if (task.id === 'hive-021') {
            try {
                ...
                const benchmarker = require('./services/model-benchmarker');
                
                // 1. Scan provider endpoints
                const scannedModels = await registry.scan();
                
                // 2. Run benchmark suite against models
                await benchmarker.runSuite();
                
                // 3. Compile report at /home/headyme/workspace/artifacts/model_roster_report.md
                const roster = registry.getRoster();
                const reportPath = '/home/headyme/workspace/artifacts/model_roster_report.md';
                const reportDir = path.dirname(reportPath);
                if (!fs.existsSync(reportDir)) fs.mkdirSync(reportDir, { recursive: true });

                const modelsList = Object.values(roster.models);
                modelsList.sort((a, b) => (b.elo || 0) - (a.elo || 0));

                let md = `# Heady OS AI Model Roster Report\n\n`;
                md += `**Generated**: ${new Date().toISOString()}\n`;
                md += `**Last Discovery Scan**: ${roster.lastScan}\n\n`;
                md += `## Active Model Rankings (by ELO)\n\n`;
                md += `| Rank | Model ID | Provider | ELO | Tier | Max Context | Latency (Avg) | Status |\n`;
                md += `| --- | --- | --- | --- | --- | --- | --- | --- |\n`;
                
                modelsList.forEach((m, idx) => {
                    const latency = m.lastBenchmark ? `${m.lastBenchmark.avgLatencyMs}ms` : 'N/A';
                    md += `| ${idx + 1} | \`${m.id}\` | ${m.provider} | **${m.elo || 1000}** | ${m.tier} | ${m.contextWindow.toLocaleString()} | ${latency} | ${m.available ? '✅ Available' : '❌ Offline'} |\n`;
                });

                md += `\n## Default Routing Tier Adjustments\n\n`;
                md += `Based on the latest benchmarking and ELO ratings, the following configurations are active in the Inference Gateway:\n\n`;
                
                const providers = ['gemini', 'openai', 'groq', 'claude'];
                md += `| Provider | Fast Tier Model | Quality Tier Model | Default Model |\n`;
                md += `| --- | --- | --- | --- |\n`;
                
                providers.forEach(p => {
                    const pName = p === 'gemini' ? 'google' : p === 'claude' ? 'anthropic' : p;
                    const pModels = modelsList.filter(m => m.provider === pName && m.available);
                    if (pModels.length > 0) {
                        const fastModels = pModels.filter(m => m.tier === 'fast' || m.tier === 'credits' || m.tier === 'speed');
                        const qualityModels = pModels.filter(m => m.tier === 'quality');
                        const fast = fastModels[0]?.id || pModels[0]?.id;
                        const quality = qualityModels[0]?.id || pModels[0]?.id;
                        const def = pModels[0]?.id;
                        md += `| ${p.toUpperCase()} | \`${fast}\` | \`${quality}\` | \`${def}\` |\n`;
                    } else {
                        md += `| ${p.toUpperCase()} | *No active models* | *No active models* | *No active models* |\n`;
                    }
                });
                ...
                return {
                    finding: `Model awareness optimized. Discovered ${scannedModels.length} models, run benchmarks, generated report at ${reportPath}`,
                    adjusted: true
                };
            } catch (err) {
                logger.error('[AutoSuccessEngine] hive-021 execution failed', { error: err.message });
                throw err;
            }
        }
