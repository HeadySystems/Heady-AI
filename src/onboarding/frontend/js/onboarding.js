...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/onboarding/frontend/js/onboarding.js                                                    ║
// ║  LAYER: ui/frontend                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/**
...
/* ---- Constants ---- */
const STAGES = [
  { id: 'auth',        index: 0, label: 'Sign In' },
  { id: 'identity',    index: 1, label: 'Identity' },
  { id: 'email',       index: 2, label: 'Email' },
  { id: 'permissions', index: 3, label: 'Permissions' },
  { id: 'buddy',       index: 4, label: 'HeadyBuddy' },
  { id: 'complete',    index: 5, label: 'Complete' },
];
...
const NATIVE_PROVIDERS = new Map([
  ['google.com',    { class: 'GoogleAuthProvider' }],
  ['github.com',    { class: 'GithubAuthProvider' }],
  ['facebook.com',  { class: 'FacebookAuthProvider' }],
  ['twitter.com',   { class: 'TwitterAuthProvider' }],
  ['microsoft.com', { class: 'OAuthProvider', param: 'microsoft.com' }],
  ['apple.com',     { class: 'OAuthProvider', param: 'apple.com' }],
]);
...
export class OnboardingWizard {
  ...
  #initStage(index) {
    ...
    switch (STAGES[index].id) {
      case 'auth':        this.#initAuthStage(); break;
      case 'identity':    this.#initIdentityStage(); break;
      case 'email':       this.#initEmailStage(); break;
      case 'permissions': this.#initPermissionsStage(); break;
      case 'buddy':       this.#initBuddyStage(); break;
      case 'complete':    this.#initCompleteStage(); break;
    }
  }
  ...
  #populateSummary() {
    ...
    if (emailEl) {
      emailEl.textContent = this.#sessionData.emailChoice === 'inbox'
        ? `${this.#sessionData.username || 'user'}@headyme.com (Secure Inbox)`
        : `Forwarding to ${this.#sessionData.forwardEmail || '\u2014'}`;
    }

    const modeEl = panel.querySelector('#summary-mode');
    if (modeEl) {
      modeEl.textContent = this.#sessionData.permissionMode === 'hybrid'
        ? `Hybrid (${this.#sessionData.deviceName || 'device'})`
        : 'Cloud Only';
    }
    ...
    if (ifaceEl) {
      ...
      ifaceEl.innerHTML = ifaces.map(i =>
        `<span class="interface-tag">${this.#escapeHTML(i)}</span>`
      ).join('');
    }
  }
