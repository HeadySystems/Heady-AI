...
class KronosV2Agent {
  ...
  addEdge(fromId, toId, relation, value, validFrom = Date.now(), metadata = {}) {
    ...
    const edge = {
      ...
      weight: 1.0,
      // starts at full weight; decays over time
      metadata,
      invalidatedBy: null,
      createdAt: Date.now()
    };
  ...
  queryLatest(fromId, relation) {
    return this._edgesFrom(fromId).filter(e => e.relation === relation && e.validTo === null).sort((a, b) => b.validFrom - a.validFrom)[0] || null;
  }
  ...
  queryAsOf(fromId, relation, asOf) {
    return this._edgesFrom(fromId).filter(e => e.relation === relation && e.validFrom <= asOf && (e.validTo === null || e.validTo > asOf)).sort((a, b) => b.validFrom - a.validFrom)[0] || null;
  }
  ...
  queryRange(fromId, relation, t1, t2) {
    return this._edgesFrom(fromId).filter(e => e.relation === relation && e.validFrom >= t1 && e.validFrom <= t2).sort((a, b) => a.validFrom - b.validFrom);
  }
  ...
  changelog(fromId, relation, since = 0) {
    return this._edgesFrom(fromId).filter(e => e.relation === relation && e.createdAt >= since).sort((a, b) => a.validFrom - b.validFrom).map(e => ({
      value: e.value,
  ...
  neighborhood(nodeId, asOf = Date.now()) {
    return this._edgesFrom(nodeId).filter(e => e.validFrom <= asOf && (e.validTo === null || e.validTo > asOf)).map(e => ({
      relation: e.relation,
      targetId: e.toId,
      value: e.value,
      weight: e.weight,
  ...
  health() {
    return {
      ...
      status: 'healthy',
      activeTasks: this.tasks.size,
      graphNodes: this._nodes.size,
      graphEdges: this._edges.size,
      decayCycles: this._decayCycles,
      phi: PHI
    };
  }
