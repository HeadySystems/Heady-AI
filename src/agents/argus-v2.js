...
const DriftSignals = {
  ...
  // v2 additions
  TOKEN_BUDGET: 'token_budget_exhaustion',
  // LLM token budget approaching limit
...
class ArgusV2Agent {
  constructor(opts = {}) {
    ...
    this.driftThresholds = {
      ...
      [DriftSignals.TOKEN_BUDGET]: opts.tokenBudgetThreshold || 0.90,
      // 90% of budget
  ...
  recordAudit(event) {
    const prevHash = this.auditLog.length > 0 ? this.auditLog[this.auditLog.length - 1].hash : '0'.repeat(64);
    const entry = {
  ...
  setBaseline(signal, value) {
    this.driftBaselines.set(signal, value);
  }
  _checkSpanDrift(span, latencyMs, result) {
    ...
    if (latencyMs > latencyBase) signals.push({
      signal: DriftSignals.LATENCY,
      value: latencyMs,
      baseline: latencyBase
    });
    if (result.csl_score !== undefined && result.csl_score < this.driftThresholds[DriftSignals.AGENT_COHERENCE]) {
      signals.push({
        signal: DriftSignals.AGENT_COHERENCE,
        value: result.csl_score,
        threshold: this.driftThresholds[DriftSignals.AGENT_COHERENCE]
      });
    }
    if (signals.length) this.driftAlerts.push(...signals.map(s => ({
      ...s,
      timestamp: Date.now()
    })));
    return signals;
  ...
  _runDriftCheck() {
    ...
    if (errorRate > this.driftThresholds[DriftSignals.ERROR_RATE]) {
      this.driftAlerts.push({
        signal: DriftSignals.ERROR_RATE,
        value: errorRate,
        threshold: this.driftThresholds[DriftSignals.ERROR_RATE],
        timestamp: Date.now()
      });
    }
    ...
    if (avgLatency > this.driftThresholds[DriftSignals.LATENCY]) {
      this.driftAlerts.push({
        signal: DriftSignals.LATENCY,
        value: avgLatency,
        threshold: this.driftThresholds[DriftSignals.LATENCY],
        timestamp: Date.now()
      });
    }
  }
  getRecentDriftAlerts(limit = 13) {
  ...
  health() {
    ...
    return {
      agent: this.name,
      version: this.version,
      status: phiHealth >= PHI_HEALTH.NOMINAL ? 'healthy' : 'degraded',
      phiHealth,
      auditLength: this.auditLog.length,
      activeSpans: this._spans.size,
      wideEvents: this._wideEvents.length,
      genAiMetrics: this.getGenAiMetrics(),
      recentDrift: this.getRecentDriftAlerts(5)
    };
  }
  computePhiHealth() {
    const recent = this._wideEvents.slice(-89);
