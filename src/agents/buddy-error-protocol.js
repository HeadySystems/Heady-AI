...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/agents/buddy-error-protocol.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/*
...
function analyzeRootCause(snapshot, haltedState) {
  ...
  const rootCause = {
    ...
    semanticAnalysis: {
      ...
      // Environment has been virtualized
...
function updateBaseline(rule) {
  ...
  try {
    ...
    // Append the new rule
    const ruleEntry = [`### ${rule.id}`, `- **Constraint:** ${rule.constraint}`, `- **Classification:** ${rule.classification?.type || "unknown"}`, `- **Severity:** ${rule.severity}`, `- **Source:** \`${rule.failingModule?.file || "unknown"}\` → \`${rule.failingModule?.function || "unknown"}()\``, `- **Error Hash:** \`${rule.errorHash}\``, `- **Learned:** ${rule.createdAt}`, ``].join("\n");
    agentsMd += ruleEntry + "\n";
...
function classifyError(error) {
  ...
  if (name === "typeerror") return {
    type: "TYPE_ERROR",
    category: "logic",
    recoverable: true
  };
  if (name === "referenceerror") return {
  ...
  if (msg.includes("econnrefused") || msg.includes("enotfound")) return {
    type: "NETWORK_ERROR",
  ...
  if (msg.includes("budget") || msg.includes("exceeded")) return {
    type: "BUDGET_ERROR",
    category: "finops",
    recoverable: true
  };
  ...
  if (msg.includes("unauthorized") || msg.includes("forbidden")) return {
    type: "AUTH_ERROR",
    category: "security",
    recoverable: true
  };
  if (msg.includes("hallucination") || msg.includes("invalid output")) return {
    type: "HALLUCINATION",
    category: "ai_drift",
    recoverable: true
  };
