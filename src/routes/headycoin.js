...
const {
  ...
  TOKEN,
  MINING_REWARDS
} = require("../headycoin/headycoin-core");
...
router.get("/wallets", (req, res) => {
  ...
  const list = wallet.listWallets(type ? {
    type
  } : {});
  res.json({
    ok: true,
...
router.post("/transfer", (req, res) => {
  try {
    ...
    const tx = headyCoin.transfer(from, to, parseFloat(amount), {
      note
    });
    res.json({
      ok: true,
...
router.post("/burn", (req, res) => {
  try {
    ...
    if (!from || !amount) {
      return res.status(400).json({
        ok: false,
        error: "from and amount required"
      });
    }
    const tx = headyCoin.burn(from, parseFloat(amount), {
      reason
    });
    res.json({
      ok: true,
...
router.post("/stake", (req, res) => {
  try {
    ...
    if (!walletAddress || !amount) {
      return res.status(400).json({
        ok: false,
        error: "walletAddress and amount required"
      });
    }
...
router.post("/unstake", (req, res) => {
  try {
    ...
    if (!stakeId) return res.status(400).json({
      ok: false,
      error: "stakeId required"
    });
...
router.post("/merkle/verify", (req, res) => {
  try {
    ...
    if (!leaf || !proof || !root) {
      return res.status(400).json({
        ok: false,
        error: "leaf, proof, and root required"
      });
    }
...
router.post("/anchor", async (req, res) => {
  try {
    ...
    if (!treeData.root) {
      return res.json({
        ok: true,
        message: "No unanchored transactions"
      });
    }
    let evmTxHash;
    try {
      const {
        anchorToLedger
      } = require("../security/web3-ledger-anchor");
