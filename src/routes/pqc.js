...
import { headyPQC, PQC_CONFIG, getStatus } from '../security/pqc.js';
...
router.post('/sign', (req, res) => {
  ...
  if (!serviceId || !message) {
    return res.status(400).json({ ok: false, error: 'serviceId and message are required' });
  }
  try {
...
router.post('/encapsulate', (req, res) => {
  ...
  if (!recipientServiceId) {
    return res.status(400).json({ ok: false, error: 'recipientServiceId is required' });
  }
  try {
