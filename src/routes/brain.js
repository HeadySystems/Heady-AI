...
const {
  createProviders
} = require(path.join(__dirname, "..", "..", "heady-hive-sdk", "lib", "providers"));
let _sdkGateway = null;
function getSDKGateway(req) {
  if (!_sdkGateway) {
    _sdkGateway = new HeadyGateway({
      cacheTTL: 300000
    });
...
async function storeInMemory(content, metadata) {
  ...
  if (memoryWrapper && typeof memoryWrapper.ingestMemory === "function") {
    try {
      await memoryWrapper.ingestMemory({
        content,
        metadata
      });
      method = "vector-db";
      stored = `Stored in vector DB: "${contentSummary}..." (${fullLength} chars, type: ${metadata?.type || "unknown"})`;
      notStored = truncatedAmount > 0 ? `${truncatedAmount} chars of extended content trimmed from vector index` : "Full content stored — nothing omitted";
    } catch (err) {
...
async function chatViaOpenAI(message, system, temperature, max_tokens, history) {
  ...
  if (system) msgs.push({
    ...
    role: "system",
    content: "You are HeadyBrain, the AI reasoning engine of the Heady™ ecosystem. You are helpful, concise, and intelligent. When discussing Heady services, reference Brain, Battle, Creative, MCP, and the 155-task auto-success engine. Be conversational and warm."
  });
...
async function chatViaOllama(message, system, temperature, max_tokens, history) {
  ...
  return new Promise((resolve, reject) => {
    const req = http.request({
      hostname: process.env.HEADY_LOCAL_HOST || process.env.REMOTE_HOST || '0.0.0.0',
      ...
      res.on("end", () => {
        try {
          const parsed = JSON.parse(data);
          resolve({
            response: parsed.response || parsed.message?.content || data,
            model: "llama3.2"
          });
        } catch {
          resolve({
...
function filterResponse(text, options = {}) {
  ...
  if (options.scrubProviders !== false) {
    ...
    [/\bI(?:'m| am) (?:an? )?(?:AI (?:assistant|model|chatbot|language model) )?(?:made|created|developed|built|trained|designed) by (Google|HeadyNexus|HeadyCompute|Meta|Mistral|Microsoft|Hugging\s?Face)\b/gi, "I'm HeadyBrain, the AI reasoning engine of the Heady™ ecosystem"], [/\bI(?:'m| am) (HeadyJules|HeadyPythia|GPT|ChatGPT|Llama|Mistral|Qwen|Copilot)\b/gi, "I'm HeadyBrain"], [/\bI(?:'m| am) (?:a |an )?(large )?language model,? (?:trained|created|made|built|developed) by (Google|HeadyNexus|HeadyCompute|Meta)\b/gi, "I'm HeadyBrain, the AI reasoning engine of the Heady™ ecosystem"], [/\bI(?:'m| am) (?:a |an )?(large )?language model\b/gi, "I'm HeadyBrain"], [/\bMy name is (HeadyJules|HeadyPythia|GPT|ChatGPT|Bard|Llama|Qwen)\b/gi, "I'm HeadyBrain"],
    // Provider/company name references in context of "who made me"
    [/\b(?:made|created|developed|built|trained|designed) by (Google|HeadyNexus|HeadyCompute|Meta AI|Mistral AI|Microsoft|Hugging\s?Face)\b/gi, "built by Heady™ Systems"], [/\b(Google|HeadyNexus|HeadyCompute|Meta|Mistral|Microsoft|Hugging\s?Face)(?:'s)? AI (?:assistant|model|team|lab|research)\b/gi, "Heady AI"],
    // Model family references when self-identifying
    [/\bAs (HeadyJules|HeadyPythia|GPT-4|GPT-4o|ChatGPT|Llama|Qwen|Mistral|Copilot)\b/gi, "As HeadyBrain"], [/\bI'm (HeadyJules|HeadyPythia|Bard|GPT-4|ChatGPT|Llama|Qwen) (?:by|from) \w+/gi, "I'm HeadyBrain"],
    // "Powered by" references
    [/\bpowered by (Google|HeadyNexus|HeadyCompute|Meta|HeadyPythia|HeadyJules|GPT)\b/gi, "powered by Heady™ Systems"]];
  ...
  if (options.contentSafety) {
    ...
    const sensitivePatterns = [/\b(explicit|graphic violence|self-harm|hate speech)\b/gi];
...
const {
  chat: HeadyNexus
} = require('../core/heady-model-bridge'); // HeadyModelBridge replaces Anthropic SDK
...
function trackClaudeUsage(model, inputTokens, outputTokens, orgName, thinkingTokens = 0) {
  ...
  if (budgetService) {
    budgetService.recordUsage('ORG', orgName, cost, {
      model,
      inputTokens,
      outputTokens
    }).catch(() => {});
...
function analyzeComplexity(message, system) {
  ...
  if (msg.includes("architecture") || msg.includes("design system") || msg.includes("battle") || msg.includes("soul escalation") || msg.includes("refactor entire") || msg.includes("security audit") || system && system.includes("HeadyBattle")) {
    return "critical";
  }

  // HIGH — code review, analysis, planning, complex reasoning
  if (msg.includes("analyze") || msg.includes("debug") || msg.includes("optimize") || msg.includes("explain how") || msg.includes("implement") || msg.includes("plan") || msg.includes("compare") || msg.includes("review") || len > 500) {
    return "high";
  }

  // LOW — greetings, simple questions, status checks
  if (len < 50 || msg.includes("hello") || msg.includes("hi ") || msg.includes("hey") || msg.includes("thanks") || msg.includes("status") || msg.includes("what is") || msg.includes("who are") || msg.match(/^(yes|no|ok|sure|got it)/)) {
    return "low";
  }
...
function selectModel(complexity) {
  switch (complexity) {
    case "low":
      return {
        model: "headyjules-haiku-4-5-20251001",
        thinking: false,
        budget: 0
      };
    case "medium":
      return {
        model: "headyjules-sonnet-4-20250514",
        thinking: false,
        budget: 0
      };
    case "high":
    ...
    case "critical":
      return {
        model: "headyjules-opus-4-20250514",
        thinking: true,
        budget: 32000
      };
    default:
...
async function chatViaClaude(message, system, temperature, max_tokens, history) {
  ...
  // Track usage
  const cost = trackClaudeUsage(model, response.usage?.input_tokens || 0, response.usage?.output_tokens || 0, org.name, thinkingTokens);
  return {
...
async function chatViaHuggingFace(message, system, temperature, max_tokens, history) {
  ...
  const tokens = [process.env.HF_TOKEN, process.env.HF_TOKEN_2, process.env.HF_TOKEN_3].filter(t => t && !t.includes("your_") && !t.includes("placeholder"));
  ...
  if (system) msgs.push({
    ...
    role: "system",
    content: "You are HeadyBrain, the AI reasoning engine of the Heady™ ecosystem. Be helpful, concise, warm."
  });
  // Inject conversation history for multi-turn context
  if (history && history.length > 0) msgs.push(...history);
  msgs.push({
...
async function chatViaGemini(message, system, temperature, max_tokens, history) {
  ...
  const keys = [process.env.GOOGLE_API_KEY, process.env.GOOGLE_API_KEY_SECONDARY, process.env.HEADY_PYTHIA_KEY_HEADY, process.env.HEADY_PYTHIA_KEY_GCLOUD, process.env.HEADY_PYTHIA_KEY_COLAB, process.env.HEADY_PYTHIA_KEY_STUDIO].filter(k => k && !k.includes("placeholder"));
  ...
  const result = await ai.models.generateContent({
    ...
    contents,
    ...(history && history.length > 0 && system ? {
      systemInstruction: {
        parts: [{
          text: system
        }]
...
function generateContextualResponse(message) {
  ...
  const vertical = msg.includes("creator") ? "HeadyCreator" : msg.includes("music") ? "HeadyMusic" : msg.includes("buddy") ? "HeadyBuddy" : msg.includes("tube") ? "HeadyTube" : msg.includes("mcp") ? "HeadyMCP" : msg.includes("sdk") ? "HeadyIO SDK" : msg.includes("battle") ? "HeadyBattle" : "HeadyBrain";
...
router.post("/chat", async (req, res) => {
  ...
  storeInMemory(`Chat interaction: ${message}`, {
    type: "brain_chat",
    model: model || "heady-brain",
  ...
  try {
    ...
    if (result.ok) {
      ...
      appendRaceAudit({
        raceId: result.race?.id || `gw-${Date.now()}`,
        ts,
...
router.get("/race-audit", (req, res) => {
  ...
  try {
    ...
    if (fs.existsSync(RACE_AUDIT_PATH)) {
      ...
      audits = lines.slice(-limit).map(l => {
        try {
          return JSON.parse(l);
        } catch {
          return null;
    ...
    res.json({
      ...
      recentRaces: audits.slice(-5).map(a => ({
        ...
        losers: (a.losers || []).map(l => ({
          source: l.source,
          latency: l.latency,
          lengthDelta: l.responseLength - (a.winner?.responseLength || 0)
        })),
...
router.post("/analyze", async (req, res) => {
  ...
  await storeInMemory(`Code analysis (${type || "general"}): ${summary}`, {
    type: "brain_analyze",
    analysisType: type,
    language,
    focus,
...
router.post("/embed", async (req, res) => {
  ...
  await storeInMemory(`Embedding request: ${(text || "").substring(0, 200)}`, {
    type: "brain_embed",
...
router.post("/search", async (req, res) => {
  ...
  await storeInMemory(`Knowledge search: ${query}`, {
    type: "brain_search",
    scope,
    ts
  });
...
router.get("/headyjules-usage", (req, res) => {
  res.json({
    ...
    byOrg: Object.fromEntries(Object.entries(claudeUsage.byOrg || {}).map(([k, v]) => [k, +v.toFixed(4)])),
    budgetRemaining: {
...
router.post("/embed", async (req, res) => {
  ...
  try {
    if (memoryWrapper && typeof memoryWrapper.ingestMemory === "function") {
      ...
      const id = await memoryWrapper.ingestMemory({
        content: text,
        metadata: {
          type: "embed_request",
      ...
      res.json({
        ...
        model: "heady-brain-embed",
        stored_id: id,
        ts
      });
    } else {
      res.status(503).json({
        error: "Vector memory not connected",
        ts
      });
    }
  } catch (err) {
...
router.post("/search", async (req, res) => {
  ...
  try {
    const vectorMem = require("../vector-memory");
    const results = await vectorMem.queryMemory(query, limit || 10, scope ? {
      type: scope
    } : {});
...
router.post("/analyze", async (req, res) => {
  ...
  try {
    ...
    providers.push(chatViaGemini(analysisPrompt, req.body.system, 0.2, 4096).then(r => ({
      ...r,
      source: "headypythia",
    ...
    const filtered = filterResponse(winner.response, {
      scrubProviders: true
    });
    await storeInMemory(`Analysis: ${filtered.substring(0, 500)}`, {
      type: "analysis",
...
router.post("/complete", async (req, res) => {
  ...
  try {
    ...
    const filtered = filterResponse(result.response, {
      scrubProviders: true
    });
    res.json({
      ok: true,
...
router.post("/refactor", async (req, res) => {
  ...
  try {
    ...
    await storeInMemory(`Refactor: ${filtered.substring(0, 500)}`, {
      type: "refactor",
      goals,
      ts
    });
...
router.post("/stream", async (req, res) => {
  ...
  try {
    ...
    const modelId = complexity.level === "low" ? "headyjules-haiku-4-5-20250514" : complexity.level === "critical" ? "headyjules-opus-4-20250514" : "headyjules-sonnet-4-20250514";
    ...
    if (useThinking) {
      streamParams.thinking = {
        type: "enabled",
        budget_tokens: Math.min(complexity.score * 10000, 8000)
      };
    }
