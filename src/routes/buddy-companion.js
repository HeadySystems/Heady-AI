...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/routes/buddy-companion.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/*
...
const companionState = {
  persona: 'empathic_safe_space',
  // empathic_safe_space | analytical_coach | environmental_actuator
  activeCards: new Map(),
  connectors: new Map(),
  // connectorId → { schema, status, lastActivity }
  contextInbox: [],
  // passive ingestion queue
  midiPorts: new Map(),
  // portId → { channel, status, target }
  metrics: {
    cardsGenerated: 0,
    connectorsCreated: 0,
...
router.post('/context/ingest', (req, res) => {
  ...
  if (!source || !data) return res.status(400).json({
    ok: false,
    error: 'source and data required'
  });
  const entry = {
  ...
  res.json({
    ok: true,
    entryId: entry.id,
    inboxSize: companionState.contextInbox.length
  });
});
...
router.post('/cards/generate', (req, res) => {
  ...
  if (!type || !title) return res.status(400).json({
    ok: false,
...
router.delete('/cards/:id', (req, res) => {
  ...
  res.json({
    ok: true,
    message: `Card ${req.params.id} destroyed`,
...
router.post('/persona/switch', (req, res) => {
  ...
  if (!valid.includes(persona)) {
    return res.status(400).json({
      ok: false,
      error: `Invalid persona. Must be one of: ${valid.join(', ')}`
    });
  }
  ...
  res.json({
    ok: true,
    persona,
    message: `Switched to ${persona} mode`
  });
});
...
router.post('/connectors/create', (req, res) => {
  ...
  if (!targetSystem) return res.status(400).json({
    ok: false,
    error: 'targetSystem required'
  });
...
router.delete('/connectors/:id', (req, res) => {
  ...
  res.json({
    ok: true,
    message: `Connector ${req.params.id} destroyed`
  });
});
...
router.get('/protocol-bridge', (req, res) => {
  res.json({
    ...
    bridges: {
      'midi-to-udp': {
        status: 'ready',
        useCase: 'High-speed local hardware/IoT control'
      },
      'midi-to-tcp': {
        status: 'ready',
        useCase: 'Reliable state transfers (financial, DB writes)'
      },
      'midi-to-mcp': {
        status: 'ready',
        useCase: 'Physical gesture → LLM tool-calling via MCP'
      },
      'midi-to-api': {
        status: 'ready',
        useCase: 'External web services via Edge Gateway + mTLS'
      }
    },
