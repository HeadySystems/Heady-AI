...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/routes/headyme-onboarding.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
...
router.get("/templates", (req, res) => {
  res.json({
    ...
    total: CONTEXT_TEMPLATES.length
  });
});
...
router.post("/plan", (req, res) => {
  ...
  if (!userId || !selectedContexts || !Array.isArray(selectedContexts)) {
    return res.status(400).json({
      ok: false,
      error: "userId and selectedContexts[] required"
    });
  }
  const planId = `plan-${Date.now().toString(36)}-${crypto.randomBytes(3).toString("hex")}`;

  // Resolve selected contexts into a unified plan
  const resolvedContexts = selectedContexts.map(ctxId => CONTEXT_TEMPLATES.find(t => t.id === ctxId)).filter(Boolean);
...
router.post("/buddy-setup", (req, res) => {
  ...
  const contexts = (selectedContexts || []).map(ctxId => CONTEXT_TEMPLATES.find(t => t.id === ctxId)).filter(Boolean);
...
router.get("/buddy-suggestions", (req, res) => {
  ...
  const providerHints = {
    github: {
      ...
      reason: 'Developer workflows + creative tools based on your GitHub sign-in.'
    },
    google: {
      contexts: ['business-ops', 'personal-wellness'],
      reason: 'Productivity + wellness based on your Google profile.'
    },
    spotify: {
      contexts: ['creative-studio'],
      reason: 'Music production tools for Spotify creators.'
    },
    instagram: {
      contexts: ['creative-studio', 'business-ops'],
      reason: 'Visual content creation + brand analytics.'
    },
    tiktok: {
      contexts: ['creative-studio', 'business-ops'],
      reason: 'Short-form content creation + growth analytics.'
    },
    amazon: {
      contexts: ['business-ops'],
      reason: 'Commerce and operations management.'
    },
    discord: {
      contexts: ['dev-platform', 'creative-studio'],
      reason: 'Community management + development tools.'
    }
  };
  const suggestion = providerHints[provider] || {
    contexts: ['dev-platform', 'business-ops'],
    reason: 'Popular starting contexts for new users.'
  };
  res.json({
    ok: true,
    userId: userId || null,
    ...
    allTemplates: CONTEXT_TEMPLATES.map(t => ({
      id: t.id,
      label: t.label,
...
router.post("/finalize", (req, res) => {
  ...
  if (setupId) {
    ...
    const userSetups = Array.from(onboardingProfiles.values()).filter(p => p.userId === userId && p.step === 'buddy-guided-setup').sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
    setup = userSetups[0] || null;
  }
  if (!setup) {
    return res.status(404).json({
      ok: false,
      error: "No buddy setup found — complete /buddy-setup first"
    });
  }
