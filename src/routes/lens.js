...
} = require('../core/heady-model-bridge'); // HeadyModelBridge replaces Google Generative AI SDK
...
const MONITORED_SERVICES = ["heady-brain", "heady-soul", "heady-battle", "heady-hcfp", "heady-patterns", "heady-ops", "heady-maintenance", "heady-vinci", "heady-notebooklm", "heady-auto-success", "heady-lens", "heady-conductor"];
    ...
    for (const [service, endpoint] of entries) {
      ...
      try {
        const {
          ...
          statusCode,
          latencyMs
        } = await fetchManagerJson(endpoint);
...
router.post("/observe", (req, res) => {
  ...
  if (truthKey && (metric === "health" || metric === "status" || metric === "state-change")) {
    ...
    const healthy = numeric !== null ? numeric > 0 : !["down", "error", "critical", "unhealthy", "failed"].includes(asString);
    const status = healthy ? "healthy" : "degraded";
    const prev = serviceTruth.get(truthKey) || {
      endpoint: MONITORED_ENDPOINTS[truthKey]
    };
...
router.post("/analyze", async (req, res) => {
  ...
  const analysis = {
    ...
    systemHealth: recent.length > 0 ? recent.reduce((s, d) => s + d.significance, 0) / recent.length : 1.0
  };
  ...
  try {
    ...
    if (genAI) {
      try {
        ...
        const resolvedPath = path.isAbsolute(imageRef) ? imageRef : path.join(__dirname, "..", "..", imageRef);
...
router.get("/differentials", (req, res) => {
  ...
  res.json({
    ok: true,
    differentials: differentials.slice(-limit),
...
router.get("/memory", (req, res) => {
  ...
  res.json({
    ...
    memory: {
      ...
      recentGained: gained.slice(-5).map(d => ({
        source: d.source,
        metric: d.metric,
        significance: d.significance,
        ts: d.ts
      })),
      recentDiscarded: discarded.slice(-3).map(d => ({
        source: d.source,
        metric: d.metric,
        significance: d.significance,
        ts: d.ts
...
router.get("/analyze", (req, res) => res.json({
  ok: true,
  recentDifferentials: differentials.slice(-10)
}));
router.get("/process", (req, res) => res.json({
  ok: true,
  snapshots: snapshots.size,
  differentials: differentials.length
}));
