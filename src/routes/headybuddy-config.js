...
router.get('/', (req, res) => {
  res.json({
    ...
    providers: {
      ...
      'headyjules-haiku': {
        label: 'HeadyJules Haiku',
        icon: '⚡',
        color: '#fbbf24'
      },
      'headyjules-sonnet': {
        label: 'HeadyJules Sonnet',
        icon: '⬡',
        color: '#fb923c'
      },
      'headyjules-opus': {
        label: 'HeadyJules Opus',
        icon: '🔮',
        color: '#f472b6'
      },
      'gpt-4o': {
    ...
    capabilities: {
      ...
      inputExamples: ['super random stuff for buddy', 'analyze my nonprofit fundraising data and make it pretty', 'build me a dashboard showing KPIs for Q1', 'create a branded infographic for our annual report', 'give me raw JSON of all active services']
    },
    suggestedCombos: [{
...
router.get('/services', async (req, res) => {
  ...
  try {
    ...
    checks.manager = 'disconnected';
  }
  try {
    const headylocal = await fetch("http://0.0.0.0:11434/", {
      signal: AbortSignal.timeout(2000)
    });
    checks.headylocal = headylocal.ok ? 'connected' : 'disconnected';
  } catch {
    checks.headylocal = 'disconnected';
  }
