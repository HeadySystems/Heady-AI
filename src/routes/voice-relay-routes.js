...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/routes/voice-relay-routes.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/**
...
router.get('/status', (_req, res) => {
  try {
    ...
    try {
      ...
      if (typeof relay.getStatus === 'function') relayStatus = relay.getStatus();else relayStatus.active = true;
  ...
  try {
    ...
    try {
      ...
      if (typeof ttsProvider.textToSpeech === 'function') {
        audioUrl = await ttsProvider.textToSpeech(text, {
          voice,
          format
        });
      }
...
router.post('/stt', async (req, res) => {
  try {
    ...
    if (!audioUrl) return res.status(400).json({
      ok: false,
      error: 'audioUrl (string) required'
    });
    let transcription = null;
    try {
      const sttProvider = require('../services/edge-diffusion');
      if (typeof sttProvider.speechToText === 'function') {
        transcription = await sttProvider.speechToText(audioUrl, {
          language
        });
      }
