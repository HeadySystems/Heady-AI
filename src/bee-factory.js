...
function createBee(domain, config = {}) {
  ...
  const entry = {
    ...
    vector,
    csl: {
      priorityState: priorityClass.state,
      // +1 = critical, 0 = normal, -1 = low
...
function spawnBee(name, work, priority = 0.8) {
  ...
  const entry = {
    ...
    vector,
    csl: {
      priorityState: CSL.ternary_gate(priority, INV_PHI, 1 - INV_PHI).state
    },
      ...
      workers: [{
        ...
        fn: async () => {
          ...
          try {
            ...
            const body = res.headers.get('content-type')?.includes('json') ? await res.json().catch(() => null) : await res.text().catch(() => null);
            return {
              target: cfg.target,
              url,
      ...
      workers: [{
        ...
        fn: async () => {
          ...
          return {
            ...
            uptimeHuman: uptimeSec > 86400 ? `${Math.floor(uptimeSec / 86400)}d ${Math.floor(uptimeSec % 86400 / 3600)}h` : uptimeSec > 3600 ? `${Math.floor(uptimeSec / 3600)}h ${Math.floor(uptimeSec % 3600 / 60)}m` : `${Math.floor(uptimeSec / 60)}m ${Math.round(uptimeSec % 60)}s`,
        ...
        fn: async () => {
          ...
          if (heapPercent > (cfg.heapThreshold || 85)) {
            alerts.push({
              ...
              threshold: `${cfg.heapThreshold || 85}%`
            });
          }
          if (process.uptime() > (cfg.maxUptimeSeconds || 86400 * 7)) {
            alerts.push({
              ...
              threshold: 'restart recommended'
            });
          }
          if (global.eventBus && alerts.length > 0) {
            global.eventBus.emit('bee:alerts', {
              target: cfg.target,
              alerts
            });
          }
...
function createSwarm(name, beeConfigs = [], policy = {}) {
  ...
  const affinityScores = beeVectors.length > 0 ? CSL.multi_resonance(swarmIntentVec, beeVectors, 0.2) : [];

  // CSL: Build composite swarm vector via consensus superposition
  const swarmVector = beeVectors.length > 0 ? CSL.consensus_superposition(beeVectors) : swarmIntentVec;
  ...
  const swarmBee = createBee(`swarm-${name}`, {
    ...
    workers: [{
      ...
      fn: async (ctx = {}) => {
        ...
        const orderedBees = affinityScores.length > 0 ? affinityScores.map(s => bees[s.index]) : bees;
        if (mode === 'parallel') {
          const settled = await Promise.allSettled(orderedBees.map(async bee => {
            const workFns = bee.getWork(ctx);
            const beeResults = [];
            for (const fn of workFns) {
              const result = await Promise.race([fn(ctx), new Promise((_, reject) => setTimeout(() => reject(new Error('timeout')), timeoutMs))]);
          ...
          for (const s of settled) {
            if (s.status === 'fulfilled') {
              results[s.value.domain] = {
                status: 'ok',
                results: s.value.results
              };
            } else {
              results[s.reason?.domain || 'unknown'] = {
                status: 'error',
        ...
        return {
          ...
          csl: {
            affinityScores: affinityScores.map(s => ({
              index: s.index,
              score: s.score
            })),
  ...
  swarmBee.csl.affinityScores = affinityScores.map(s => ({
    index: s.index,
    score: s.score
  }));
  return swarmBee;
