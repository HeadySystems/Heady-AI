...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/persona/empathic-persona-engine.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/**
...
class EmotionVector {
  ...
  blend(other, alpha = 1 / PHI) {
    return new EmotionVector(this.valence * (1 - alpha) + other.valence * alpha, this.arousal * (1 - alpha) + other.arousal * alpha, this.dominance * (1 - alpha) + other.dominance * alpha);
  }
   ...
   */
  distanceTo(other) {
    return Math.sqrt(Math.pow(this.valence - other.valence, 2) + Math.pow(this.arousal - other.arousal, 2) + Math.pow(this.dominance - other.dominance, 2));
  }
...
class EmotionDetector {
  ...
  detectFromText(text) {
    ...
    const detected = new EmotionVector(Math.max(-1, Math.min(1, valence)), Math.max(-1, Math.min(1, arousal)), Math.max(-1, Math.min(1, dominance)));

    // Smooth with history using α
    this._current = this._current.blend(detected, this._smoothing);
    this._history.push({
      ts: Date.now(),
      emotion: this._current.toJSON(),
      source: 'text'
    });
  ...
  detectFromVoiceMeta(meta) {
    ...
    const arousal = Math.min(1, Math.max(-1, (pitchHz - 150) / 100 * 0.4 + (tempoWpm - 130) / 80 * 0.3 + (energyDb + 30) / 30 * 0.3));
...
class ResponseModulator {
  ...
  _buildSystemPrompt(persona, emotion) {
    const parts = [];
    if (persona.formality > 0.7) parts.push('Respond in a formal, professional tone.');else if (persona.formality < 0.3) parts.push('Respond in a casual, conversational tone.');else parts.push('Respond in a friendly, clear tone.');
...
class PersonaEngine {
  ...
  constructor(opts = {}) {
    ...
    this._biometric.onChange(state => {
      ...
      if (state.arousal !== 0) {
        this._detector._current = this._detector._current.blend(new EmotionVector(0, state.arousal, 0), 0.15);
      }
    });

    // Create default profile
    const defaultProfile = new PersonaProfile(opts.defaultProfile || {
      name: 'Heady'
    });
  ...
  process(input, voiceMeta = null) {
    const emotion = this._detector.detectFromText(typeof input === 'string' ? input : input.text || '');
  ...
  getEmotionHistory() {
    return this._detector.getHistory();
  }
  getProcessHistory() {
    return this._history.slice();
  }
  ...
  getModulator() {
    return this._modulator;
  }
  listProfiles() {
    return Array.from(this._profiles.values()).map(p => p.toJSON());
  }
}
