...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/source-reference/v2/self-awareness.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/*
...
const DOMAINS = ['headysystems.com', 'headyme.com', 'headyconnection.org', 'headyio.com', 'headymcp.com', 'headybuddy.org', 'manager.headysystems.com'];
const BRAND_CHECKS = {
  requiredMeta: ['og:title', 'og:description', 'og:image'],
  requiredHeaders: ['x-heady-edge', 'x-heady-serve'],
  forbiddenStrings: ["0.0.0.0:3301", "0.0.0.0:3301", 'placeholder', 'TODO', 'FIXME'],
...
async function runBrandingScan() {
  ...
  for (const domain of DOMAINS) {
    ...
    if (result.healthy) monitorState.healthy++;else {
...
async function ingestTelemetry(event) {
  ...
  if (entry.severity === 'error' || entry.severity === 'critical') _telemetryStats.errors++;else if (entry.severity === 'warn') _telemetryStats.warnings++;else _telemetryStats.successes++;
...
async function assessSystemState(context = '') {
  ...
  let confidence = 1.0;
  if (_telemetryStats.errorRate1m > 50) confidence *= 0.3;else if (_telemetryStats.errorRate1m > 25) confidence *= 0.5;else if (_telemetryStats.errorRate1m > 10) confidence *= 0.7;else if (_telemetryStats.errorRate1m > 5) confidence *= 0.85;

  // Memory pressure factor
  const mem = process.memoryUsage();
  const heapUsagePercent = mem.heapUsed / mem.heapTotal * 100;
  if (heapUsagePercent > 90) confidence *= 0.5;else if (heapUsagePercent > 75) confidence *= 0.8;

  // Vector memory depth query (if available)
  let memoryContext = [];
  if (vectorMemory && context.length > 0) {
    try {
      memoryContext = await vectorMemory.queryMemory(`system health assessment for: ${context}`, 3, {
        type: 'self_telemetry'
      });
    } catch {/* best-effort */}
  }
  ...
  const contextString = [`[HEADY SELF-AWARENESS STATE]`, `Confidence: ${(confidence * 100).toFixed(0)}%`, `Error rate (1m): ${_telemetryStats.errorRate1m}% | (5m): ${_telemetryStats.errorRate5m}%`, `Heap usage: ${heapUsagePercent.toFixed(0)}%`, `Recent events (5m): ${recent5m.length} total, ${recentErrors.length} errors`, `Uptime: ${(process.uptime() / 3600).toFixed(1)}h`, recommendations.length > 0 ? `Recommendations: ${recommendations.join('; ')}` : 'Status: NOMINAL', memoryContext.length > 0 ? `Historical context: ${memoryContext.map(m => m.content?.substring(0, 80)).join(' | ')}` : ''].filter(Boolean).join('\n');
  return {
    confidence,
    contextString,
...
function getSystemIntrospection() {
  const vmStats = vectorMemory ? vectorMemory.getStats() : {
    total_vectors: 0
  };
  const mem = process.memoryUsage();
  return {
