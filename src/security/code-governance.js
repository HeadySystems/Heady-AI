...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/security/code-governance.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/*
...
function checkDeveloper(email) {
  ...
  for (const dev of config.approved_developers || []) {
    ...
    if (gitEmails.includes(normalizedEmail)) {
      ...
      return {
        approved: true,
        identity: dev,
        reason: `Approved developer: ${dev.id}`
      };
    }
  }
  audit({
    action: "DEVELOPER_DENIED",
    email: normalizedEmail,
  ...
  return {
    approved: false,
    identity: null,
    reason: `Email '${normalizedEmail}' not in approved developers list`
  };
}
...
function checkAgent(agentId) {
  ...
  for (const agent of config.approved_agents || []) {
    if (agent.id.toLowerCase() === normalizedId) {
      ...
      return {
        approved: true,
        identity: agent,
        reason: `Approved agent: ${agent.id}`
      };
    }
  }
  ...
  audit({
    action: "AGENT_UNKNOWN_BLOCKED",
    agentId: normalizedId
  });
  return {
    approved: false,
    identity: null,
    reason: `Unknown agent '${normalizedId}' — not in approved list. Owner must approve.`
  };
}
...
function authorize(opts = {}) {
    ...
    if (protectedHits.length > 0) {
      ...
      if (identity.type !== "owner") {
        ...
        audit({
          action: "AUTH_GATE_DENIED",
          ...opts,
          step: "protected-files",
          protectedFiles: protectedHits
        });
        return result;
      }
    }
  ...
  result.reason = opts.agentId ? `Agent '${opts.agentId}' authorized via Heady™ auth schema` : `Developer '${opts.email}' authorized`;
  audit({
    action: "AUTH_GATE_APPROVED",
    ...opts
  });
  return result;
}
...
function approveAgent(agentId, approverEmail, permissions = ["read", "write"]) {
  ...
  if (!approver.approved || approver.identity?.type !== "owner") {
    ...
    return {
      ok: false,
      reason: "Only owners can approve agents"
    };
  }
  ...
  // Write back
  try {
    fs.writeFileSync(GOVERNANCE_CONFIG, yaml.dump(config, {
      lineWidth: 120,
      noRefs: true
    }));
    _configHash = null; // force reload
    audit({
      action: "AGENT_APPROVED_BY_OWNER",
      agentId,
      approverEmail,
      permissions
    });
    ...
    return {
      ok: false,
      reason: `Failed to write config: ${err.message}`
    };
  }
}
...
function isPatentLocked(filePath) {
  ...
  if (!patentLock) return {
    locked: false,
    claim: null,
    reason: "No patent_lock config"
  };
  ...
  return {
    locked: false,
    claim: null,
    reason: "Not in patent lock zone"
  };
}
...
function generateEvidenceSnapshot() {
  ...
  const compositeHash = crypto.createHash("sha384").update(Buffer.concat(contents.filter(c => Buffer.isBuffer(c)))).digest("hex");
  const snapshot = {
    ok: true,
...
function registerRoutes(router) {
  ...
  router.get("/api/governance/status", (req, res) => {
    ...
    res.json({
      ok: true,
      configHash: _configHash,
      approvedDevelopers: (config.approved_developers || []).map(d => ({
        id: d.id,
        type: d.type
      })),
      approvedAgents: (config.approved_agents || []).map(a => ({
        id: a.id,
        type: a.type
      })),
      blockedGateways: (config.blocked_gateways || []).map(b => ({
        id: b.id,
        vendor: b.vendor
      })),
