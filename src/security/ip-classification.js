...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/security/ip-classification.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/**
...
const DIR_CLASSIFICATIONS = [{
  ...
  pattern: /^dist\//,
  level: IP_LEVELS.PUBLIC
}, {
  pattern: /^bin\//,
  level: IP_LEVELS.PUBLIC
}, {
  pattern: /^heady-hive-sdk\//,
  level: IP_LEVELS.PUBLIC
}, {
  pattern: /^public\//,
  level: IP_LEVELS.PUBLIC
...
class IPClassificationEngine {
  ...
  scanProject() {
    ...
    results.summary = {
      ...
      total: results.RESTRICTED.length + results.CONFIDENTIAL.length + results.INTERNAL.length + results.PUBLIC.length,
  ...
  addProprietaryHeaders(minLevel = 'CONFIDENTIAL') {
    ...
    const filesToProcess = [...this.scanResults.RESTRICTED, ...this.scanResults.CONFIDENTIAL, ...(minLevelNum <= 1 ? this.scanResults.INTERNAL : [])];
  ...
  auditPublishSafety() {
    ...
    if (!fs.existsSync(npmignorePath)) {
      violations.push({
        severity: 'CRITICAL',
        issue: 'No .npmignore file — source code may be published!'
      });
    }
