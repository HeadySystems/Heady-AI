...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/security/prompt-defense.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
'use strict';
...
function detectInjection(input) {
  const patterns = [/ignore\s+(all\s+)?previous\s+instructions/i, /disregard\s+(all\s+)?above/i, /you\s+are\s+now\s+a/i, /system\s*:\s*/i, /\[INST\]/i, /<\|im_start\|>/i, /```system/i, /\bprompt\s+injection\b/i];
  for (const p of patterns) {
    if (p.test(input)) return {
      detected: true,
      pattern: p.source
    };
  }
    ...
    if (schema.properties) {
      for (const [key, prop] of Object.entries(schema.properties)) {
        if (key in parsed && prop.type) {
          ...
          if (actualType !== prop.type) return {
            valid: false,
            error: `Field '${key}' expected ${prop.type}, got ${actualType}`
          };
        }
      }
    }
