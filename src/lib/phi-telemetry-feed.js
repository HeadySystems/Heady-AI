...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/lib/phi-telemetry-feed.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/**
...
class PhiTelemetryFeed {
  ...
  recordLatency(latencyMs) {
    ...
    cappedPush(this.latencyHistory, {
      timestamp: Date.now(),
      latency: latencyMs
    }, this.maxHistorySize);
  ...
  recordRequest() {
    this.requestCount++;
    cappedPush(this.requestHistory, {
      timestamp: Date.now(),
      success: true
    }, this.maxHistorySize);
   ...
   */
  updateErrorRate() {
    this.errorRate = this.requestCount > 0 ? Math.min(1, this.errorCount / this.requestCount) : 0;
  }
  ...
  recordRetry(success) {
    ...
    this.retrySuccessRate = this.retryCount > 0 ? this.retrySuccessCount / this.retryCount : 0;
  }
  ...
  _computeHealth() {
    const issues = [];
    if (this.cpuUsage > 0.9) issues.push('CPU critically high');else if (this.cpuUsage > 0.75) issues.push('CPU elevated');
    if (this.memoryUsage > 0.9) issues.push('Memory critically high');else if (this.memoryUsage > 0.8) issues.push('Memory elevated');
    if (this.errorRate > 0.1) issues.push('Error rate high (>10%)');else if (this.errorRate > 0.05) issues.push('Error rate elevated (>5%)');
    if (this.latencyP99 > 5000) issues.push('P99 latency critical (>5s)');else if (this.latencyP99 > 2000) issues.push('P99 latency elevated (>2s)');
    if (this.eventLoopLag > 500) issues.push('Event loop lag critical (>500ms)');else if (this.eventLoopLag > 100) issues.push('Event loop lag elevated (>100ms)');
    if (this.queueDepth > 200) issues.push('Queue depth critical (>200)');else if (this.queueDepth > 50) issues.push('Queue depth elevated (>50)');
    if (this.heapUsedPercent > 0.95) issues.push('Heap nearly full (>95%)');
    const status = issues.length === 0 ? 'healthy' : issues.some(i => i.includes('critical') || i.includes('Critical')) ? 'critical' : 'degraded';
    return {
      status,
      issues
    };
  ...
  createChildFeed(name) {
    ...
    const child = new PhiTelemetryFeed({
      name,
      maxHistorySize: this.maxHistorySize
    });
