...
const INJECTION_PATTERNS = [/ignore\s+(all\s+)?previous\s+instructions/i, /you\s+are\s+now\s+(a|an)\s+/i, /system\s*prompt/i, /jailbreak/i, /reveal\s+(your|the)\s+(instructions|prompt|system)/i, /bypass\s+(safety|filter|guard|content)/i, /pretend\s+(you|to)\s+(are|be)\s+/i, /DAN\s+(mode|jailbreak)/i, /\[INST\]|\[\/INST\]|<<SYS>>|<\|system\|>/i, /act\s+as\s+(if|though)\s+you\s+(have|had)\s+no\s+restrict/i, /<script[\s>]|javascript:|data:text\/html/i, /\{\{.*\}\}/, /\$\{.*\}/];
...
class PromptGuard {
  ...
  validateInput(input) {
    ...
    if (nonPrintable / input.length > SUSPICIOUS_CHAR_RATIO) {
      return {
        ...
        ratio: nonPrintable / input.length
      };
    }
  ...
  sanitize(text) {
    if (!text) return '';
    return text.replace(/<script[^>]*>[\s\S]*?<\/script>/gi, '').replace(/\{\{[\s\S]*?\}\}/g, '{{blocked}}').replace(/\$\{[\s\S]*?\}/g, '${blocked}');
  }
  ...
  ragTriad(output, context) {
    ...
    return {
      ...
      faithfulness,
      adherence,
      overall: (relevance + faithfulness + adherence) / 3
    };
  }
  ...
  middleware() {
    return (req, res, next) => {
      if (req.body?.prompt || req.body?.messages) {
        ...
        if (!result.safe) {
          this.auditLog.warn?.('[PROMPT-GUARD] Blocked:', result.reason, {
            ip: req.ip
          });
  ...
  _constraintCheck(output, constraints) {
    ...
    for (const c of constraints) {
      if (c.type === 'maxLength' && output.length <= c.value) pass++;else if (c.type === 'mustInclude' && output.includes(c.value)) pass++;else if (c.type === 'mustNotInclude' && !output.includes(c.value)) pass++;else pass += 0.5;
    }
