...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/core/heady-service-mesh.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
'use strict';
...
const DEFAULT_CONFIG = {
  healthCheckIntervalMs: Math.round(PHI_TIMING.CYCLE / PHI),
  // ~18 500 ms
  healthCheckTimeoutMs: 5_000,
  healthCheckPath: '/healthz',
  circuitBreakerThreshold: 5,
  // failures before OPEN
  circuitBreakerResetMs: Math.round(60_000 * PHI),
  // ~97 s
  maxRetries: 3,
...
const filterInstances = instances => IS_PRODUCTION ? instances.filter(i => !i.url.includes("0.0.0.0")) : instances;
const SEED_SERVICES = [
// ── Primary domains ────────────────────────────────────────────────────────
{
  name: 'headyme',
  ...
  instances: filterInstances([{
    url: process.env.SERVICE_URL || 'http://0.0.0.0:3301',
    weight: 1.0,
    tags: ['local', 'primary']
  }, {
    url: 'https://heady-manager-609590223909.us-central1.run.app',
    weight: PHI,
    tags: ['cloud-run', 'primary']
  }]),
  healthPath: '/healthz',
  version: '3.1.0',
  tier: 'core'
  ...
  domain: 'headyapi.com',
  instances: filterInstances([{
    url: process.env.SERVICE_URL || 'http://0.0.0.0:3302',
    weight: 1.0,
    tags: ['local']
  }, {
    url: 'https://api.headyapi.com',
    weight: PHI,
    tags: ['production']
  }]),
  healthPath: '/healthz',
  ...
  instances: filterInstances([{
    url: process.env.SERVICE_URL || 'http://0.0.0.0:3303',
    weight: 1.0,
    tags: ['local']
  }, {
    url: 'https://systems.headysystems.com',
    weight: PHI,
    tags: ['production']
  }]),
  healthPath: '/healthz',
  ...
  instances: filterInstances([{
    url: process.env.SERVICE_URL || 'http://0.0.0.0:3304',
    weight: 1.0,
    tags: ['local']
  }, {
  ...
  domain: 'headymcp.com',
  instances: filterInstances([{
    url: process.env.SERVICE_URL || 'http://0.0.0.0:3305',
    weight: 1.0,
    tags: ['local']
  }, {
  ...
  instances: filterInstances([{
    url: process.env.SERVICE_URL || 'http://0.0.0.0:3306',
    weight: 1.0,
    tags: ['local']
  }, {
  ...
  domain: 'headyio.com',
  instances: filterInstances([{
    url: process.env.SERVICE_URL || 'http://0.0.0.0:3307',
    weight: 1.0,
    tags: ['local']
  }, {
  ...
  domain: 'headybot.com',
  instances: filterInstances([{
    url: process.env.SERVICE_URL || 'http://0.0.0.0:3308',
    weight: 1.0,
    tags: ['local']
  }, {
  ...
  domain: 'heady-ai.com',
  instances: filterInstances([{
    url: process.env.SERVICE_URL || 'http://0.0.0.0:3309',
    weight: 1.0,
    tags: ['local']
  }, {
...
// ── Infrastructure services ────────────────────────────────────────────────
{
  ...
  {
    url: 'https://headysystems-edge-diffusion.hf.space',
    weight: PHI,
    tags: ['huggingface', 'gpu']
  }],
  healthPath: '/healthz',
  version: '1.0.0',
  tier: 'ml'
  ...
  domain: null,
  instances: filterInstances([{
    url: process.env.SERVICE_URL || 'http://0.0.0.0:5432',
    weight: 1.0,
    tags: ['postgres', 'pgvector']
  }, {
    url: process.env.DATABASE_URL || process.env.SERVICE_URL || 'http://0.0.0.0:5432',
    weight: PHI,
    tags: ['neon', 'pgvector', 'production']
  }]),
  healthPath: null,
  // TCP probe only
  version: '16.0',
...
function httpGet(rawUrl, timeoutMs = 5_000) {
  return new Promise((resolve, reject) => {
    ...
    const options = {
      ...
      timeout: timeoutMs,
      headers: {
        'User-Agent': 'heady-service-mesh/3.1.0'
      }
    };
    ...
    req.on('timeout', () => {
      req.destroy();
      reject(new Error(`health-probe timeout: ${rawUrl}`));
    });
    req.on('error', reject);
    req.end();
  });
...
class ServiceEntry {
  ...
  constructor(def, config = DEFAULT_CONFIG) {
    ...
    this.instances = (def.instances ?? []).map(inst => new ServiceInstance(inst, cbCfg));
  }
  ...
  _pickLeastConnections(pool) {
    return pool.reduce((best, inst) => inst.activeConns < best.activeConns ? inst : best);
  }
...
class HeadyServiceMesh extends EventEmitter {
  ...
  async start() {
    ...
    this._runHealthProbes().catch(err => logger.error('[mesh] initial health probe error:', err.message));

    // Periodic probe
    this._probeTimer = setInterval(() => this._runHealthProbes().catch(console.error), this._config.healthCheckIntervalMs);
    ...
    this._publish('heady:service:mesh_started', {
      ts: Date.now()
    });
  }
  ...
  stop() {
    ...
    this._started = false;
    this._publish('heady:service:mesh_stopped', {
      ts: Date.now()
    });
  }
  ...
  register(def) {
    ...
    this._publish('heady:service:registered', {
      name,
      url,
      tags,
      ts: Date.now()
  ...
  deregister(name, url) {
    ...
    this._publish('heady:service:deregistered', {
      name,
      url,
      ts: Date.now()
    });
    ...
    for (let attempt = 0; attempt <= this._config.maxRetries; attempt++) {
      ...
      try {
        ...
        if (attempt < this._config.maxRetries) {
          const delay = this._config.retryDelayBaseMs * Math.pow(PHI, attempt);
          await _sleep(delay);
        }
      }
    }
    ...
    try {
      ...
      if (inst.healthy) {
        ...
        if (!wasHealthy) {
          ...
          this._publish('heady:service:healthy', {
            name: entry.name,
            url: inst.url,
            ts: Date.now()
          });
  ...
  metricsText() {
    const lines = ['# HELP heady_mesh_instance_healthy Whether each service instance is healthy (1=yes, 0=no)', '# TYPE heady_mesh_instance_healthy gauge'];
    for (const [, entry] of this._registry) {
      for (const inst of entry.instances) {
