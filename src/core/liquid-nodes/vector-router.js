...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/core/liquid-nodes/vector-router.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/**
...
import { PHI, PSI, fib, CSL_THRESHOLDS, cslGate, cslBlend, phiBackoff, phiFusionWeights, phiResourceWeights } from '../../../shared/phi-math.js';
...
class VectorRouter extends EventEmitter {
  ...
  scoreCandidates(nodes, taskVector, taskType) {
    ...
    for (const node of nodes) {
      ...
      const healthFactor = node.health.consecutiveFailures === 0 ? 1.0 : 1.0 / (1.0 + node.health.consecutiveFailures * PSI);

      // Capacity factor: prefer less loaded nodes
      const capacityFactor = 1.0 - node.capacity.utilization;

      // CSL-gated composite score
      const rawScore = cosineSim * wSimilarity + healthFactor * wHealth + capacityFactor * wCapacity;
  ...
  selectOptimal(taskVector, taskType, constraints = {}) {
    ...
    if (constraints.minCapacity) {
      candidates = candidates.filter(n => n.capacity.max - n.capacity.current >= constraints.minCapacity);
    }
    if (candidates.length === 0) {
      logger.warn('No healthy candidates for routing', {
        taskType,
        constraints
      });
      return null;
    ...
    const topTier = scored.filter(s => s.score >= scored[0].score * PSI // Within PSI ratio of top score
    );
  ...
  routeWithFallback(taskVector, taskType, maxAttempts = fib(5)) {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      let candidates = this._registry.getHealthyNodes().filter(n => !triedNodes.has(n.id));
      if (candidates.length === 0) {
  ...
  rebalance() {
    ...
    if (redistributions.length > 0) {
      this.emit('route:rebalance', {
        redistributions
      });
      logger.info('Rebalance computed', {
...
export { VectorRouter, computeDistance, cosineSimilarity3D, normalize3D, TASK_VECTORS, AXIS_WEIGHTS };