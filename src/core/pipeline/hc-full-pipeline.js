...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/core/pipeline/hc-full-pipeline.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/**
...
const QUALITY_GATES = {
  PASS: 1 - Math.pow(PSI, 3) * 0.5,
  // ≈ 0.882
  REVIEW: 1 - Math.pow(PSI, 2) * 0.5,
  // ≈ 0.809
  RETRY: 1 - PSI * 0.5,
  // ≈ 0.691
...
const STAGE_DEFINITIONS = [{
  ...
  timeoutMs: Math.round(PHI * 1000 * FIB[4]),
  // ~5s
...
export class HCFullPipeline extends EventEmitter {
  constructor(config = {}) {
    super();
    this.stages = STAGE_DEFINITIONS.map(s => ({
      ...s
    }));
  ...
  getRunHistory(limit = FIB[7]) {
    ...
    return {
      ...
      stats: {
        ...
        avgLatency: completed.length > 0 ? Math.round(completed.reduce((s, r) => s + r.latency, 0) / completed.length) : 0
      }
    };
  }
  ...
  async _executeDAG(runState) {
    ...
    while (completed.size + failed.size < this.stages.length) {
      ...
      const ready = this.stages.filter(stage => !completed.has(stage.id) && !failed.has(stage.id) && stage.dependencies.every(dep => completed.has(dep)) && !stage.dependencies.some(dep => failed.has(dep)));
      if (ready.length === 0) {
        // Check if we're blocked by failures
        const blocked = this.stages.filter(stage => !completed.has(stage.id) && !failed.has(stage.id) && stage.dependencies.some(dep => failed.has(dep)));
        if (blocked.length > 0) {
  ...
  async _executeStage(stage, input, context) {
    ...
    for (let attempt = 0; attempt <= stage.retries; attempt++) {
      ...
      try {
        const output = await Promise.race([handler(input, {
          ...context,
          attempt,
          stage
        }), new Promise((_, reject) => setTimeout(() => reject(new Error(`Stage timeout: ${stage.name}`)), stage.timeoutMs))]);
