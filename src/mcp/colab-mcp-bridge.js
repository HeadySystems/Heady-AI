...
(function seedKnowledge() {
  ...
  logger.info({
    count: learner.interactionCount
  }, 'Seeded knowledge vectors');
  logger.info('Telemetry: audit trail + optimization engine active');
})();

// ── Project History Ingestion — Full codebase context on boot ────
const {
  ProjectHistoryIngestor
} = require('./project-history-ingestor');
const historyIngestor = new ProjectHistoryIngestor(learner);
historyIngestor.ingestAll().catch(e => logger.error({
  err: e
}, 'History ingest error'));
...
function loadMCPTools() {
  ...
  // heady_deep_scan merged into heady_analyze (type: 'deep-scan')
  {
    ...
    description: 'Combined auto-flow: HeadyBattle + HeadyCoder + HeadyAnalyze + HeadyRisks + HeadyPatterns via HCFP.',
    inputSchema: {
      type: 'object',
      properties: {
        task: {
    ...
    name: 'heady_chat',
    description: 'Chat with Heady™ Brain. Routes 100% through Heady™ AI.',
    inputSchema: {
      type: 'object',
      properties: {
        message: {
    ...
    name: 'heady_complete',
    description: 'Code/text completion via Heady™ Brain.',
    inputSchema: {
      type: 'object',
      properties: {
        prompt: {
    ...
    name: 'heady_analyze',
    description: 'Unified Heady analysis — code, deep-scan, web research (Perplexity Sonar Pro), architecture, security, performance. All analysis flows through this tool.',
    inputSchema: {
      type: 'object',
      properties: {
        content: {
    ...
    name: 'heady_embed',
    description: 'Generate vector embeddings via Heady™ embedding service.',
    inputSchema: {
      type: 'object',
      properties: {
        text: {
    ...
    name: 'heady_health',
    description: 'Check health/status of all Heady services.',
    inputSchema: {
      type: 'object',
      properties: {
        service: {
    ...
    name: 'heady_deploy',
    description: 'Trigger deployment/service action via Heady™ Manager.',
    inputSchema: {
      type: 'object',
      properties: {
        action: {
    ...
    name: 'heady_search',
    description: 'Search Heady knowledge base and service catalog.',
    inputSchema: {
      type: 'object',
      properties: {
        query: {
    ...
    name: 'heady_memory',
    description: 'Search HeadyMemory (3D vector space) for persistent user facts.',
    inputSchema: {
      type: 'object',
      properties: {
        query: {
    ...
    name: 'heady_refactor',
    description: 'Code refactoring suggestions from Heady™ Brain.',
    inputSchema: {
      type: 'object',
      properties: {
        code: {
    ...
    description: 'Dispatch async background coding task to HeadyJules agent.',
    inputSchema: {
      type: 'object',
      properties: {
        task: {
  ...
  // heady_perplexity_research merged into heady_analyze (type: 'research'|'academic'|'news')
  {
    ...
    description: 'Search/interact with Heady™Hub models via HuggingFace.',
    inputSchema: {
      type: 'object',
      properties: {
        action: {
          type: 'string',
          enum: ['search', 'info', 'inference']
        },
        modelId: {
          type: 'string'
        },
    ...
    name: 'heady_soul',
    description: 'HeadySoul — intelligence, consciousness, and learning layer.',
    inputSchema: {
      type: 'object',
      properties: {
        content: {
    ...
    description: 'HCFP auto-success engine status and metrics.',
    inputSchema: {
      type: 'object',
      properties: {
        detail: {
    ...
    description: 'HeadyOrchestrator — trinity communication and wavelength alignment.',
    inputSchema: {
      type: 'object',
      properties: {
        message: {
    ...
    name: 'heady_battle',
    description: 'HeadyBattle Arena — AI node competition, evaluation, leaderboard.',
    inputSchema: {
      type: 'object',
      properties: {
        action: {
          type: 'string',
          enum: ['session', 'evaluate', 'arena', 'leaderboard', 'compare']
        },
        task: {
          type: 'string'
        },
    ...
    name: 'heady_patterns',
    description: 'Design pattern detection and deep code analysis.',
    inputSchema: {
      type: 'object',
      properties: {
        code: {
    ...
    name: 'heady_risks',
    description: 'Risk assessment, vulnerability scanning, mitigation plans.',
    inputSchema: {
      type: 'object',
      properties: {
        content: {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        action: {
          type: 'string',
          enum: ['generate', 'orchestrate', 'scaffold'],
          default: 'generate'
        },
        language: {
          type: 'string'
    ...
    name: 'heady_claude',
    description: 'Advanced reasoning via Heady™Jules (Opus 4.6 Thinking 1M).',
    inputSchema: {
      type: 'object',
      properties: {
        message: {
    ...
    name: 'heady_openai',
    description: 'HeadyCompute (GPT integration with function calling).',
    inputSchema: {
      type: 'object',
      properties: {
        message: {
    ...
    name: 'heady_gemini',
    description: 'Multimodal AI via Heady™Pythia.',
    inputSchema: {
      type: 'object',
      properties: {
        prompt: {
    ...
    name: 'heady_groq',
    description: 'Ultra-fast inference via Heady™Fast.',
    inputSchema: {
      type: 'object',
      properties: {
        message: {
    ...
    name: 'heady_codex',
    description: 'Code generation/transformation via Heady™Builder (GPT-Codex).',
    inputSchema: {
      type: 'object',
      properties: {
        code: {
        ...
        action: {
          type: 'string',
          enum: ['generate', 'transform', 'document'],
          default: 'generate'
        },
        language: {
          type: 'string'
    ...
    name: 'heady_copilot',
    description: 'Inline code suggestions via Heady™Copilot.',
    inputSchema: {
      type: 'object',
      properties: {
        code: {
    ...
    name: 'heady_ops',
    description: 'DevOps automation via Heady™Ops.',
    inputSchema: {
      type: 'object',
      properties: {
        action: {
          type: 'string',
          enum: ['deploy', 'infrastructure', 'monitor', 'scale']
        },
        service: {
          type: 'string'
        },
    ...
    name: 'heady_maid',
    description: 'System cleanup and scheduling via Heady™Maid.',
    inputSchema: {
      type: 'object',
      properties: {
        action: {
          type: 'string',
          enum: ['clean', 'schedule', 'status'],
          default: 'status'
        },
        target: {
          type: 'string'
    ...
    description: 'Health monitoring, backups, updates via Heady™Maintenance.',
    inputSchema: {
      type: 'object',
      properties: {
        action: {
          type: 'string',
          enum: ['status', 'backup', 'update', 'restore'],
          default: 'status'
        },
        service: {
          type: 'string'
    ...
    name: 'heady_lens',
    description: 'Visual analysis, image processing via Heady™Lens.',
    inputSchema: {
      type: 'object',
      properties: {
        action: {
          type: 'string',
          enum: ['analyze', 'process', 'detect'],
          default: 'analyze'
        },
        image_url: {
          type: 'string'
    ...
    name: 'heady_vinci',
    description: 'Pattern recognition and prediction via Heady™Vinci.',
    inputSchema: {
      type: 'object',
      properties: {
        action: {
    ...
    name: 'heady_buddy',
    description: 'HeadyBuddy — multi-provider personal AI assistant.',
    inputSchema: {
      type: 'object',
      properties: {
        message: {
        ...
        action: {
          type: 'string',
          enum: ['chat', 'memory', 'skills', 'tasks', 'providers'],
          default: 'chat'
        },
        provider: {
          type: 'string',
    ...
    description: 'Sync Heady Knowledge Vault to NotebookLM (11 pages).',
    inputSchema: {
      type: 'object',
      properties: {
        action: {
          type: 'string',
          enum: ['sync', 'status', 'health'],
          default: 'sync'
        }
      }
    }
    ...
    name: 'heady_edge_ai',
    description: 'Cloudflare edge AI — embeddings, chat, classification, vector search.',
    inputSchema: {
      type: 'object',
      properties: {
        action: {
          type: 'string',
          enum: ['embed', 'chat', 'classify', 'vectorize-insert', 'vectorize-query', 'queue']
        },
        text: {
          type: 'string'
        },
  ...
  // ── 3D Vector Space tools (bridge-only) ──
  {
    ...
    description: 'Store a vector embedding with metadata in the 3D GPU vector space.',
    inputSchema: {
      type: 'object',
      properties: {
        embedding: {
          ...
          description: '384-dim float array'
        },
        metadata: {
          type: 'object',
    ...
    description: 'Search the 3D GPU vector space for semantically similar vectors.',
    inputSchema: {
      type: 'object',
      properties: {
        embedding: {
          ...
          description: '384-dim query vector'
        },
        topK: {
          type: 'integer',
          default: 5
    ...
    description: 'Get 3D vector space statistics (count, memory, GPU status).',
    inputSchema: {
      type: 'object',
      properties: {}
    }
    ...
    description: 'Get template auto-generation stats (cached templates, active bees, swarm history).',
    inputSchema: {
      type: 'object',
      properties: {}
    }
    ...
    name: 'heady_learn',
    description: 'Store a learning (interaction, directive, preference, decision, identity) in 3D vector memory.',
    inputSchema: {
      type: 'object',
      properties: {
        content: {
        ...
        category: {
          type: 'string',
          enum: ['directive', 'preference', 'interaction', 'decision', 'identity', 'pattern'],
          default: 'interaction'
        },
        metadata: {
          type: 'object'
    ...
    name: 'heady_recall',
    description: 'Search 3D vector memory for relevant past interactions and learnings.',
    inputSchema: {
      type: 'object',
      properties: {
        query: {
    ...
    description: 'Get continuous learning stats (interactions, directives, categories, memory usage).',
    inputSchema: {
      type: 'object',
      properties: {}
    }
    ...
    description: 'Get comprehensive telemetry stats (audit trail, tool call analytics, optimizations, environment).',
    inputSchema: {
      type: 'object',
      properties: {}
    }
...
async function callTool(name, args) {
    ...
    case 'heady_template_stats':
      {
        return {
          content: [{
            type: 'text',
            text: JSON.stringify(getTemplateStats(), null, 2)
          }]
        };
      }
    case 'heady_learn':
    ...
    case 'heady_memory_stats':
      {
        return {
          content: [{
            type: 'text',
            text: JSON.stringify(learner.getStats(), null, 2)
          }]
        };
      }
    case 'heady_telemetry':
      {
        return {
          content: [{
            type: 'text',
            text: JSON.stringify(telemetry.getStats(), null, 2)
          }]
        };
      }
        ...
        if (['research', 'deep', 'academic', 'news', 'quick'].includes(analyzeType)) {
          ...
          if (!PPLX_KEY) {
            return {
              content: [{
                type: 'text',
                text: 'PERPLEXITY_API_KEY not set'
              }],
              isError: true
            };
          }
          ...
          try {
            const res = await fetch('https://api.perplexity.ai/chat/completions', {
              ...
              body: JSON.stringify({
                ...
                messages: [{
                  role: 'system',
                  content: 'You are a deep research specialist. Provide thorough analysis with citations and evidence.'
                }, {
                  role: 'user',
                  content: args.content || args.query
                }],
            ...
            try {
              learner.learn(`[Research:${analyzeType}] ${(args.content || '').substring(0, 200)}`, 'interaction', {
                type: 'research',
                mode: analyzeType
              });
            ...
            return {
              content: [{
                type: 'text',
                text: `Perplexity Error: ${err.message}`
              }],
              isError: true
            };
          }
  ...
  try {
    ...
    return {
      content: [{
        type: 'text',
        text: `Heady™ API Error: ${err.message}`
      }],
      isError: true
    };
  }
...
function handleWebSocketUpgrade(req, socket, head) {
  ...
  const accept = crypto.createHash('sha1').update(key + '258EAFA5-E914-47DA-95CA-5AB5DC11E860').digest('base64');
  ...
  switch (msg.method) {
    ...
    case 'resources/list':
      return {
        ...
        result: {
          resources: [{
            uri: 'heady://vector/stats',
            name: '3D Vector Space Stats',
            mimeType: 'application/json'
          }, {
            uri: 'heady://services/catalog',
            name: 'Heady Service Catalog',
            mimeType: 'application/json'
          }]
        }
      };
...
function startHTTPServer() {
  const server = http.createServer(async (req, res) => {
    ...
    jsonRes(res, 404, {
      error: 'Not found',
      endpoints: ['GET  /health', 'GET  /sse', 'POST /mcp/rpc', 'GET  /mcp/tools', 'POST /mcp/tools/call', 'POST /vector/store', 'POST /vector/search', 'GET  /vector/stats', 'WS   ws://host:port (WebSocket)']
    }, req.headers.origin);
  });
