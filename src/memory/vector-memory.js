...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/memory/vector-memory.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/*
...
let federation = null;
try { federation = require("../latent/vector-federation"); } catch { }
...
async function embed(text) {
    if (Array.isArray(text) || text instanceof Float64Array || text instanceof Float32Array) {
...
class VectorMemory {
    constructor(opts = {}) {
        this.defaultNamespace = opts.defaultNamespace || 'default';
    }
    ...
    async shutdown() {
        return stopAutonomousMaintenance();
    }

    async store(key, vector, metadata = {}, namespace) {
        let embedding = null;
        let content = key;
        if (Array.isArray(vector) || vector instanceof Float64Array || vector instanceof Float32Array) {
        ...
        return smartIngest({
            content,
            metadata: { ...metadata, key, namespace: namespace || this.defaultNamespace },
            embedding
        });
    }

    async storeText(key, text, metadata = {}) {
        return smartIngest({
            content: text,
            metadata: { ...metadata, key, namespace: this.defaultNamespace },
        });
    }

    async get(key, namespace) {
        for (const shard of shards) {
            if (!shard || !shard.vectors) continue;
            const entry = shard.vectors.find(v => v.id === key || (v.metadata && v.metadata.key === key));
            if (entry) {
                return {
                    vector: Float64Array.from(entry.embedding),
                    metadata: entry.metadata,
                    updatedAt: entry.metadata?.ts || Date.now()
                };
            }
        }
        return null;
    ...
    async set(key, value) {
        ...
        const metadata = value?.metadata || (typeof value === 'object' ? value : {});
        const embedding = value?.embedding || null;
        return smartIngest({
            content,
            metadata: { ...metadata, key, namespace: this.defaultNamespace },
            embedding
    ...
    async update(key, vector, metadata = {}, namespace) {
        return this.store(key, vector, metadata, namespace);
    }
    ...
    async delete(key, namespace) {
        ...
        for (const shard of shards) {
            if (!shard || !shard.vectors) continue;
            const idx = shard.vectors.findIndex(v => v.id === key || (v.metadata && v.metadata.key === key));
            if (idx !== -1) {
                shard.vectors.splice(idx, 1);
                shard.dirty = true;
                deleted = true;
            }
        }
    ...
    async clear(namespace) {
        ...
        for (const shard of shards) {
            if (!shard || !shard.vectors) continue;
            shard.vectors = shard.vectors.filter(v => v.metadata?.namespace !== ns);
            shard.dirty = true;
        }
        rebuildZoneIndex();
        return true;
    ...
    async search(queryVector, limit = 5, minScore = 0.6, namespace) {
        if (Array.isArray(queryVector) || queryVector instanceof Float64Array || queryVector instanceof Float32Array) {
            return this.searchByVector({ vector: Array.from(queryVector), limit, minScore });
        }
        const results = await queryMemory(queryVector, limit, { namespace });
        return results
    ...
    async searchText(query, limit = 5) {
    ...
    async searchByVector({ vector, limit = 5, minScore = 0.6 }) {
        const queryEmbedding = Array.from(vector);
        const queryPos = to3D(queryEmbedding);
        const queryZone = assignZone(queryPos.x, queryPos.y, queryPos.z);
        
        let results = searchZone(queryZone, queryEmbedding, limit, {});
        const bestScore = results.length > 0 ? results[0].score : 0;
        if (bestScore < ZONE_EXPAND_THRESHOLD || results.length < limit) {
            const adjacent = getAdjacentZones(queryZone);
            for (const adjZone of adjacent) {
                const adjResults = searchZone(adjZone, queryEmbedding, limit, {});
                results = results.concat(adjResults);
            }
        }
        if (results.length < limit) {
            for (let z = 0; z < 8; z++) {
                if (z === queryZone || getAdjacentZones(queryZone).includes(z)) continue;
                const farResults = searchZone(z, queryEmbedding, limit, {});
                results = results.concat(farResults);
            }
        }
        const seen = new Set();
    ...
    async getRecent({ limit = 10, filter = {} } = {}) {
        let all = [];
        for (const shard of shards) {
            if (!shard || !shard.vectors) continue;
            all = all.concat(shard.vectors);
        }
        all.sort((a, b) => (b.metadata?.ts || 0) - (a.metadata?.ts || 0));
        return all.slice(0, limit).map(v => ({
            key: v.metadata?.key || v.id,
            content: v.content,
            metadata: v.metadata,
            updatedAt: v.metadata?.ts || Date.now()
        }));
    }

    detectDrift(vectorA, vectorB) {
        const similarity = cosineSim(vectorA, vectorB);
