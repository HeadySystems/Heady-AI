...
const PHI_INTERVALS = {
  pulse: Math.round(PHI ** 2 * 1000),
  // 2.6s   — heartbeat
  scan: Math.round(PHI ** 4 * 1000),
  // 6.85s  — quick scan
  analyze: Math.round(PHI ** 6 * 1000),
  // 17.9s  — deep analysis
  compact: Math.round(PHI ** 8 * 1000),
  // 46.9s  — compaction cycle
...
class AntiSprawlEngine {
  ...
  captureBaseline() {
    ...
    return {
      zones: this.baselineZoneDensities.size,
  ...
  detectSprawl() {
    if (!this.vectorMemory) return {
      sprawlDetected: false,
      reason: "no vector memory"
    };
    const stats = this.vectorMemory.getStats();
    ...
    for (const [zone, count] of Object.entries(stats.zones)) {
      ...
      if (baseline === 0 && count > 10) {
        ...
        alerts.push({
          ...
          count,
          baseline,
          ratio: +(count / baseline).toFixed(2),
          threshold: +(PHI * PHI).toFixed(3),
    ...
    return {
      sprawlDetected: alerts.length > 0,
      alerts,
  ...
  scan() {
    ...
    if (stats.queryCount > 1000 && stats.ingestCount === 0) {
      threats.push({
        ...
        ingestCount: stats.ingestCount,
        severity: "medium",
        note: "System is being queried without ingestion — possible data extraction"
      });
    }
    const result = {
  ...
  validate() {
    ...
    if (sprawl.sprawlDetected) {
      ...
      if (criticals.length > 0) {
        blockers.push({
          check: "anti-sprawl",
          message: `${criticals.length} critical sprawl zones detected`,
          details: criticals
        });
      } else {
        warnings.push({
          check: "anti-sprawl",
          message: `${sprawl.alerts.length} sprawl warnings`,
          details: sprawl.alerts
        });
      }
    }
    ...
    if (!security.healthy) {
      ...
      if (highThreats.length > 0) {
        blockers.push({
          check: "security",
          message: `${highThreats.length} high-severity threats in vector space`,
          details: highThreats
        });
      } else {
        warnings.push({
          check: "security",
          message: `${security.threats.length} security warnings`,
    ...
    if (!health.healthy) {
      blockers.push({
        check: "maintenance",
        message: "Vector memory unhealthy",
        details: health
      });
    }
    ...
    if (health.zones) {
      ...
      if (total > 0 && maxZone > total * 0.5) {
        warnings.push({
          check: "zone-balance",
          message: `Zone imbalance: largest zone has ${maxZone}/${total} vectors (>${50 .toFixed(0)}%)`
        });
      }
    }
    return {
...
class ProjectionManager {
  ...
  async scanPerception() {
    ...
    try {
      ...
      if (fs.existsSync(readmePath)) {
        ...
        perception.sources.readme = {
          exists: false,
          warning: 'No README — bad first impression'
        };
      }
    } catch {
      perception.sources.readme = {
  ...
  wireEventBus(eventBus) {
    if (!eventBus) return;
    const scanEvents = ['auto_success:reaction', 'deployment:completed', 'bee_swarm:discovered', 'vector_ops:started', 'auto_success:tasks_loaded', 'health:checked'];
...
class VectorSpaceOps {
  constructor(vectorMemory) {
    ...
    this.preDeployValidator = new PreDeployValidator(vectorMemory, this.antiSprawl, this.security, this.maintenance);
  ...
  registerRoutes(app) {
    ...
    app.get("/api/vector-ops/pre-deploy", (req, res) => {
      ...
      res.status(result.clear ? 200 : 422).json({
        ok: result.clear,
        ...result
      });
    });
