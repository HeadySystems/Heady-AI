...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/memory/vector-template-engine.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/*
  ...
  'site-builder': {
    ...
    workerFactory: vectorData => [{
      ...
      fn: async () => {
        ...
        return {
          action: 'assemble-site',
          content: typeof content === 'string' ? content.substring(0, 500) : 'compiled',
          ts: Date.now()
        };
      }
    }, {
    ...
    workerFactory: vectorData => [{
      ...
      fn: async () => {
        return {
          action: 'parse-code',
          parsed: true,
          type: vectorData.metadata?.type || 'js',
          ts: Date.now()
        };
      }
    }, {
    ...
    workerFactory: vectorData => [{
      ...
      fn: async () => {
        return {
          ...
          deployed: true,
          environment: vectorData.metadata?.env || 'production',
          ts: Date.now()
        };
      }
    }]
    ...
    workerFactory: vectorData => [{
      ...
      fn: async () => {
        return {
          ...
          route: vectorData.metadata?.route || '/api/*',
          ts: Date.now()
        };
      }
    }, {
  ...
  'agent-spawner': {
    ...
    workerFactory: vectorData => [{
      ...
      fn: async () => {
        ...
        return {
          action: 'init-agent',
          agent: vectorData.metadata?.agentName || 'dynamic-agent',
          ts: Date.now()
        };
      }
    }, {
      name: 'attach-personality',
      fn: async () => {
        return {
          action: 'attach-personality',
          personality: vectorData.metadata?.personality || 'default',
          ts: Date.now()
        };
      }
    }, {
    ...
    workerFactory: vectorData => [{
      ...
      fn: async () => {
        return {
          ...
          steps: vectorData.metadata?.steps || [],
          ts: Date.now()
        };
      }
    }, {
    ...
    workerFactory: vectorData => [{
      ...
      fn: async () => {
        return {
          action: 'load-data',
          loaded: true,
          target: vectorData.metadata?.target || 'default',
          ts: Date.now()
        };
      }
    }]
  ...
  'infra-deployer': {
    ...
    workerFactory: vectorData => [{
      ...
      fn: async () => {
        return {
          action: 'plan-deploy',
          provider: vectorData.metadata?.provider || 'gcloud',
          ts: Date.now()
        };
      }
    }, {
...
async function instantiate(taskQuery, options = {}) {
  ...
  const results = await vm.queryMemory(taskQuery, topK, {
    vectorTemplate: true
  });
...
async function swarm(taskQuery, options = {}) {
  ...
  for (const bee of result.bees) {
    ...
    execResults.push({
      bee: bee.domain,
      results: workerResults.map(r => r.status === 'fulfilled' ? r.value : {
...
async function indexDirectory(dirPath, options = {}) {
  ...
  for (const file of files) {
    ...
    try {
      ...
      const result = await indexArtifact(content, 'auto', {
        filename: file,
        path: fullPath
      });
      results.push(result);
