...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/bootstrap/service-registry.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
...
module.exports = function mountServices(app, {
  ...
  const routeServices = [['../routes/buddy-companion', '/api/buddy-companion', 'Buddy Companion'], ['../routes/headybuddy-config', '/api/headybuddy-config', 'HeadyBuddy Config'], ['../routes/headyme-onboarding', '/api/headyme-onboarding', 'HeadyMe Onboarding'], ['../routes/health-routes', '/health', 'Health Routes'],
  // ─── Skill-derived services (Sprint: heady-new-skills-20) ─────────────────
  ['../routes/drift-detection-routes', '/api/drift', 'Drift Detection'], ['../routes/resilience-cache-routes', '/api/cache', 'Resilience Cache'], ['../routes/incident-ops-routes', '/api/incidents', 'Incident Ops'], ['../routes/intelligence-analytics-routes', '/api/analytics', 'Intelligence Analytics'], ['../routes/cognitive-runtime-routes', '/api/cognitive', 'Cognitive Runtime'], ['../routes/connector-vault-routes', '/api/connectors', 'Connector Vault'], ['../routes/voice-relay-routes', '/api/voice', 'Voice Relay']];
  for (const [mod, mount, name] of routeServices) {
    try {
      app.use(mount, require(mod));
      logger.logNodeActivity("CONDUCTOR", `  ∞ ${name}: LOADED → ${mount}`);
    } catch (err) {
  ...
  try {
    const harmonyRoutes = require('../routes/harmony')({
      orchestrator: _engines?.orchestrator,
      engines: _engines,
      authEngine,
      midiBus
    });
    ...
    logger.logNodeActivity("CONDUCTOR", `  ⚠ Harmony not loaded: ${err.message}`);
  }

  // Enterprise Ops
  try {
    const opsRoutes = require('../routes/enterprise-ops')({
      orchestrator: _engines?.orchestrator,
      engines: _engines,
      midiBus
    });
  ...
  // Core API
  try {
    app.use('/api', require('../services/core-api'));
  } catch {
    try {
  ...
  try {
    require('../routes/buddy')(app, {
      ...
      storyDriver: _engines?.storyDriver,
      resourceManager: _engines?.resourceManager,
      patternEngine: _engines?.patternEngine,
      selfCritiqueEngine: _engines?.selfCritiqueEngine,
      orchestrator: _engines?.orchestrator,
      engines: _engines,
      vectorMemory,
      midiBus
    ...
    logger.logNodeActivity("CONDUCTOR", `  ⚠ Buddy routes not loaded: ${err.message}`);
  }

  // Secrets and Cloudflare routes
  try {
    if (secretsManager) {
      require('../hc_secrets_manager').registerSecretsRoutes(app);
      secretsManager.startMonitor(60000);
    }
    if (cfManager) {
  ...
  const autoServices = [['../services/liquid-state-manager', s => {
    s.boot();
    s.liquidStateRoutes(app);
  }, 'LiquidState'], ['../services/ide-bridge', s => s.ideBridgeRoutes(app), 'IDEBridge'], ['../services/projection-engine', s => s.projectionRoutes(app), 'ProjectionEngine'], ['../services/projection-governance', s => s.governanceRoutes(app), 'ProjectionGov'], ['../services/domain-router', s => s.domainRouterRoutes(app), 'DomainRouter'], ['../services/ui-registry', s => s.uiRegistryRoutes(app), 'UIRegistry'], ['../services/llm-router', s => s.llmRouterRoutes(app), 'LLMRouter']];
  ...
  try {
    const {
      igniteSwarm,
      swarmIgnitionRoutes
    } = require('../orchestration/swarm-ignition');
    igniteSwarm({
      enablePruner: true,
      enableTester: true,
      enableEmbedder: true
