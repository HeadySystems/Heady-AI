...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/bootstrap/service-routes.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/*
...
function registerServiceRoutes(app, deps = {}) {
  ...
  const _bootChain = (async () => {
    ...
    try {
      const {
        bootVault,
        registerVaultProjectionRoutes
      } = require("../services/vault-boot");
    ...
    try {
      ...
      } = require("../boot/wire-liquid-nodes");
  ...
  try {
    ...
    } = require("../services/auto-projection");
  ...
  try {
    const {
      contextWeaverRoutes
    } = require("../bees/context-weaver-bee");
  ...
  try {
    const {
      registerDeepScanRoutes,
      runDeepScan
    } = require("../intelligence/hc_deep_scan");
  ...
  try {
    ...
    } = require("../intelligence/hc_creative");
  ...
  try {
    const {
      DeepIntelEngine,
      registerDeepIntelRoutes
    } = require("../intelligence/hc_deep_intel");
      ...
      deepIntel.deepScanProject("/home/headyme/Heady").then(scan => {
        if (global.__sseBroadcast) {
          global.__sseBroadcast("deep_intel_scan", {
            scanId: scan.id,
            perspectives: Object.keys(scan.perspectives).length,
            score: scan.compositeScore,
            findings: scan.findings.length,
            nodesInvoked: scan.nodesInvoked.length,
  ...
  try {
    const {
      LiquidAllocator,
      registerLiquidRoutes
    } = require("../runtime/hc_liquid");
  ...
  try {
    ...
    const {
      getBrainConnector
    } = require("../memory/brain_connector");
  ...
  try {
    const {
      router: brainCoreRoutes
    } = require("../routes/brain");
    ...
    if (orchestrator) {
      app.use("/api/brain", (req, res, next) => {
        ...
        const serviceGroup = orchestrator.conductor.routeSync({
          action
        });
      ...
      orchestrator.registerHandler("chat", async () => ({
        note: "use /api/brain/chat HTTP endpoint"
      }));
      orchestrator.registerHandler("analyze", async () => ({
        note: "use /api/brain/analyze HTTP endpoint"
      }));
      orchestrator.registerHandler("embed", async () => ({
        note: "use /api/brain/embed HTTP endpoint"
      }));
      orchestrator.registerHandler("search", async () => ({
        note: "use /api/brain/search HTTP endpoint"
      }));
    }
  ...
  try {
    const {
      router: hiveSdkRoutes
    } = require("../routes/hive-sdk");
  ...
  try {
    ...
    } = require("../services/heady-notebooklm");
  ...
  try {
    ...
    if (process.env.DATABASE_URL) {
      neonDb.connect().then(r => {
        if (r.ok) logger.logNodeActivity("CONDUCTOR", "  ∞ Neon DB: CONNECTED (Scale Plan)");else logger.logNodeActivity("CONDUCTOR", `  ⚠ Neon DB connection deferred: ${r.error}`);
      }).catch(() => {});
    } else {
  ...
  const routerMounts = [["soul", "soul"], ["battle", "battle"], ["hcfp", "hcfp"], ["budget", "budget-router"], ["patterns", "patterns"]];
  ...
  try {
    ...
    app.get("/api/hcfp/manifests", (req, res) => res.json({
      ok: true,
      manifests: pipelineRunner.listManifests()
    }));
    ...
    app.get("/api/telemetry/audit", (req, res) => res.json({
      ok: true,
      entries: cogTel.readAuditLog(parseInt(req.query.limit) || 50)
    }));
    app.get("/api/telemetry/stats", (req, res) => res.json({
      ok: true,
      stats: cogTel.getAuditStats()
    }));
    logger.logNodeActivity("CONDUCTOR", "  ∞ HCFP Pipeline + Telemetry Audit: INSTALLED");
  } catch (err) {
    logger.logNodeActivity("CONDUCTOR", `  ⚠ Pipeline/Telemetry not loaded: ${err.message}`);
  }
  ...
  try {
    const {
      registerPerplexityRoutes
    } = require("../services/perplexity-research");
  ...
  app.get("/api/config/:filename", (req, res) => {
    ...
    const candidates = [path.join(projectRoot || __dirname, "src", safe), path.join(projectRoot || __dirname, "configs", safe), path.join(projectRoot || __dirname, "data", safe)];
  ...
  app.get("/api/auto-success/history", (req, res) => {
    ...
    if (autoSuccessEngine) {
      return res.json({
        ok: true,
        tasks: autoSuccessEngine.getHistory(limit)
      });
    }
    res.json({
      ok: true,
  ...
  app.get("/api/auto-success/tasks", (req, res) => {
    ...
    if (autoSuccessEngine) {
      return res.json({
        ok: true,
        tasks: autoSuccessEngine.getTaskCatalog(category)
      });
    }
    res.json({
      ok: true,
  ...
  try {
    const {
      getPool
    } = require("../services/redis-connection-pool");
    const pool = getPool();
  ...
  try {
    const {
      healthRegistry
    } = require("../services/health-registry");
  ...
  try {
    ...
    if (modelRouter.registerRoutes) modelRouter.registerRoutes(app);else if (modelRouter.router) app.use("/api/model-router", modelRouter.router);
  ...
  try {
    const {
      LogicOrchestrator
    } = require("../services/logic-orchestrator");
  ...
  try {
    const {
      SpatialRegistry
    } = require("../services/spatial-registry");
  ...
  try {
    ...
    } = require("../services/projection-dispatcher");
  ...
  try {
    const {
      ProjectionSyncAutomation
    } = require("../services/projection-sync");
  ...
  try {
    ...
    } = require("../services/decentralized-governance");
  ...
  try {
    const {
      registerGlobalNodeRoutes
    } = require("../services/global-node-network");
  ...
  try {
    const {
      registerSelfHealingRoutes,
      mesh
    } = require("../services/self-healing-mesh");
  ...
  try {
    const {
      registerDynamicWeightRoutes,
      dynamicWeights
    } = require("../services/dynamic-weight-manager");
  ...
  try {
    ...
    } = require("../services/headyme-helper");
  ...
  try {
    const {
      getArenaModeService
    } = require("../services/arena-mode-service");
  ...
  try {
    const {
      getBranchAutomationService
    } = require("../services/branch-automation-service");
  ...
  try {
    const {
      QuantumBridge
    } = require("../services/quantum-bridge");
  ...
  try {
    const {
      TraderWidgetService
    } = require("../services/trader-widget");
