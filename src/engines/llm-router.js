...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/engines/llm-router.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
'use strict';
...
const ROUTING_MATRIX = {
  ...
  [TASK_TYPES.ARCHITECTURE]: {
    primary: PROVIDERS.ANTHROPIC,
    // claude-3-opus equivalent path
    fallback1: PROVIDERS.OPENAI,
    fallback2: PROVIDERS.ANTHROPIC,
    // retry with sonnet on fb2
...
class LLMRouter {
  ...
  route(task) {
    ...
    for (let i = 0; i < chain.length; i++) {
      ...
      const priority = phiPriorityScore(health, i === 0 ? 1.0 : i === 1 ? PSI : PSI * PSI,
      // position preference
  ...
  _emergencyRoute(taskType, tokens) {
    const sorted = Object.values(this._providers).sort((a, b) => a.costPerKTokens - b.costPerKTokens);
  ...
  async execute(task) {
    ...
    for (let i = 0; i < chain.length; i++) {
      ...
      const model = useFast ? this._providers[pid].fastModel : this._providers[pid].defaultModel;
      ...
      while (rateLimitAttempt <= RATE_LIMIT_MAX_RETRIES) {
        const t0 = Date.now();
        try {
          const result = await this._callProviderWithTimeout(pid, model, task.prompt, opts, timeout);
    ...
    const err = new Error(`LLMRouter: all providers failed for taskType="${taskType}". Errors: ` + errors.map(e => `${e.providerId}(${e.reason})`).join(', '));
  ...
  async embed(text, options = {}) {
    ...
    for (const pid of chain) {
      ...
      const t0 = Date.now();
      try {
        const result = await this._callEmbedWithTimeout(pid, p.embedModel, texts, timeout);
        const latencyMs = Date.now() - t0;
        cb.onSuccess();
        this.updateHealth(pid, true, latencyMs);
  ...
  async _callProviderWithTimeout(providerId, model, prompt, opts, timeoutMs) {
    ...
    return Promise.race([callPromise, this._timeoutPromise(timeoutMs)]);
  }
  ...
  async _callProvider(providerId, model, prompt, opts) {
    ...
    switch (providerId) {
      ...
      default:
        throw Object.assign(new Error(`Unknown provider: ${providerId}`), {
  ...
  async _callEmbed(providerId, model, texts) {
    switch (providerId) {
      ...
      default:
        throw Object.assign(new Error(`Provider ${providerId} does not support embeddings`), {
  ...
  async _callAnthropic(model, prompt, opts) {
    ...
    if (!apiKey) throw Object.assign(new Error('Anthropic API key not configured'), {
      code: 'AUTH_ERROR'
    });
    ...
    throw Object.assign(new Error('Anthropic adapter not yet wired — replace stub with SDK call'), {
      code: 'STUB'
    });
  }
  ...
  async _callOpenAI(model, prompt, opts) {
    ...
    if (!apiKey) throw Object.assign(new Error('OpenAI API key not configured'), {
      code: 'AUTH_ERROR'
    });
    ...
    throw Object.assign(new Error('OpenAI adapter not yet wired — replace stub with SDK call'), {
      code: 'STUB'
    });
  }
  ...
  async _callGoogle(model, prompt, opts) {
    ...
    if (!apiKey) throw Object.assign(new Error('Google API key not configured'), {
      code: 'AUTH_ERROR'
    });
    ...
    throw Object.assign(new Error('Google adapter not yet wired — replace stub with SDK call'), {
      code: 'STUB'
    });
  }
  ...
  async _callGroq(model, prompt, opts) {
    ...
    if (!apiKey) throw Object.assign(new Error('Groq API key not configured'), {
      code: 'AUTH_ERROR'
    });
    ...
    throw Object.assign(new Error('Groq adapter not yet wired — replace stub with SDK call'), {
      code: 'STUB'
    });
  }
  ...
  async _callPerplexity(model, prompt, opts) {
    ...
    if (!apiKey) throw Object.assign(new Error('Perplexity API key not configured'), {
      code: 'AUTH_ERROR'
    });
    ...
    //   { model, messages: [{ role: 'user', content: prompt }] }
    throw Object.assign(new Error('Perplexity adapter not yet wired — replace stub with HTTP call'), {
      code: 'STUB'
    });
  }
  ...
  async _callLocal(model, prompt, opts) {
    ...
    throw Object.assign(new Error('Local Ollama adapter not yet wired — replace stub with fetch call'), {
      code: 'STUB'
    });
  }
  ...
  async _embedOpenAI(model, texts) {
    ...
    throw Object.assign(new Error('OpenAI embed adapter not yet wired'), {
      code: 'STUB'
    });
  }
  ...
  async _embedGoogle(model, texts) {
    ...
    throw Object.assign(new Error('Google embed adapter not yet wired'), {
      code: 'STUB'
    });
  }
  ...
  async _embedLocal(model, texts) {
    ...
    //   const resp = await fetch('http://0.0.0.0:11434/api/embeddings', {
    ...
    throw Object.assign(new Error('Local embed adapter not yet wired'), {
      code: 'STUB'
    });
  }
  ...
  diagnostics() {
    ...
    return {
      ...
      // fib(11) = 89
      cbThreshold: CB_FAILURE_THRESHOLD,
      // fib(5)  = 5
      healthWeights: {
        successRate: parseFloat(w0.toFixed(4)),
        latency: parseFloat(w1.toFixed(4)),
        cost: parseFloat(w2.toFixed(4))
      },
      timeouts: TIMEOUTS,
