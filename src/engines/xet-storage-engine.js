...
let fetch = null;
try {
  fetch = require('../core/heady-fetch');
} catch (e) {/* graceful */}
let logger = null;
try {
...
class XetStorageEngine {
  ...
  _log(level, msg) {
    ...
    if (level === 'error') logger.error(line);else logger.logSystem(line);
    if (global.eventBus) global.eventBus.emit('log', {
      source: 'xet-storage',
      level,
      msg
    });
