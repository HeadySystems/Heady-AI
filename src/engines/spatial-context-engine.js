...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: src/engines/spatial-context-engine.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/*
...
class SpatialContextEngine extends EventEmitter {
  ...
  generateStateVector(event) {
    ...
    return {
      ...
      y,
      z,
      pitch: this._attention.pitch,
      yaw: this._attention.yaw,
      roll: this._attention.roll
    };
  }
  _mapTemporal(event) {
  ...
  ingest(event, metadata = {}, embedding = null) {
    ...
    midiBus.cc(CHANNELS.TELEMETRY, 3, Math.min(127, Math.floor(this._spatialMemory.length / (this._maxMemory / 127))), {
      source: "spatial"
    });
    return entry;
  }
  ...
  query(stateOrEvent, topK = 5, opts = {}) {
    ...
    const candidates = this._spatialMemory.map(entry => {
      const dist = Math.sqrt(Math.pow(entry.x - qx, 2) + Math.pow(entry.y - qy, 2) + Math.pow(entry.z - qz, 2));
      const weightedDist = dist / Math.max(0.01, Math.abs(entry.weight));
      return {
        entry,
        dist,
        weightedDist,
        effectiveScore: entry.weight > 0 ? 1 / (1 + dist) : -(1 / (1 + dist))
      };
    }).filter(c => {
      ...
      return true;
    }).sort((a, b) => a.dist - b.dist).slice(0, topK);
  ...
  getMemorySize() {
    return this._spatialMemory.length;
  }
  getMetrics() {
    return {
      ...this._metrics,
      memorySize: this._spatialMemory.length,
      correctionCount: this._corrections.length
    };
  }
  ...
  // ═══ Express Routes ═══
  registerRoutes(app) {
    app.get("/api/spatial/status", (req, res) => res.json({
      ok: true,
      state: this.getCurrentState(),
      metrics: this.getMetrics()
    }));
    app.get("/api/spatial/clusters", (req, res) => res.json({
      ok: true,
      clusters: this.getClusters(parseFloat(req.query.resolution) || 0.2)
    }));
...
function getInstance(opts) {
  if (!_instance) _instance = new SpatialContextEngine(opts);
  return _instance;
}
module.exports = {
