 ...
 */
function dist3D(a, b) {
    const ax = a.x !== undefined ? a.x : (a[0] || 0);
    const ay = a.y !== undefined ? a.y : (a[1] || 0);
    const az = a.z !== undefined ? a.z : (a[2] || 0);
    const bx = b.x !== undefined ? b.x : (b[0] || 0);
    const by = b.y !== undefined ? b.y : (b[1] || 0);
    const bz = b.z !== undefined ? b.z : (b[2] || 0);
...
function toIsometric(point) {
    ...
    const y = point?.y !== undefined ? point.y : (point?.[1] || 0);
    const z = point?.z !== undefined ? point.z : (point?.[2] || 0);
    const ix = +(x - y).toFixed(6);
    const iy = +(((x + y) / 2) - z).toFixed(6);
    return {
        ix,
        iy,
        x: ix,
...
async function ingestMemory({ content, metadata = {}, embedding = null }) {
    ...
    const res = Object.assign(new String(entry.id), entry);
    return res;
}
...
module.exports = {
    ...
    // Coords & Math Exports
    toSpherical, toIsometric, dist3D, getAdjacentZones, localHashEmbed,
};
