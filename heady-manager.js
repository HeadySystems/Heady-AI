...
if (process.env.UPSTASH_REDIS_REST_URL && process.env.UPSTASH_REDIS_REST_TOKEN) {
  ...
  if (redisUrl) {
    ...
    redisClient.connect().catch((err) => {
      log.warn('Redis connect failed — falling back to in-memory rate limiting', {
...
function enforceHeadyConnectivity() {
  const criticalServices = Object.entries(remoteConfig.services).filter(
  ...
  criticalServices.forEach(([name, config]) => {
    ...
    if (!status.ok) {
      log.error('CRITICAL service unavailable', {
        serviceName: name,
        errorMessage: status.error?.message || 'Unknown error',
      });
    } else {
...
app.use((req, res, next) => {
  ...
  const isAllowed =
    allowedOrigins.includes(origin) || (origin && /\.us-central1\.run\.app$/.test(origin));
  ...
  res.setHeader(
    ...
    'Content-Type, Authorization, X-Request-ID, X-Heady-Domain',
  );
...
import('./src/gateway/ai-gateway.js')
  .then((mod) => {
...
// The legacy registry routes have been extracted to src/routes/registry.js
app.use('/api/registry', require('./src/routes/registry').createRegistryRouter());
...
app.post('/api/system/production', requireAdmin, (req, res) => {
  ...
  for (const [name, svc] of Object.entries(reg.services || {})) {
    svc.status = name === 'heady-manager' ? 'healthy' : 'active';
    report.services.push(name);
  }
  for (const [, sk] of Object.entries(reg.skills || {})) {
    sk.status = 'active';
  }
...
app.get('/api/pipeline/config', (req, res) => {
  if (!pipeline)
    return res.status(503).json({ error: 'Pipeline not loaded', reason: pipelineError });
  try {
...
app.get('/api/pipeline/status', (req, res) => {
  ...
  const lastHistory =
    pipeline?.history?.length > 0 ? pipeline.history[pipeline.history.length - 1] : null;
  res.json({
...
app.get('/api/auto-success/status', (req, res) => {
  if (!autoSuccessEngine) {
    return res
      .status(503)
      .json({ error: 'Auto-Success Engine not loaded', reason: autoSuccessError });
  }
  try {
    const status =
      typeof autoSuccessEngine.getStatus === 'function' ? autoSuccessEngine.getStatus() : {};
    const metrics =
      typeof autoSuccessEngine.getMetrics === 'function' ? autoSuccessEngine.getMetrics() : {};
...
app.post('/api/auto-success/trigger', async (req, res) => {
  ...
  try {
    const result =
      typeof autoSuccessEngine.runCycle === 'function'
...
try {
  const {
    HCResourceManager,
    registerRoutes: registerResourceRoutes,
  } = require('./src/hc_resource_manager');
  ...
  resourceManager.on('resource_event', (event) => {
    if (event.severity === 'WARN_HARD' || event.severity === 'CRITICAL') {
      log.warn('Resource severity event', {
        severity: event.severity,
        resourceType: event.resourceType,
        usagePercent: event.currentUsagePercent,
      });
    }
  });

  resourceManager.on('escalation_required', (data) => {
    log.warn('ESCALATION required - user prompt needed', {
      resourceType: data.event.resourceType,
      usagePercent: data.event.currentUsagePercent,
    });
  });
  ...
  app.get('/api/resources/health', (req, res) => {
    ...
    res.json({
      ...
      ram: {
        currentPercent: ramPercent,
        absoluteValue: Math.round(usedMem / 1048576),
        capacity: Math.round(totalMem / 1048576),
        unit: 'MB',
      },
...
try {
  ...
  } = require('./src/hc_resource_diagnostics');
...
try {
  ...
  mcPlanScheduler.on('drift:detected', (alert) => {
    log.warn('MC Drift detected', {
      taskType: alert.taskType,
      strategyId: alert.strategyId,
      medianMs: alert.medianMs,
      targetMs: alert.targetMs,
    });
  });
      ...
      if (event.severity === 'WARN_HARD' || event.severity === 'CRITICAL') {
        try {
          patternEngine.observe(
            ...
            event.currentUsagePercent,
            {
              severity: event.severity,
              tags: ['resource', event.resourceType],
            },
          );
        } catch (err) {
  ...
  if (mcPlanScheduler) {
    mcPlanScheduler.on('drift:detected', (alert) => {
      selfCritiqueEngine.recordCritique({
        ...
        weaknesses: [
          `Latency drift on ${alert.taskType}: ${alert.medianMs}ms vs ${alert.targetMs}ms target`,
        ],
...
function setupAutoTaskConversion() {
  ...
  eventBus.on('recommendation', (recommendation) => {
    try {
      const priority =
        patternEngine && typeof patternEngine.classifyPriority === 'function'
          ? patternEngine.classifyPriority(recommendation)
          : 'medium';
      const taskId = `rec-${Date.now()}`;
      const text =
        typeof recommendation === 'string' ? recommendation : recommendation.text || 'auto-task';
...
try {
  ...
  autoSuccessEngine.on('incident', (data) => {
    ...
    log.error('AutoSuccess INCIDENT', {
      cycleNumber: data.cycleNumber,
      failures: data.cumulativeFailures,
    });
  });
...
try {
  ...
  } = require('./src/hc_improvement_scheduler');
...
try {
  ...
  log &&
    log.info &&
    log.info('Swarm intelligence modules loaded', {
      swarms: !!headySwarms,
...
try {
  ...
  log &&
    log.warn &&
    log.warn('Orchestrator module not loaded (non-fatal)', { errorMessage: e.message });
}
...
try {
  ...
  log &&
    log.warn &&
    log.warn('Conductor module not loaded (non-fatal)', { errorMessage: e.message });
}
...
try {
  ...
  log &&
    log.info &&
    log.info('Blueprint subsystems loaded', {
      pd04Codec: true,
...
app.post('/api/blueprint/pd04/decode', (req, res) => {
  ...
  try {
    ...
    res.json({
      ...decoded,
      position: decoded.position.toArray(),
      intentVector: decoded.intentVector.toArray(),
    });
  } catch (e) {
    res.status(400).json({ error: e.message });
  }
...
try {
  ...
  log.info(
    'HCSysOrchestrator endpoints available: /api/orchestrator/health, /route, /brains, /layers, /contract, /rebuild-status',
  );
} catch (err) {
...
try {
  ...
  brainConnector.on('circuitBreakerOpen', (data) => {
    log.warn('Brain circuit breaker OPEN', {
  ...
  brainConnector.on('healthCheck', (results) => {
    ...
    if (healthy < results.size) {
      log.warn('Brain health check degraded', {
        healthyEndpoints: healthy,
        totalEndpoints: results.size,
      });
    }
  });
...
app.post('/api/buddy/chat', async (req, res) => {
  ...
  if (lowerMsg.includes('plan') && lowerMsg.includes('day')) {
    ...
    const contState = continuousPipeline.running
      ? `running (cycle ${continuousPipeline.cycleCount})`
      : 'stopped';
    ...
    lowerMsg.includes('why slow') ||
    lowerMsg.includes('bottleneck') ||
    lowerMsg.includes('fix resource')
  ) {
    if (resourceDiagnostics) {
      ...
      const ramPct = snap.ram?.currentPercent || 0;
      const contributors =
        events.length > 0 && events[events.length - 1].contributors
          ? events[events.length - 1].contributors
              .slice(0, 3)
              .map((c) => `${c.description} (${c.ramMB || 0} MB)`)
              .join(', ')
    ...
    lowerMsg.includes('gpu') ||
    lowerMsg.includes('tier')
  ) {
    if (resourceManager) {
      const snap = resourceManager.getSnapshot();
      const diskInfo =
        snap.disk && snap.disk.capacity > 0 ? `, Disk ${snap.disk.currentPercent}%` : '';
    ...
    lowerMsg.includes('what changed') ||
    lowerMsg.includes('narrative')
  ) {
    if (storyDriver) {
      ...
      reply =
        'Story Driver is not loaded. It tracks project narratives, feature lifecycles, and incident timelines.';
    }
    ...
    if (aiGatewayModule && typeof aiGatewayModule.generateHeadyBuddyResponse === 'function') {
      try {
        const aiResult = await aiGatewayModule.generateHeadyBuddyResponse(message, 'Heady');
        ...
        log.warn('Failed to generate HeadyBuddy response, falling back: ' + err.message);
        reply = `${greeting} I'm HeadyBuddy, your perfect day AI companion and orchestration copilot. ${activeNodes} nodes standing by. How can I help?`;
      }
    } else {
      reply = `${greeting} I'm HeadyBuddy, your perfect day AI companion and orchestration copilot. ${activeNodes} nodes standing by. How can I help?`;
...
app.get('/api/buddy/suggestions', (req, res) => {
  ...
  if (hour < 10)
    chips.push({ label: 'Plan my morning', icon: 'calendar', prompt: 'Help me plan my morning.' });
  else if (hour < 14)
    chips.push({
      label: 'Plan my afternoon',
      icon: 'calendar',
      prompt: 'Help me plan my afternoon.',
    });
  else if (hour < 18)
    chips.push({ label: 'Wrap up my day', icon: 'calendar', prompt: 'Help me wrap up today.' });
  else chips.push({ label: 'Plan tomorrow', icon: 'calendar', prompt: 'Help me plan tomorrow.' });

  chips.push({
    label: 'Summarize this',
    icon: 'file-text',
    prompt: "Summarize the content I'm looking at.",
  });
  chips.push({
    label: continuousPipeline.running ? 'Pipeline status' : 'Run pipeline',
    icon: 'play',
    prompt: continuousPipeline.running ? 'Show pipeline status.' : 'Start HCFullPipeline.',
  });
  if (activeNodes > 0)
    chips.push({
      label: 'Check resources',
      icon: 'activity',
      prompt: 'Show resource usage and node health.',
    });
  chips.push({
    label: 'Surprise me',
    icon: 'sparkles',
    prompt: 'Suggest something useful right now.',
  });
  ...
  const runCycle = async () => {
      ...
      if (storyDriver) {
        storyDriver.ingestSystemEvent({
          ...
          refs: {
            ...
            gate:
              Object.entries(continuousPipeline.gateResults).find(([, v]) => !v)?.[0] || 'unknown',
    ...
    if (pipeline) {
      try {
        ...
        continuousPipeline.errors.push({
          cycle: continuousPipeline.cycleCount,
...
app.get('/api/buddy/sync-events', (req, res) => {
  ...
  const interval = setInterval(() => {
    res.write(
      `data: ${JSON.stringify({ status: Math.random() > 0.2 ? 'connected' : 'syncing' })}\n\n`,
    );
  }, 10000);
...
// ─── HeadyBuddy Demo and Sync Endpoints ──────────────────────────────
const demoReposList = [
  {
    id: 'headybuddy',
    name: 'HeadyBuddy',
    description:
      'Breathing AI companion widget featuring sacred geometry and dynamic cross-device sync.',
  },
  {
    id: 'heady-ide',
    name: 'HeadyAI-IDE',
    description: 'Monaco-powered rich IDE interface with live LLM multi-provider chat panel.',
  },
  {
    id: 'heady-latent-os',
    name: 'Heady Latent OS',
    description: 'Sovereign microkernel-inspired orchestrator with continuous pipeline execution.',
  },
  {
    id: 'heady-microfrontend',
    name: 'Heady Microfrontend Portal',
    description: 'Unified portal with module federation routing across 7 active dashboard remotes.',
  },
  {
    id: 'heady-fintech',
    name: 'Heady Fintech Trading',
    description: 'Apex Trader Funding compliant execution layer with real-time risk checks.',
  },
  {
    id: 'heady-nonprofit',
    name: 'HeadyConnection Web',
    description: 'Nonprofit grant workflow and community program operations portal.',
  },
];

const userDevicesState = {};
const getDevicesForUser = (userId) => {
  if (!userDevicesState[userId]) {
    userDevicesState[userId] = {
      activeDevice: 'dev-desktop',
      devices: [
        { id: 'dev-desktop', name: 'Antigravity Workstation', type: 'Desktop' },
        { id: 'dev-mbp', name: 'MacBook Pro', type: 'Laptop' },
        { id: 'dev-phone', name: 'Mobile Buddy', type: 'Mobile' },
      ],
    };
  }
  return userDevicesState[userId];
};

app.get('/api/registry/demo-repos', (req, res) => {
  res.json({ repos: demoReposList });
});

app.post('/api/demo/:repoId/run', (req, res) => {
  const { repoId } = req.params;
  log.info(`[HeadyBuddy] Started demo run for repository: ${repoId}`);
  res.json({
    success: true,
    message: `Demo started successfully for ${repoId}`,
    status: 'running',
    ts: new Date().toISOString(),
  });
});
...
app.get('/api/users/:userId/devices', (req, res) => {
  ...
  const userRecord = getDevicesForUser(userId);
  res.json(userRecord);
});

app.post('/api/users/:userId/active-device', (req, res) => {
  const { userId } = req.params;
  const { deviceId } = req.body;
  if (!deviceId) return res.status(400).json({ error: 'deviceId required' });

  const userRecord = getDevicesForUser(userId);
  userRecord.activeDevice = deviceId;
  log.info(`[HeadyBuddy] Active device for user ${userId} updated to: ${deviceId}`);
  res.json({
    success: true,
    message: `Active device updated to ${deviceId}`,
...
app.post('/api/aloha/crash-report', (req, res) => {
  ...
  log.warn('ALOHA CRASH REPORT', {
    reportId: report.id,
    description: report.description,
    severity: report.severity,
  });
  const recentCrashes = alohaState.crashReports.filter(
    (r) => new Date(r.ts) > new Date(Date.now() - 3600000),
  );
...
app.post('/api/router/route', (req, res) => {
  ...
  try {
    ...
    const result = count ? vectorRouter.routeWeighted(task, count) : vectorRouter.route(task);
    res.json({ status: 'ok', result });
  } catch (e) {
    res.status(500).json({ status: 'error', error: e.message });
  }
...
app.get('/api/liquid-nodes', (req, res) => {
  try {
    ...
    const active = allNodes.filter(
      (n) => n.status === 'connected' || n.status === 'active' || n.status === 'runtime',
    ).length;
    const summary = {
  ...
  try {
    ...
    if (!domainNodes) {
      return res.status(404).json({
        ...
        validDomains: Object.keys(LIQUID_NODES_REGISTRY),
      });
    }
    ...
    log.error('Error in /api/liquid-nodes/:domain', {
      errorMessage: err.message,
...
app.get('/api/liquid-nodes/health/check', (req, res) => {
  try {
    ...
    log.error('Error in /api/liquid-nodes/health/check', {
      errorMessage: err.message,
      errorStack: err.stack,
    });
...
app.get('/api/vault/status', requireAdmin, (req, res) => {
  ...
  res.json({
    ...
    summary,
    totals: { keys: totalKeys, configured: totalSet, missing: totalKeys - totalSet },
...
app.post(
  ...
  async (req, res) => {
    ...
    const isClosed = ['done', 'completed', 'canceled', 'cancelled', 'duplicate'].includes(
      stateName,
    );
  ...
  colabLatentOps
    ...
    .then((connected) => {
      if (connected) {
        ...
        log &&
          log.warn &&
          log.warn('⚠️  Colab Latent Ops: no credentials configured — running in local-only mode');
      }
    })
    .catch((e) => {
