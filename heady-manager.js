...
async function initSecrets() {
  try {
    ...
    logger.logNodeActivity("CONDUCTOR", "Initializing Hybrid Secret Manager...");
    ...
    logger.logNodeActivity("CONDUCTOR", `Hybrid secrets synced to process.env (${results.loaded.length} loaded)`);

    // 3. Initialize legacy metadata tracker (Graceful fallback if module missing)
    try {
      ...
      logger.logNodeActivity("CONDUCTOR", `  ⚠ Legacy Secrets/Cloudflare not loaded: ${e.message}`);
    }
  } catch (err) {
    logger.logNodeActivity("CONDUCTOR", `  ⚠ Secrets initialization failed: ${err.message}`);
  }
}
...
const coreApi = require('./src/services/core-api');
/**
 * @swagger
 * /api/health:
 *   get:
...
try {
  ...
  if (brainRoutes.router) {
    app.use("/api/brain", brainRoutes.router);
    logger.logNodeActivity("CONDUCTOR", "  ∞ Brain Router: MOUNTED (GET/POST /api/brain/*)");
  }
} catch (err) {
...
// Start Dropzone Watcher
try {
  const { startDropzoneWatcher } = require('./src/services/dropzone-watcher');
  startDropzoneWatcher(process.cwd());
  logger.info({ context: 'boot' }, 'Started dropzone watcher successfully');
} catch (error) {
