---
name: heady-sync
description: "Run the canonical heady-sync Heady workflow. Cross-device git sync — manual, auto, status, or watch mode"
---
...
# `heady-sync` Codex shortcut

This skill is a generated Codex adapter for the canonical workflow at
`.agents/workflows/heady-sync.md`.

It is an explicit-only command shortcut. Invoke it as `$heady-sync`; do not
select it implicitly from ordinary task wording.

When invoked:

1. Read `.agents/workflows/heady-sync.md` completely before taking task actions.
2. Follow that workflow as the authoritative execution contract.
3. Treat text following `$heady-sync` as workflow input and preserve its approval boundaries.
