...
description: "Run the canonical edge-cache-warm Heady workflow. Edge cache warm — pre-warm Cloudflare cache for all domains after deploy"
---
...
# `edge-cache-warm` Codex shortcut

This skill is a generated Codex adapter for the canonical workflow at
`.agents/workflows/edge-cache-warm.md`.

It is an explicit-only command shortcut. Invoke it as `$edge-cache-warm`; do not
select it implicitly from ordinary task wording.

When invoked:

1. Read `.agents/workflows/edge-cache-warm.md` completely before taking task actions.
2. Follow that workflow as the authoritative execution contract.
3. Treat text following `$edge-cache-warm` as workflow input and preserve its approval boundaries.
