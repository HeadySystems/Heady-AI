---
name: health-check
description: "Run the canonical health-check Heady workflow. Multi-domain health check — validates all 9 Heady™ sites + Cloud Run + internal services"
---
...
# `health-check` Codex shortcut

This skill is a generated Codex adapter for the canonical workflow at
`.agents/workflows/health-check.md`.

It is an explicit-only command shortcut. Invoke it as `$health-check`; do not
select it implicitly from ordinary task wording.

When invoked:

1. Read `.agents/workflows/health-check.md` completely before taking task actions.
2. Follow that workflow as the authoritative execution contract.
3. Treat text following `$health-check` as workflow input and preserve its approval boundaries.
