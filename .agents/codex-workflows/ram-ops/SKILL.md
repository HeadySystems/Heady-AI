---
name: ram-ops
description: "Run the canonical ram-ops Heady workflow. RAM-first operations — all ops in vector space, external stores are projections"
---
...
# `ram-ops` Codex shortcut

This skill is a generated Codex adapter for the canonical workflow at
`.agents/workflows/ram-ops.md`.

It is an explicit-only command shortcut. Invoke it as `$ram-ops`; do not
select it implicitly from ordinary task wording.

When invoked:

1. Read `.agents/workflows/ram-ops.md` completely before taking task actions.
2. Follow that workflow as the authoritative execution contract.
3. Treat text following `$ram-ops` as workflow input and preserve its approval boundaries.
