---
name: vector-space-ops
description: "Run the canonical vector-space-ops Heady workflow. how to operate through Heady™'s vector space, bees, and event system for all work"
---
...
# `vector-space-ops` Codex shortcut

This skill is a generated Codex adapter for the canonical workflow at
`.agents/workflows/vector-space-ops.md`.

It is an explicit-only command shortcut. Invoke it as `$vector-space-ops`; do not
select it implicitly from ordinary task wording.

When invoked:

1. Read `.agents/workflows/vector-space-ops.md` completely before taking task actions.
2. Follow that workflow as the authoritative execution contract.
3. Treat text following `$vector-space-ops` as workflow input and preserve its approval boundaries.
