...
description: "Run the canonical agent-performance-review Heady workflow. Agent performance review — evaluate all agents for effectiveness and optimization"
---
...
# `agent-performance-review` Codex shortcut

This skill is a generated Codex adapter for the canonical workflow at
`.agents/workflows/agent-performance-review.md`.

It is an explicit-only command shortcut. Invoke it as `$agent-performance-review`; do not
select it implicitly from ordinary task wording.

When invoked:

1. Read `.agents/workflows/agent-performance-review.md` completely before taking task actions.
2. Follow that workflow as the authoritative execution contract.
3. Treat text following `$agent-performance-review` as workflow input and preserve its approval boundaries.
