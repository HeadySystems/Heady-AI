...
(function () {
  ...
  h3.style = 'margin:0;color:#e0e0ff';
  h3.textContent = '🧠 HeadyBuddy';
  const closeBtn = document.createElement('button');
  closeBtn.id = 'hb-close';
  closeBtn.style = 'background:none;border:none;color:#6b7280;cursor:pointer';
  closeBtn.textContent = '✕';
  header.appendChild(h3);
  header.appendChild(closeBtn);

  const auth = document.createElement('div');
  auth.className = 'hb-auth';
  const authBtnEl = document.createElement('button');
  authBtnEl.className = 'hb-auth-btn';
  authBtnEl.id = 'hb-auth-btn';
  authBtnEl.textContent = 'Authenticate';
  const userInfoEl = document.createElement('div');
  userInfoEl.className = 'hb-user';
  userInfoEl.id = 'hb-user-info';
  auth.appendChild(authBtnEl);
  auth.appendChild(userInfoEl);

  const messages = document.createElement('div');
  messages.className = 'hb-messages';
  messages.id = 'hb-messages';
  const sysMsg = document.createElement('div');
  sysMsg.className = 'hb-msg system';
  sysMsg.textContent = 'Connected to secure Heady 3D vector workspace.';
