...
function checkHealth(urlStr, callback, redirectCount = 0) {
    if (redirectCount > 5) {
        callback(false);
        return;
    }
    try {
        ...
        const req = proto.get(urlStr, (res) => {
            if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
                ...
                checkHealth(redirectUrl, callback, redirectCount + 1);
...
function activate(context) {
    ...
    // Check health on startup and bind listeners
    function runHealthCheck() {
        checkHealth(`${getApiUrl()}/api/pulse`, (success) => {
            if (success) {
    ...
    // Listen for config changes to dynamically update status bar and health
    context.subscriptions.push(
        vscode.workspace.onDidChangeConfiguration((e) => {
            if (e.affectsConfiguration("heady.apiUrl") || e.affectsConfiguration("heady.apiKey") || e.affectsConfiguration("heady.model")) {
                updateModelStatusBar();
                runHealthCheck();
            }
        })
