...
describe('Continuous Embedder Enterprise Capture', () => {
    test('queueForEmbed accepts valid content and increments queue length', () => {
    ...
    test('queueForEmbed rejects empty content', () => {
        const emptyResult = embedder.queueForEmbed('', { domain: 'manual' });
        expect(emptyResult).toBe(false);
    });

    test('analyst/system/user events are queued for embedding', () => {
        const before = embedder.getStats().queueLength;
    ...
    test('syncProjections queries vector memory', async () => {
        const vmMock = {
            calls: [],
    ...
    test('stats exposes projection state', () => {
        const stats = embedder.getStats();
        expect(typeof stats.projections).toBe('object');
    });

    test('live context snapshot returns ok and includes counts', async () => {
        const context = await embedder.buildLiveContextSnapshot();
        expect(context.ok).toBe(true);
        expect(typeof context.counts).toBe('object');
    });

    test('injectable templates returns ok and count respects topK', async () => {
