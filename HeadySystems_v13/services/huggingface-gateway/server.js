...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: HeadySystems_v13/services/huggingface-gateway/server.js                                                    ║
// ║  LAYER: root                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
...
const app = express();
const logger = createLogger('huggingface-gateway');
...
app.use(cors({
    origin: (origin, callback) => {
        if (!origin || origin.endsWith('.headysystems.com') || origin === (process.env.HUGGINGFACE_GATEWAY_URL || 'http://localhost:3351')) {
            callback(null, true);
        } else {
...
app.get('/health/live', (req, res) => {
  res.status(200).json({ status: 'ALIVE', service: 'huggingface-gateway', timestamp: new Date().toISOString() });
});

app.get('/health/ready', (req, res) => {
  res.status(_serviceReady ? 200 : 503).json({
    status: _serviceReady ? 'READY' : 'NOT_READY',
    service: 'huggingface-gateway',
...
app.get('/health', (req, res) => {
    res.status(200).json({ status: 'OK', service: 'huggingface-gateway', timestamp: Date.now() });
});
...
app.listen(port, () => {
    ...
    logger.info(`Service huggingface-gateway listening on port ${port}`);
});
