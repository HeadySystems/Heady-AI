...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: HeadySystems_v13/services/heady-brains/server.js                                                    ║
// ║  LAYER: root                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
...
app.use(cors({
    origin: (origin, callback) => {
        if (!origin || origin.endsWith('.headysystems.com') || origin === (process.env.HEADY_BRAINS_URL || 'http://localhost:3311')) {
            callback(null, true);
        } else {
...
app.get('/health/live', (req, res) => {
  res.status(200).json({ status: 'ALIVE', service: 'heady-brains', timestamp: new Date().toISOString() });
});
...
app.get('/health', (req, res) => {
    res.status(200).json({ status: 'OK', service: 'heady-brains', timestamp: Date.now() });
});
...
app.listen(port, () => {
    ...
    logger.info(`Service heady-brains listening on port ${port}`);
});
