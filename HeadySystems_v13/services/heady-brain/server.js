...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: HeadySystems_v13/services/heady-brain/server.js                                                    ║
// ║  LAYER: root                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
...
const app = express();
const logger = createLogger('heady-brain');
...
app.use(cors({
    origin: (origin, callback) => {
        if (!origin || origin.endsWith('.headysystems.com') || origin === (process.env.HEADY_BRAIN_URL || 'http://localhost:3310')) {
            callback(null, true);
        } else {
...
app.get('/health/live', (req, res) => {
  res.status(200).json({ status: 'ALIVE', service: 'heady-brain', timestamp: new Date().toISOString() });
});
...
app.get('/health', (req, res) => {
    res.status(200).json({ status: 'OK', service: 'heady-brain', timestamp: Date.now() });
});
...
app.listen(port, () => {
    ...
    logger.info(`Service heady-brain listening on port ${port}`);
});
