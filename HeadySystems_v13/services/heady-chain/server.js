...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: HeadySystems_v13/services/heady-chain/server.js                                                    ║
// ║  LAYER: root                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
...
const app = express();
const logger = createLogger('heady-chain');
...
app.use(cors({
    origin: (origin, callback) => {
        if (!origin || origin.endsWith('.headysystems.com') || origin === (process.env.HEADY_CHAIN_URL || 'http://localhost:3327')) {
            callback(null, true);
        } else {
...
app.get('/health/live', (req, res) => {
  res.status(200).json({ status: 'ALIVE', service: 'heady-chain', timestamp: new Date().toISOString() });
});
...
app.get('/health', (req, res) => {
    res.status(200).json({ status: 'OK', service: 'heady-chain', timestamp: Date.now() });
});
...
app.listen(port, () => {
    ...
    logger.info(`Service heady-chain listening on port ${port}`);
});
