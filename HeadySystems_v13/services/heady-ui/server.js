...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: HeadySystems_v13/services/heady-ui/server.js                                                    ║
// ║  LAYER: root                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
...
const app = express();
const logger = createLogger('heady-ui');
...
app.use(cors({
    origin: (origin, callback) => {
        if (!origin || origin.endsWith('.headysystems.com') || origin === (process.env.HEADY_UI_URL || 'http://localhost:3339')) {
            callback(null, true);
        } else {
...
app.get('/health/live', (req, res) => {
  res.status(200).json({ status: 'ALIVE', service: 'heady-ui', timestamp: new Date().toISOString() });
});
...
app.get('/health', (req, res) => {
    res.status(200).json({ status: 'OK', service: 'heady-ui', timestamp: Date.now() });
});
...
app.listen(port, () => {
    ...
    logger.info(`Service heady-ui listening on port ${port}`);
});
