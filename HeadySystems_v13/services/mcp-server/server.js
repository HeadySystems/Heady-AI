...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: HeadySystems_v13/services/mcp-server/server.js                                                    ║
// ║  LAYER: root                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
...
app.use(cors({
    origin: (origin, callback) => {
        if (!origin || origin.endsWith('.headysystems.com') || origin === (process.env.MCP_SERVER_URL || 'http://localhost:3346')) {
            callback(null, true);
        } else {
...
app.get('/health/live', (req, res) => {
  res.status(200).json({ status: 'ALIVE', service: 'mcp-server', timestamp: new Date().toISOString() });
});
...
app.get('/health', (req, res) => {
    res.status(200).json({ status: 'OK', service: 'mcp-server', timestamp: Date.now() });
});
...
app.listen(port, () => {
    ...
    logger.info(`Service mcp-server listening on port ${port}`);
});
