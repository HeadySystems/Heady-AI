...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: heady-system-build/services/heady-researcher/src/index.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/**
 ...
 * ║  HeadyResearcher — Perplexity-powered research and synthesis                                                  ║
 * ║  © 2026 HeadySystems Inc. — 60+ Provisional Patents             ║
 * ╚══════════════════════════════════════════════════════════════════╝
 *
 * Domain:    heady.io
 * Port:      3317
 * Upstreams: heady-inference-gateway, heady-vector-memory
 *
...
// Upstream: heady-inference-gateway
health.register('heady-inference-gateway', async () => {
  const url = process.env['HEADY_INFERENCE_GATEWAY_URL'];
  if (!url) return { status: 'degraded', message: 'Env var HEADY_INFERENCE_GATEWAY_URL not set' };
  try {
    const controller = new AbortController();
    const t = setTimeout(() => controller.abort(), TIMEOUTS.PHI_3);
    const r = await fetch(`${url}/health/live`, { signal: controller.signal });
    clearTimeout(t);
    return { status: r.ok ? 'healthy' : 'degraded', message: `heady-inference-gateway returned ${r.status}` };
  } catch (err) {
    return { status: 'unhealthy', message: err.message };
  }
});

// Upstream: heady-vector-memory
health.register('heady-vector-memory', async () => {
  const url = process.env['HEADY_VECTOR_MEMORY_URL'];
  if (!url) return { status: 'degraded', message: 'Env var HEADY_VECTOR_MEMORY_URL not set' };
  try {
    const controller = new AbortController();
    const t = setTimeout(() => controller.abort(), TIMEOUTS.PHI_3);
    const r = await fetch(`${url}/health/live`, { signal: controller.signal });
    clearTimeout(t);
    return { status: r.ok ? 'healthy' : 'degraded', message: `heady-vector-memory returned ${r.status}` };
  } catch (err) {
    return { status: 'unhealthy', message: err.message };
  }
});
