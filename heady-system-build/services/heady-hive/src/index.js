...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: heady-system-build/services/heady-hive/src/index.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/**
 ...
 * ║  HeadyHive — bee factory, spawning/managing 10K concurrent bees                                                  ║
 * ║  © 2026 HeadySystems Inc. — 60+ Provisional Patents             ║
 * ╚══════════════════════════════════════════════════════════════════╝
 *
 * Domain:    headybee.co
 * Port:      3306
 * Upstreams: heady-swarm-coordinator, heady-seventeen-swarm
 *
...
// Upstream: heady-swarm-coordinator
health.register('heady-swarm-coordinator', async () => {
  const url = process.env['HEADY_SWARM_COORDINATOR_URL'];
  if (!url) return { status: 'degraded', message: 'Env var HEADY_SWARM_COORDINATOR_URL not set' };
  try {
    const controller = new AbortController();
    const t = setTimeout(() => controller.abort(), TIMEOUTS.PHI_3);
    const r = await fetch(`${url}/health/live`, { signal: controller.signal });
    clearTimeout(t);
    return { status: r.ok ? 'healthy' : 'degraded', message: `heady-swarm-coordinator returned ${r.status}` };
  } catch (err) {
    return { status: 'unhealthy', message: err.message };
  }
});

// Upstream: heady-seventeen-swarm
health.register('heady-seventeen-swarm', async () => {
  const url = process.env['HEADY_SEVENTEEN_SWARM_URL'];
  if (!url) return { status: 'degraded', message: 'Env var HEADY_SEVENTEEN_SWARM_URL not set' };
  try {
    const controller = new AbortController();
    const t = setTimeout(() => controller.abort(), TIMEOUTS.PHI_3);
    const r = await fetch(`${url}/health/live`, { signal: controller.signal });
    clearTimeout(t);
    return { status: r.ok ? 'healthy' : 'degraded', message: `heady-seventeen-swarm returned ${r.status}` };
  } catch (err) {
    return { status: 'unhealthy', message: err.message };
  }
});
