...
// ║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
// ║  FILE: heady-system-build/services/heady-buddy/src/index.js                                                    ║
// ║  LAYER: backend/src                                                  ║
// ╚══════════════════════════════════════════════════════════════════╝
// HEADY_BRAND:END
/**
 ...
 * ║  HeadyBuddy — AI companion, persistent memory, empathic personas                                                  ║
...
health.register('heady-soul', async () => {
  ...
  if (!url) return { status: 'degraded', message: 'Env var HEADY_SOUL_URL not set' };
  try {
    const controller = new AbortController();
    const t = setTimeout(() => controller.abort(), TIMEOUTS.PHI_3);
    const r = await fetch(`${url}/health/live`, { signal: controller.signal });
    clearTimeout(t);
    return { status: r.ok ? 'healthy' : 'degraded', message: `heady-soul returned ${r.status}` };
  } catch (err) {
    return { status: 'unhealthy', message: err.message };
  }
});
